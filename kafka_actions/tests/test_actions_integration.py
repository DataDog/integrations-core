...
"""Integration tests for Kafka Actions - one test per action."""
...
class TestReadMessagesAction:
    ...
    def test_read_messages_with_filter(self, aggregator, dd_run_check):
        ...
        # Create test messages (Schema Registry format: magic byte + schema_id + JSON)
        messages = [
            MockKafkaMessage(
                key=b'key-1',
        ...
        instance = {
            ...
            'read_messages': {
                ...
                'filter': '.value.status == "active"',  # Only active messages
            },
        }

        # Run check with mocked kafka_client.consume_messages
        check = KafkaActionsCheck('kafka_actions', {}, [instance])
        with patch.object(check.kafka_client, 'consume_messages', return_value=messages) as mock_consume:
            dd_run_check(check)

            # Verify consume_messages was called with correct parameters
        ...
        # Verify events were emitted (only 2 messages with status=active)
        # Filter for message events only (exclude action success/failure events)
...
class TestCreateTopicAction:
    ...
    def test_create_topic(self, aggregator, dd_run_check):
        ...
        # Run check with mocked kafka_client.create_topic
        check = KafkaActionsCheck('kafka_actions', {}, [instance])
        with patch.object(check.kafka_client, 'create_topic', return_value=True) as mock_create:
            dd_run_check(check)
        ...
        # Verify service check was submitted
        aggregator.assert_service_check('kafka_actions.can_connect', count=0)  # Not emitted in tests
...
class TestUpdateTopicConfigAction:
    ...
    def test_update_topic_config(self, aggregator, dd_run_check):
        ...
        # Run check with mocked kafka_client.update_topic_config
        check = KafkaActionsCheck('kafka_actions', {}, [instance])
        with patch.object(check.kafka_client, 'update_topic_config', return_value=True) as mock_update:
            dd_run_check(check)

            # Verify update_topic_config was called with correct parameters
...
class TestDeleteTopicAction:
    ...
    def test_delete_topic(self, aggregator, dd_run_check):
        ...
        # Run check with mocked kafka_client.delete_topic
        check = KafkaActionsCheck('kafka_actions', {}, [instance])
        with patch.object(check.kafka_client, 'delete_topic', return_value=True) as mock_delete:
            dd_run_check(check)

            # Verify delete_topic was called with correct parameters
...
class TestDeleteConsumerGroupAction:
    ...
    def test_delete_consumer_group(self, aggregator, dd_run_check):
        ...
        # Run check with mocked kafka_client.delete_consumer_group
        check = KafkaActionsCheck('kafka_actions', {}, [instance])
        with patch.object(check.kafka_client, 'delete_consumer_group', return_value=True) as mock_delete_cg:
            dd_run_check(check)

            # Verify delete_consumer_group was called with correct parameters
...
class TestUpdateConsumerGroupOffsetsAction:
    ...
    def test_update_consumer_group_offsets(self, aggregator, dd_run_check):
        ...
        # Run check with mocked kafka_client.update_consumer_group_offsets
        check = KafkaActionsCheck('kafka_actions', {}, [instance])
        with patch.object(
            check.kafka_client, 'update_consumer_group_offsets', return_value=True
        ) as mock_update_offsets:
            dd_run_check(check)

            # Verify update_consumer_group_offsets was called with correct parameters
    ...
    def test_produce_message(self, aggregator, dd_run_check):
        ...
        instance = {
            ...
            'produce_message': {
                ...
                'partition': -1,
                'headers': {'source': 'datadog-agent', 'version': '1.0'},
            },
        }

        # Mock successful produce (must return dict with 'delivered' key)
        def mock_produce(*args, **kwargs):
            return {'delivered': True, 'partition': 0, 'offset': 12345}

        # Run check with mocked kafka_client.produce_message
        check = KafkaActionsCheck('kafka_actions', {}, [instance])
        with patch.object(check.kafka_client, 'produce_message', side_effect=mock_produce) as mock_prod:
            dd_run_check(check)

            # Verify produce_message was called with correct parameters
            mock_prod.assert_called_once()
            call_kwargs = mock_prod.call_args[1]
            assert call_kwargs['topic'] == 'test-topic'
            assert call_kwargs['key'] == 'test-key-123'
            assert call_kwargs['value'] == '{"order_id": "12345", "status": "pending"}'
            assert call_kwargs['partition'] == -1
            assert call_kwargs['headers'] == {'source': 'datadog-agent', 'version': '1.0'}
...
class TestReadMessagesAdvancedFiltering:
    ...
    def test_read_messages_nested_field_filter(self, aggregator, dd_run_check):
        ...
        # Create test messages with nested data
        messages = [
            MockKafkaMessage(
                key=b'order-1',
        ...
        # Configuration - filter for US gold users
        instance = {
        ...
        with patch.object(check.kafka_client, 'consume_messages', return_value=messages) as mock_consume:
            ...
            # Verify consume_messages was called
            mock_consume.assert_called_once()

        # Verify only 1 message matched (order-1 with US + gold)
        # Filter for message events only (exclude action success/failure events)
        events = [e for e in aggregator.events if e.get('event_type') == 'kafka_message']
