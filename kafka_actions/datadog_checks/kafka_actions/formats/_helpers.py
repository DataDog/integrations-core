...
"""Standalone deserialization helpers shared by the built-in format handlers.

These live here (rather than in ``..message_deserializer``) so the dependency
flows one way: ``message_deserializer`` -> ``formats``. The format handler
packages never import back from ``message_deserializer``.
"""
