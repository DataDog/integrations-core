...
from datadog_checks.teleport.config_models import discovery_overrides
from datadog_checks.teleport.config_models.instance import InstanceConfig
from datadog_checks.teleport.config_models.shared import SharedConfig
...
def _generated_candidates(service: Service) -> Iterator[dict[str, Any]]:
    ...
    for port in candidate_ports(service, [3000]):
        ctx = {'port': port}
        instance_data = {
            'teleport_url': 'http://{service.host}'.format(service=service, **ctx),
            'diag_port': '{port.number}'.format(service=service, **ctx),
        }
