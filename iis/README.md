...
To collect metrics on a per-site basis, you *must* use the `sites` option. The Agent collects metrics for each site you list and tags them with the site name. If you don't configure `sites`, the Agent collects the same metrics, but their values reflect totals across all sites.
