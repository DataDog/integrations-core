...
class MacAuditLogsCheck(AgentCheck):
    ...
    def collect_relevant_files(
        ...
        if not self.os.isdir(self.audit_logs_dir_path):
            err_message = (
        ...
        for file_name in self.os.listdir(self.audit_logs_dir_path):
            if file_name == "current":
                continue

            file_path = os.path.join(self.audit_logs_dir_path, file_name)
            if not self.os.isfile(file_path):
    ...
    def fetch_audit_logs(self, file_paths: list[str], time_filter_arg: str) -> tuple[bytes, bytes]:
        ...
        try:
            ...
            auditreduce_process = self.os.popen(
                ["sudo", "auditreduce", "-a", time_filter_arg, *file_paths],
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
                env={**os.environ, "TZ": "UTC"},
            )
            praudit_process = self.os.popen(
                ["sudo", "praudit", "-xsl"],
    ...
    def collect_data_from_files(
        ...
        for start_time, end_time, file_name in closed:
            file_path = os.path.join(self.audit_logs_dir_path, file_name)
            if not self.os.exists(file_path):
                self.log.info(
                    constants.LOG_TEMPLATE.format(
    ...
    def check(self, _):
        ...
        if self.monitor:
            ...
            timezone_offset = self.os.run(["date", "+%z"], capture_output=True, text=True).stdout.strip()
