...
from datadog_checks.mesos_master.config_models import discovery_overrides
from datadog_checks.mesos_master.config_models.instance import InstanceConfig
from datadog_checks.mesos_master.config_models.shared import SharedConfig
...
def _generated_candidates(service: Service) -> Iterator[dict[str, Any]]:
    ...
    for port in candidate_ports(service, [5050]):
        ctx = {'port': port}
        instance_data = {
            'url': 'http://{service.host}:{port.number}'.format(service=service, **ctx),
        }
