...
def test_legacy(aggregator, dd_default_hostname, dd_run_check, mock_performance_objects):
    mock_performance_objects(PERFORMANCE_OBJECTS)
    check = AspdotnetCheck('aspdotnet', {}, [{'host': dd_default_hostname, 'use_legacy_check_version': True}])
    check.hostname = dd_default_hostname
    dd_run_check(check)
