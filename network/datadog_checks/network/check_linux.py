...
class LinuxNetwork(Network):
    ...
    def check(self, _):
        ...
        if self.is_collect_cx_state_runnable(net_proc_base_location):
            try:
                ...
                for ip_version in ['4', '6']:
                    ...
                    output, _, _ = self.os.get_subprocess_output(["sh", "-c", cmd], self.log, env=ss_env)
                ...
                output, _, _ = self.os.get_subprocess_output(["netstat", "-n", "-u", "-t", "-a"], self.log)
                lines = output.splitlines()
        ...
        try:
            with self.os.open(proc_dev_path, 'r') as proc:
        ...
        for f in ['netstat', 'snmp']:
            ...
            try:
                with self.os.open(proc_data_path, 'r') as netstat:
                    while True:
        ...
        try:
            for metric_file in self.os.listdir(conntrack_files_location):
                if (
                    self.os.isfile(os.path.join(conntrack_files_location, metric_file))
    ...
    def _read_int_file(self, file_location):
        try:
            with self.os.open(file_location, 'r') as f:
                try:
    ...
    def _get_iface_sys_metrics(self, custom_tags):
        ...
        try:
            ifaces = self.os.listdir(sys_net_location)
        except OSError as e:
    ...
    def _collect_iface_queue_metrics(self, iface, iface_queues_location, custom_tags):
        try:
            iface_queues = self.os.listdir(iface_queues_location)
        except OSError as e:
