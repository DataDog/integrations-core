...
class BSDNetwork(Network):
    ...
    def check(self, _):
        ...
        try:
            output, _, _ = self.safe_os.get_subprocess_output(["netstat"] + netstat_flags, self.log)
        ...
        if self.is_collect_cx_state_runnable(net_proc_base_location):
            try:
                ...
                output_tcp, _, _ = self.safe_os.get_subprocess_output(["netstat", "-n", "-a", "-p", "tcp"], self.log)
                output_udp, _, _ = self.safe_os.get_subprocess_output(["netstat", "-n", "-a", "-p", "udp"], self.log)
