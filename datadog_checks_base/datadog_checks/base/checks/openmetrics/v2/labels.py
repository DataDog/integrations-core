...
class LabelAggregator:
        ...
        for metric, config in share_labels.items():
            ...
            if 'value' in config:
                ...
                try:
                    ...
                    raise TypeError(
                        f'Option `value` for metric `{metric}` of setting `share_labels` must represent an integer'
                    ) from None
                else:
                    data['value'] = value
    ...
    def collect(self, metric, config):
        ...
        if 'match' in config:
            ...
            if 'labels' in config:
                ...
                for sample in self.allowed_samples(metric, value):
                    label_set = set()
                    shared_labels = {}
    ...
    @staticmethod
    def allowed_samples(metric, value):
        if value is None:
