...
class AgentCheck(object):
    ...
    def generate_configs(cls, service):
        """
        Yield candidate instance configurations for service discovery.
    ...
    def discover_config(cls, service_json):
        """
        Return discovered instance configurations for an AD service payload.

        The Agent calls this classmethod through rtloader. Candidate instances
        are generated by ``generate_configs`` and accepted only when the real
        check can run against them successfully.
        """
        try:
            service = _discovery_service_from_json(service_json)
        except Exception:
            logging.getLogger(__name__).debug('config-discovery: could not parse service payload', exc_info=True)
            return '[]'
        ...
        try:
            ...
            logging.getLogger(__name__).debug('config-discovery: generate_configs failed', exc_info=True)
            return '[]'
        ...
        try:
            for candidate in candidates:
                try:
                    ...
                    check = cls(check_name, {}, [copy.deepcopy(candidate)])
                    with _suppress_discovery_side_effects(check):
                        error_report = check.run()
                    if not error_report:
                        accepted_configs.append(candidate)
                except Exception:
                    logging.getLogger(__name__).debug('config-discovery: candidate rejected', exc_info=True)
        except Exception:
            logging.getLogger(__name__).debug('config-discovery: generate_configs iteration failed', exc_info=True)

        return json.encode(accepted_configs)
