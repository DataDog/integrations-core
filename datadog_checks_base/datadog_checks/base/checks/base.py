    ...
    def discover_config(cls, service_json: str) -> str:
        ...
        try:
            ...
            logging.getLogger(__name__).warning(
                'config-discovery: generate_configs failed for %s', check_name, exc_info=True
            )
            return '[]'
        ...
        try:
            for candidate in candidates:
                try:
                    ...
                    logging.getLogger(__name__).warning(
                        'config-discovery: candidate raised unexpected error for %s', check_name, exc_info=True
                    )
        except Exception:
            logging.getLogger(__name__).warning(
                'config-discovery: generate_configs iteration failed for %s', check_name, exc_info=True
            )
