...
class AggregatorStub(object):
    ...
    def assert_network_devices_metadata(self, metadata_events, event_type):
        assert event_type in self._network_devices_metadata, "no events found for event type {}".format(event_type)
        actual_events = self._network_devices_metadata[event_type]
        for event in actual_events:
            event['collect_timestamp'] = 0
        assert metadata_events == self._network_devices_metadata[event_type]
