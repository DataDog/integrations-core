...
"""A unit-test double for :class:`~datadog_checks.base.utils.os_wrapper.OSWrapper`.
...
under test reaches the OS layer (the per-check ``self.os`` property or
the module-level ``self.os`` singleton). Test authors configure one object
...
``mock_os`` (from the ``datadog_checks.dev`` pytest plugin) to install
...
# The public method surface of OSWrapper, asserted in sync by the tests.
METHOD_NAMES = (
    "open",
    "os_open",
    "exists",
...
# Methods backed by the in-memory filesystem.
_FS_METHODS = (
    "open",
    "glob",
    "exists",
...
class MockOSWrapper:
    ...
    def __init__(self):
        # path -> contents for files, plus a set of directories. Parents implicit.
        ...
        # Back the FS methods from the start so an unconfigured read raises
        # FileNotFoundError rather than returning a MagicMock. Subprocess methods
        # stay bare until set_command_output, so return_value/side_effect work.
