...
class HTTPXResponseAdapter:
    ...
    def __init__(self, response: httpx.Response, default_chunk_size: int):
...
def _make_httpx_auth(config, logger):
    ...
    logger.warning('HTTPXWrapper does not yet support auth_type=%s; request will be sent without auth.', auth_type)
    return None


def _parse_uds_url(url):
    """Return (uds_path, http_url) for unix:// URLs, or (None, url) for normal URLs."""
    if not is_uds_url(url):
        return None, url
    parsed = urlparse(url)
    # quote_uds_url puts encoded socket path in netloc; we need the decoded path for httpx
    # e.g. unix://%2Fvar%2Frun%2Fdocker.sock/info -> uds_path=/var/run/docker.sock, path=/info
    netloc = parsed.netloc
    path = parsed.path or '/'
    if netloc:
        uds_path = unquote(netloc)
    else:
        # unix:///var/run/docker.sock -> path is /var/run/docker.sock
        uds_path = path
        path = '/'
...
class HTTPXWrapper:
    ...
    so that AgentCheck.http and other callers can use it interchangeably.
    """
    ...
    def __init__(self, instance, init_config, remapper=None, logger=None, session=None):
        ...
        if remapper is None:
            remapper = {}
        remapper = {**remapper, **DEFAULT_REMAPPED_FIELDS}
        ...
        self.persist_connections = self.tls_use_host_header or is_affirmative(config['persist_connections'])
        self._client = None  # persistent client when persist_connections
    ...
    def _request(self, method, url, options):
        ...
        with ExitStack() as stack:
            ...
            if isinstance(verify, str):
                ssl_context = create_ssl_context(dict(tls_config))
                verify = ssl_context
            elif verify is True:
            ...
            if uds_path:
                ...
                try:
                    response = client.request(method.upper(), request_url, headers=headers)
