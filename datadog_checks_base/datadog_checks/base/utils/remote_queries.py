...
"""JSON-page contracts, bounded page buffering, direct intake uploads, and producer timing.
...
# the page-upload and finalize phases by ``PageWriter``.
...
class RemoteQueryProducerTimings:
    ...
    def page_upload(self) -> Iterator[None]:
        """Bracket one whole ``put_page`` call — every attempt plus backoff — as page upload.
...
class PageWriter:
    ...
    def finish(self) -> dict[str, Any]:
        ...
        if self._buffer is not None:
            self._close_page()
        # Finalize includes its receipt verification in its own phase.
    ...
    def _close_page(self) -> None:
        ...
        try:
            self._buffer.seek(0)
            # The upload phase covers the whole put_page call (every attempt plus backoff);
            # the receipt verification below stays in the enclosing phase's bucket.
...
def upload_with_retry(
    ...
    first attempt) on the producer accumulator; only ``put_page`` passes one, so
    finalize and abort attempts are not page upload attempts.
    """
