...
class ConfigSetInvalidationStrategy(CacheInvalidationStrategy):
    """
    Cache invalidation strategy that invalidates the cache when a subset of the check's config options changes.

    Parameters:
        check: the check instance the key is going to be used for.
        init_config_options: the subset of init_config options to use to generate the invalidation token.
        instance_config_options: the subset of config options to use to generate the invalidation token.
    """
    ...
    def __init__(
        ...
        *,
        ...
        # Config cannot change on the fly, so we can cache the invalidation token
        self.__invalidation_token: str | None = None

    def invalidation_token(self) -> str:
        if self.__invalidation_token is not None:
            return self.__invalidation_token
        ...
        self.__invalidation_token = str(hash_mutable(selected_values)).replace("-", "")
        return self.__invalidation_token
