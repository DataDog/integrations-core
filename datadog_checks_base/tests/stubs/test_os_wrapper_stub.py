...
def test_method_names_covers_the_full_interface_surface():
    ...
    A method added to OSWrapper but missing here is not redirected by the
    `mock_os` fixture, so a test using the fixture would silently
    reach the real filesystem instead of the fake.
    """
    from datadog_checks.base.utils.os_wrapper import OSWrapper

    real = {name for name in dir(OSWrapper) if not name.startswith("_") and callable(getattr(OSWrapper, name))}
