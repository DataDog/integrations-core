...
def test_label_joins_new(benchmark, dd_run_check, mock_http_response, fixture_ksm):
    ...
    instance = {
        ...
        'metrics': ['.+'],
        'share_labels': {
            'kube_pod_info': {'match': ['pod', 'namespace'], 'labels': ['node'], 'value': 1},
            '1': {'match': ['pod', 'namespace'], 'labels': ['node'], 'value': 1},
            '2': {'match': ['pod', 'namespace'], 'labels': ['node'], 'value': 1},
            '3': {'match': ['pod', 'namespace'], 'labels': ['node'], 'value': 1},
            '4': {'match': ['pod', 'namespace'], 'labels': ['node'], 'value': 1},
            '5': {'match': ['pod', 'namespace'], 'labels': ['node'], 'value': 1},
            '6': {'match': ['pod', 'namespace'], 'labels': ['node'], 'value': 1},
            '7': {'match': ['pod', 'namespace'], 'labels': ['node'], 'value': 1},
            '8': {'match': ['pod', 'namespace'], 'labels': ['node'], 'value': 1},
            '9': {'match': ['pod', 'namespace'], 'labels': ['node'], 'value': 1},
        },
    }
    c = OpenMetricsBaseCheck('test', {}, [instance])
