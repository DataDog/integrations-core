...
NEXT_PREV_CASES = [
    pytest.param("* * * * *", BASE_TS, utc_ts(2026, 1, 1, 0, 1), utc_ts(2025, 12, 31, 23, 59), id="every-minute"),
    pytest.param("0 * * * *", BASE_TS, utc_ts(2026, 1, 1, 1), utc_ts(2025, 12, 31, 23), id="hourly"),
    pytest.param("0 9 * * *", BASE_TS, utc_ts(2026, 1, 1, 9), utc_ts(2025, 12, 31, 9), id="daily-at-9"),
    pytest.param(
        "0 9 * * *",
        utc_ts(2026, 1, 1, 9),
        utc_ts(2026, 1, 2, 9),
    ...
    pytest.param(
        "0 9 * * *", utc_ts(2026, 1, 1, 9, 1), utc_ts(2026, 1, 2, 9), utc_ts(2026, 1, 1, 9), id="one-min-after-9"
    ),
    pytest.param(
        "*/5 * * * *", utc_ts(2026, 1, 1, 0, 3), utc_ts(2026, 1, 1, 0, 5), utc_ts(2026, 1, 1, 0, 0), id="every-5-mid"
    ),
    pytest.param("0 9 * * 1", BASE_TS, utc_ts(2026, 1, 5, 9), utc_ts(2025, 12, 29, 9), id="weekly-monday-9"),
    pytest.param("0 0 1 1 *", BASE_TS, utc_ts(2027, 1, 1), utc_ts(2025, 1, 1), id="new-year-from-new-year"),
    pytest.param("0 0 29 2 *", utc_ts(2026, 3, 1), utc_ts(2028, 2, 29), utc_ts(2024, 2, 29), id="leap-day"),
    pytest.param("0 0 1 * *", utc_ts(2026, 1, 1, 12), utc_ts(2026, 2, 1), utc_ts(2026, 1, 1), id="monthly-first"),
    pytest.param(
        "7-23/4 * * * *", BASE_TS, utc_ts(2026, 1, 1, 0, 7), utc_ts(2025, 12, 31, 23, 23), id="range-step-minutes"
    ),
    pytest.param("0 0 1 */3 *", BASE_TS, utc_ts(2026, 4, 1), utc_ts(2025, 10, 1), id="quarterly-first"),
    pytest.param("0 0 1 * 1", BASE_TS, utc_ts(2026, 1, 5), utc_ts(2025, 12, 29), id="vixie-or-dom1-or-mon"),
    pytest.param("0 0 * * 0", BASE_TS, utc_ts(2026, 1, 4), utc_ts(2025, 12, 28), id="sunday-as-0"),
    pytest.param("0 0 * * 7", BASE_TS, utc_ts(2026, 1, 4), utc_ts(2025, 12, 28), id="sunday-as-7"),
    pytest.param("0 0 31 * *", utc_ts(2026, 3, 1), utc_ts(2026, 3, 31), utc_ts(2026, 1, 31), id="31st-skips-feb"),
    pytest.param(
        "15,45 * * * *", BASE_TS, utc_ts(2026, 1, 1, 0, 15), utc_ts(2025, 12, 31, 23, 45), id="list-15-and-45"
    ),
    pytest.param(
        "* * * * *",
        utc_ts(2026, 1, 1) + 0.5,
...
def test_next_tick(expression: str, anchor: float, expected_next: float, _expected_prev: float) -> None:
    ...
    assert got == expected_next, f"{expression} from {iso(anchor)}: got {iso(got)}, expected {iso(expected_next)}"
...
def test_previous_tick(expression: str, anchor: float, _expected_next: float, expected_prev: float) -> None:
    ...
    assert got == expected_prev, f"{expression} before {iso(anchor)}: got {iso(got)}, expected {iso(expected_prev)}"
...
@pytest.mark.parametrize(
    "expression,anchor",
    [
        ("0 9 * * *", utc_ts(2026, 1, 1, 9, 0)),
        ("* * * * *", utc_ts(2026, 1, 1, 0, 0)),
        ("0 0 1 1 *", utc_ts(2026, 1, 1, 0, 0)),
    ],
)
...
def test_scheduler_fresh_no_ticks_due(daily_at_nine: CronScheduler) -> None:
    out = list(daily_at_nine.due_ticks(now=utc_ts(2026, 1, 1, 8, 30)))
    assert out == []
    assert daily_at_nine.next_tick == utc_ts(2026, 1, 1, 9)


def test_scheduler_fires_after_tick_crossed(daily_at_nine: CronScheduler) -> None:
    list(daily_at_nine.due_ticks(now=utc_ts(2026, 1, 1, 8, 30)))
    out = list(daily_at_nine.due_ticks(now=utc_ts(2026, 1, 1, 9, 0)))
    assert out == [utc_ts(2026, 1, 1, 9)]
    assert daily_at_nine.next_tick == utc_ts(2026, 1, 2, 9)


def test_scheduler_idempotent_within_same_polling_window(daily_at_nine: CronScheduler) -> None:
    list(daily_at_nine.due_ticks(now=utc_ts(2026, 1, 1, 8, 55)))
    poll = utc_ts(2026, 1, 1, 9, 1)
    first = list(daily_at_nine.due_ticks(now=poll))
    second = list(daily_at_nine.due_ticks(now=poll))
    assert first == [utc_ts(2026, 1, 1, 9)]
    assert second == []


def test_scheduler_does_not_fire_past_tick_without_startup_lookback() -> None:
    s = CronScheduler("0 9 * * *")
    out = list(s.due_ticks(now=utc_ts(2026, 1, 1, 9, 30)))
    assert out == []
    assert s.next_tick == utc_ts(2026, 1, 2, 9)


def test_scheduler_does_not_block(daily_at_nine: CronScheduler) -> None:
...
def test_scheduler_yields_multiple_missed_ticks(daily_at_nine: CronScheduler) -> None:
    list(daily_at_nine.due_ticks(now=utc_ts(2026, 1, 1, 8, 30)))
    out = list(daily_at_nine.due_ticks(now=utc_ts(2026, 1, 4, 12, 0)))
    assert out == [
        utc_ts(2026, 1, 1, 9),
        utc_ts(2026, 1, 2, 9),
        utc_ts(2026, 1, 3, 9),
    ...
    assert daily_at_nine.next_tick == utc_ts(2026, 1, 5, 9)


def test_scheduler_advances_state_when_lookback_window_misses() -> None:
    s = CronScheduler("0 * * * *", startup_lookback=300)
    out = list(s.due_ticks(now=utc_ts(2026, 1, 1, 12, 30)))
    assert out == []
    assert s.next_tick == utc_ts(2026, 1, 1, 13)


@pytest.mark.parametrize(
    "expression,lookback,now,expected",
    [
        pytest.param("0 * * * *", 0.0, utc_ts(2026, 1, 1, 12, 0), [], id="no-lookback"),
        pytest.param(
            "0 * * * *",
            7200,
            utc_ts(2026, 1, 1, 12, 3),
            [utc_ts(2026, 1, 1, 12, 0)],
            id="generous-window-catches-prev",
        ),
        pytest.param("0 * * * *", 300, utc_ts(2026, 1, 1, 12, 30), [], id="narrow-window-misses"),
        pytest.param(
            "*/10 * * * *",
            600,
            utc_ts(2026, 1, 1, 12, 3),
...
def test_scheduler_default_now_uses_wall_clock(monkeypatch: pytest.MonkeyPatch) -> None:
    fixed = utc_ts(2026, 1, 1, 8, 30)
    monkeypatch.setattr(time, "time", lambda: fixed)
    s = CronScheduler("0 9 * * *")
    assert list(s.due_ticks()) == []
    assert s.next_tick == utc_ts(2026, 1, 1, 9)


def test_scheduler_due_ticks_returns_iterator(daily_at_nine: CronScheduler) -> None:
    result = daily_at_nine.due_ticks(now=utc_ts(2026, 1, 1, 8, 30))
    assert iter(result) is result


def test_scheduler_advances_correctly_across_ten_ticks() -> None:
    s = CronScheduler("0 * * * *")
    list(s.due_ticks(now=utc_ts(2026, 1, 1, 0, 30)))
    collected: list[float] = []
    for hour in range(1, 11):
        collected.extend(s.due_ticks(now=utc_ts(2026, 1, 1, hour, 0)))
    assert collected == [utc_ts(2026, 1, 1, hour) for hour in range(1, 11)]
