...
# Reads of the concrete client that the protocol deliberately does not declare, and that the
# integrations still performing them have yet to be migrated off. Each one is a hole a non-requests
# backend would fall through, so the set only ever shrinks: the tripwire below fails both on a new
# entry and on a stale one.
#   session / _session   reach straight into the requests session, and no backend-neutral member
#                        replaces them; the check has to be reworked instead.
#   no_proxy_uris        superseded by the should_bypass_proxy() capability.
UNDECLARED_CLIENT_READS = {
    'avi_vantage/datadog_checks/avi_vantage/check.py reads ._session',
    'avi_vantage/datadog_checks/avi_vantage/check.py reads .session',
    'cisco_aci/datadog_checks/cisco_aci/api.py reads .session',
    'hpe_aruba_edgeconnect/datadog_checks/hpe_aruba_edgeconnect/client.py reads .session',
    'http_check/datadog_checks/http_check/http_check.py reads ._session',
    'http_check/datadog_checks/http_check/http_check.py reads .no_proxy_uris',
    'http_check/datadog_checks/http_check/http_check.py reads .session',
    'openstack_controller/datadog_checks/openstack_controller/api/api_sdk.py reads .session',
}
        ...
        for path in sorted(repo_root.glob('*/datadog_checks/**/*.py')):
            ...
            for node in ast.walk(tree):
                if (
                    ...
                    undeclared.add(f'{path.relative_to(repo_root)} reads .{node.attr}')

        new_couplings = undeclared - UNDECLARED_CLIENT_READS
        assert not new_couplings, 'client members read by shipped code but not declared on HTTPClient:\n' + '\n'.join(
            sorted(new_couplings)
        )

        resolved = UNDECLARED_CLIENT_READS - undeclared
        assert not resolved, 'these reads are gone; drop them from UNDECLARED_CLIENT_READS:\n' + '\n'.join(
            sorted(resolved)
        )
