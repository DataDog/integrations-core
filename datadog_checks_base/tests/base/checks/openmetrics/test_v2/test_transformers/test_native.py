...
def test_basic(aggregator, dd_run_check, mock_http_response):
    ...
    check = get_check({'metrics': ['go_memstats_alloc_bytes']})
    dd_run_check(check)
...
def test_gauge_and_total_counter_collision(aggregator, dd_run_check, mock_http_response):
    """
    OpenMetrics strips the `_total` suffix from counter family names, so a gauge and its
    `_total` counter counterpart collapse into the same metric name. Left untyped (the
    default `native` type), both samples must still be submitted under their own type.
    """
    mock_http_response(
        """
        # HELP go_memstats_alloc_bytes Number of bytes allocated and still in use.
