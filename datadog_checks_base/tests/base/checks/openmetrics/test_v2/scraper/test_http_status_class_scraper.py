...
def test_http_status_class_scraper(
    ...
    # Already applied on the first run now, and on every run after that.
    ...
    dd_run_check(check)
    aggregator.assert_metric_has_tag(
        "test.http_client_request_size.count", "service_version:1.0.0", count=target_info_tag_count * 2
    )
