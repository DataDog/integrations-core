...
def test_auth_fallback_reraises_a_status_error_carrying_no_response():
    ...
    with mock.patch.object(
        type(check.http), 'get', autospec=True, side_effect=HTTPClientStatusError('failed to fetch auth token')
    ):
...
def test_single_server_keeps_its_cache_entry_when_the_status_is_unknown():
    ...
    with (
        ...
        mock.patch.object(
            check, '_make_request_with_auth_fallback', side_effect=HTTPClientStatusError('failed to fetch auth token')
        ),
    ):
        with pytest.raises(HTTPClientStatusError):
...
def test_check_does_not_back_off_when_the_status_is_unknown():
    ...
    with mock.patch.object(check, 'ensure_auth_scope', side_effect=HTTPClientStatusError('failed to fetch auth token')):
