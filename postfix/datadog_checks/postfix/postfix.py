...
class PostfixCheck(AgentCheck):
    ...
    def _get_postqueue_stats(self, postfix_config_dir, tags):
        pc_output, _, _ = self.os.get_subprocess_output(['postconf', 'authorized_mailq_users'], self.log, False)
        ...
        output, _, _ = self.os.get_subprocess_output(['postqueue', '-c', postfix_config_dir, '-p'], self.log, False)
        active_count = 0
        hold_count = 0
        deferred_count = 0
    ...
    def _get_queue_count(self, directory, queues, tags):
        for queue in queues:
            ...
            if not self.os.exists(queue_path):
                raise Exception('{} does not exist'.format(queue_path))

            count = 0
            if os.geteuid() == 0:
                # dd-agent is running as root (not recommended)
                count = sum(len(files) for root, dirs, files in self.os.walk(queue_path))
            else:
                # can dd-agent user run sudo?
                test_sudo = ['sudo', '-l']
                _, _, exit_code = self.os.get_subprocess_output(test_sudo, self.log, False)
                if exit_code == 0:
                    ...
                    output, _, _ = self.os.get_subprocess_output(cmd, self.log, False)
    ...
    def _collect_metadata(self):
        try:
            pc_output, _, _ = self.os.get_subprocess_output(['postconf', 'mail_version'], self.log, False)
