...
class MySqlSchemaCollector(SchemaCollector):
    ...
    def _get_tables_query(self):
        schemas_query = SQL_SCHEMAS_DATABASES
        tables_query = SQL_SCHEMAS_TABLES
        columns_query = SQL_SCHEMAS_COLUMNS
        indexes_query = (
            SQL_SCHEMAS_INDEXES_8_0_13
            if self._check.version.flavor == 'MySQL' and self._check.version.version_compatible((8, 0, 13))
            else SQL_SCHEMAS_INDEXES
        )
        constraints_query = SQL_SCHEMAS_FOREIGN_KEYS
        partition_query = SQL_SCHEMAS_PARTITION
        ...
        query = f"""
            SELECT schema_tables.schema_name, schema_tables.default_character_set_name,
                schema_tables.default_collation_name,
            ...
            GROUP BY schema_tables.schema_name, schema_tables.default_character_set_name,
            schema_tables.default_collation_name, schema_tables.table_name, schema_tables.engine,
            schema_tables.row_format, schema_tables.create_time
            ;
        """
        return query
