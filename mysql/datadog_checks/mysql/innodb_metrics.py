...
def get_stats_from_innodb_status(db):
            ...
            try:
                if len(row) == 8:
                    ...
                    # (len(row) == 16) Pending normal aio reads: 0 [0, 0, 0, 0] , aio writes: 0 [0, 0] ,
                        ...
                        log.warning("Can't parse result line %s", line)
                elif len(row) == 18:
                ...
                log.warning("Can't parse result line %s: %s", line, e)
    ...
    try:
        ...
        log.error("Not all InnoDB LSN metrics available, unable to compute: %s", e)
...
def process_innodb_stats(results, options, metrics):
    innodb_keys = [
        'Innodb_page_size',
    ...
    try:
        ...
        log.error("Not all InnoDB buffer pool metrics are available, unable to compute: %s", e)
