...
def from_argocd_kube_app_name(service: Service) -> Iterator[dict[str, ArgoCDDiscoveryEndpoints]]:
    """Yield the role-specific metrics endpoint for an Argo CD container."""
    tags = tagger.tag(container_tagger_entity_id(service.id), tagger.LOW) or []

    # Ignore an irrelevant init container which is part of the official manifests.
    if 'kube_container_name:copyutil' in tags:
        return
