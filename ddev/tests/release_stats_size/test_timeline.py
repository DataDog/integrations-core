...
from ddev.cli.release.stats.size.timeline import (
    format_commit_data,
    get_dependencies,
    get_dependency_data,
    get_dependency_size,
...
def test_get_compressed_files():
    with (
        ...
        patch(
            "ddev.cli.release.stats.size.timeline.os.path.relpath",
            return_value=os.path.join("datadog_checks", "__about__.py"),
        ),
        patch("ddev.cli.release.stats.size.timeline.os.path.exists", return_value=True),
        patch("ddev.cli.release.stats.size.utils.common_funcs.get_gitignore_files", return_value=set()),
        patch("ddev.cli.release.stats.size.timeline.is_valid_integration_file", return_value=True),
        patch("ddev.cli.release.stats.size.timeline.compress", return_value=1234),
        patch("ddev.cli.release.stats.size.timeline.extract_version_from_about_py", return_value='1.1.1'),
    ):
...
def test_get_compressed_files_deleted_only():
    ...
    with (
        patch("ddev.cli.release.stats.size.utils.common_funcs.get_gitignore_files", return_value=set()),
        patch("ddev.cli.release.stats.size.timeline.os.walk", return_value=[]),
        patch(
            "ddev.cli.release.stats.size.timeline.os.path.relpath",
            side_effect=lambda path, _: path.replace(f"{repo_path}{os.sep}", ""),
        ),
        patch("ddev.cli.release.stats.size.timeline.os.path.exists", return_value=False),
    ):
...
dep2 @ https://example.com/dep2/dep2-1.1.2-.whl"""
    with patch("ddev.cli.release.stats.size.timeline.open", mock_open(read_data=content)):
...
def test_get_dependency_size():
    ...
    with patch("ddev.cli.release.stats.size.timeline.requests.head", return_value=mock_response):
...
def test_get_compressed_dependencies():
    with (
        patch("ddev.cli.release.stats.size.timeline.os.path.exists", return_value=True),
        patch("ddev.cli.release.stats.size.timeline.os.path.isdir", return_value=True),
        patch("ddev.cli.release.stats.size.timeline.os.path.isfile", return_value=True),
        patch("ddev.cli.release.stats.size.timeline.os.listdir", return_value=["linux-x86_64_3.12.txt"]),
        patch(
            "ddev.cli.release.stats.size.timeline.get_dependency_data",
            return_value=("https://example.com/dep1/dep1-1.1.1-.whl", '1.1.1'),
        ),
        patch("ddev.cli.release.stats.size.timeline.requests.head", return_value=make_mock_response("12345")),
    ):
