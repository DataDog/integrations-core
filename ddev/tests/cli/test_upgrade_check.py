...
class TestReadLastRun:
    def test_upgrade_reads_valid_cache_file(self, tmp_path):
        ...
        version, date = read_last_run(cache_file)

        assert version == Version("1.6.0")
        assert date == datetime.fromisoformat("2023-04-11T10:56:39.786412")

    def test_upgrade_returns_none_when_file_does_not_exist(self, tmp_path):
    ...
    def test_upgrade_returns_none_when_data_is_invalid(self, tmp_path):
...
class TestWriteLastRun:
    def test_upgrade_writes_cache_file_and_creates_parent_dirs(self, tmp_path):
...
class TestUpgradeCheck:
    ...
    def test_upgrade_fetches_from_pypi_and_notifies_when_upgrade_available(self, tmp_path, mocker):
        ...
        mock_get = mocker.patch("ddev.cli.upgrade_check.requests.get", return_value=mock_response)
        mock_atexit = mocker.patch("ddev.cli.upgrade_check.atexit.register")
    ...
    def test_upgrade_uses_cache_when_fresh(self, tmp_path, mocker):
    ...
    def test_upgrade_no_notification_when_up_to_date(self, tmp_path, mocker):
        ...
        mocker.patch("ddev.cli.upgrade_check.requests.get", return_value=mock_response)
        mock_atexit = mocker.patch("ddev.cli.upgrade_check.atexit.register")
    ...
    def test_upgrade_handles_request_exception_gracefully(self, tmp_path, mocker):
