...
async def test_pr_comment_log_messages_name_their_revision():
    ...
    assert "PR comment revision 1 written" in messages
    assert "PR comment revision 2 written" in messages
    assert "Stale PR comment revision 1 ignored (latest rendered is 2)" in messages
    assert "PR comment written for cancelled run" in messages
