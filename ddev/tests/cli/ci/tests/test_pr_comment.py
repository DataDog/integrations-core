...
def test_a_batch_problem_its_own_targets_already_explain_is_not_said_twice():
    ...
    # The header follows the same rule, so the batch is not counted there either.
    assert "Dispatcher could not collect results for 1 batch." not in body
...
def test_a_finished_run_says_nothing_failed_only_when_that_is_the_whole_truth():
    ...
    batch_only = render_comment(
        DispatcherProgress(
            batches=(batch_progress("batch-01", job_progress(attempt(), target="ntp"), status=Status.FAILURE),),
            done=True,
        )
    )
    ...
    # The heading calls this run failed, so the totals beside it cannot call it clean.
    assert "## ❌ Dispatcher tests: failed" in batch_only
    assert "nothing failed" not in batch_only
