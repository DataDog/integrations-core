...
def nothing_collected() -> DispatcherProgress:
    ...
    return DispatcherProgress(
        batches=(
            ...
            # A batch problem its own target does not explain: the target lost its artifacts, the
            # batch timed out, and the two are different events, so both are reported.
            batch_progress(
                "batch-03",
...
def batch_only_failure() -> DispatcherProgress:
    ...
    Every job in it passed, so this is also the run whose totals must not read as a clean pass.
    """
    return DispatcherProgress(
        batches=(
            batch_progress(
