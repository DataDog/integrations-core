...
"""Tests for the OS wrapper erosion guard.
...
class MyCheck(AgentCheck):
    def check(self, _):
        with self.os.open(self.instance['path']) as f:
            f.read()
        self.os.run(['ls'], capture_output=True)
"""
...
class MyCheck(AgentCheck):
    def check(self, _):
        unchecked_os.run([self.instance['binary']], capture_output=True)
"""
...
# SKIP_OS_WRAPPER_VALIDATION: bundled asset, no config-derived path
DATA = unchecked_os.open('bundled.json')
...
class MyCheck(AgentCheck):
    def check(self, _):
        # SKIP_OS_WRAPPER_VALIDATION: reads a fixed, non-config path
...
def test_config_models_are_ignored(fake_repo, ddev):
    ...
    result = ddev('validate', 'os-wrapper', 'gen')
    assert result.exit_code == 0, result.output
...
class MyCheck(AgentCheck):
    def check(self, _):
        # SKIP_OS_WRAPPER_VALIDATION: fixed literal probe with no config input,
...
class MyCheck(AgentCheck):
    def check(self, _):
        # SKIP_OS_WRAPPER_VALIDATION: too far away to apply
