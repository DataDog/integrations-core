...
def mock_size_diff_dependencies():
    ...
    with (
        patch(
            "ddev.cli.release.size.diff.get_valid_platforms",
            return_value=({"linux-x86_64", "macos-x86_64", "linux-aarch64", "macos-aarch64", "windows-x86_64"}),
        ),
        patch(
            "ddev.cli.release.size.diff.get_valid_versions",
            return_value=({"3.12"}),
        ),
        patch("ddev.cli.release.size.diff.GitRepo.__enter__", return_value=mock_git_repo),
        patch("ddev.cli.release.size.diff.GitRepo.__exit__", return_value=None),
        patch("ddev.cli.release.size.diff.GitRepo.checkout_commit"),
        patch("ddev.cli.release.size.utils.common_funcs.tempfile.mkdtemp", return_value="fake_repo"),
        patch("ddev.cli.release.size.diff.get_files", side_effect=get_compressed_files_side_effect),
        patch("ddev.cli.release.size.diff.get_dependencies", side_effect=get_compressed_dependencies_side_effect),
        patch("ddev.cli.release.size.diff.format_modules", side_effect=lambda m, *_: m),
...
def test_diff_no_args(ddev, mock_size_diff_dependencies):
    assert ddev("release", "size", "diff", "commit1", "commit2").exit_code == 0
    assert ddev("release", "size", "diff", "commit1", "commit2", "--compressed").exit_code == 0
    assert ddev("release", "size", "diff", "commit1", "commit2", "--format", "csv,markdown,json,png").exit_code == 0
    assert ddev("release", "size", "diff", "commit1", "commit2", "--show-gui").exit_code == 0


def test_diff_with_platform_and_version(ddev, mock_size_diff_dependencies):
    assert (
        ddev(
            "release", "size", "diff", "commit1", "commit2", "--platform", "linux-aarch64", "--python", "3.12"
        ).exit_code
        == 0
    )
    assert (
...
def test_diff_no_differences(ddev):
    ...
    with (
        patch("ddev.cli.release.size.diff.GitRepo.__enter__", return_value=fake_repo),
        patch("ddev.cli.release.size.diff.GitRepo.__exit__", return_value=None),
        patch(
            "ddev.cli.release.size.diff.get_valid_platforms",
            return_value=({"linux-x86_64", "macos-x86_64", "linux-aarch64", "macos-aarch64", "windows-x86_64"}),
        ...
        patch("ddev.cli.release.size.utils.common_funcs.os.listdir", return_value=["linux-aarch64_3.12"]),
        patch(
            "ddev.cli.release.size.diff.get_files",
            return_value=[
        ...
        patch(
            "ddev.cli.release.size.diff.get_dependencies",
            return_value=[
...
def test_diff_invalid_platform(ddev):
    ...
    with (
        patch("ddev.cli.release.size.diff.GitRepo", return_value=mock_git_repo),
        patch(
            "ddev.cli.release.size.diff.get_valid_platforms",
            return_value=({"linux-x86_64", "macos-x86_64", "linux-aarch64", "macos-aarch64", "windows-x86_64"}),
        ),
        ...
        result = ddev(
            "release", "size", "diff", "commit1", "commit2", "--platform", "linux", "--python", "3.12", "--compressed"
        )
...
def test_diff_invalid_platform_and_version(ddev):
    ...
    with (
        ...
        result = ddev(
            "release", "size", "diff", "commit1", "commit2", "--platform", "linux", "--python", "2.10", "--compressed"
        )
        assert result.exit_code != 0
