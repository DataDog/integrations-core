...
def test_worktrees(repository):
    repo = Repository(repository.path.name, str(repository.path))

    # This worktree is present in the local_clone repo already
    assert repo.git.worktrees == ["wt"]

    # Add a new worktree
    repo.git.capture("worktree", "add", "wt2", "HEAD^")
    assert repo.git.worktrees == ["wt", "wt2"]

    # Remove it
    repo.git.capture("worktree", "remove", "wt2")
    assert repo.git.worktrees == ["wt"]
