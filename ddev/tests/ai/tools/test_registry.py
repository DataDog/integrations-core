...
def from_names(tool_names: list[str], tmp_path, *, owner_id: str = OWNER_ID) -> ToolRegistry:
    return ToolRegistry.from_names(
        ...
        agent_config=AgentConfig.model_construct(
            name="test_agent", provider="anthropic", tools=tool_names, system_prompt_path=Path("/fake.md")
        ),
...
def test_from_names_spawn_subagent_gets_runtime_context(tmp_path):
    ...
    assert tool._agent_config == AgentConfig(
        name="test_agent", tools=["read_file", "spawn_subagent"], system_prompt_path=Path("/fake.md")
    )
...
def test_from_names_reuses_supplied_file_registry(tmp_path):
    ...
    reg_a = ToolRegistry.from_names(
        ...
        agent_config=AgentConfig(
            name="test_agent", tools=["read_file", "create_file"], system_prompt_path=Path("/fake.md")
        ),
        process_factory=PROCESS_FACTORY,
    )
