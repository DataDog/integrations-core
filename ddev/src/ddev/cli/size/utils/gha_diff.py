...
def display_diffs_to_html_short(diffs, platform, python_version):
    ...
    text += f"Total changed: \n\t {total_changed_sign}{convert_to_human_readable_size(abs(total_changed_compressed))} "
    text += f"(Compressed) \n\t {total_changed_sign}{convert_to_human_readable_size(abs(total_changed_uncompressed))} "
    text += "(Uncompressed)\n"
    text += "</details>\n"
