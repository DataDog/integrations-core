...
def format_pr_comment(message: UpdatePRComment) -> str:
    ...
    lines = [
        ...
        "",
        _summary(message.done, success_total, failed_total),
    ]
...
def _summary(done: bool, success_total: int, failed_total: int) -> str:
    """Build the one-line counts summary."""
    counts = f"**{success_total}** succeeded, **{failed_total}** failed"
    if not done:
        return f"{counts}. Workflows are still running…"
    return f"{counts}."
