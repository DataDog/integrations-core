...
def _totals(progress: DispatcherProgress) -> str:
    ...
    if progress.done and not progress.failed and not any(_uncollected_counts(progress)):
...
def _batch_notes(progress: DispatcherProgress) -> list[str]:
    ...
    for batch in progress.batches:
        ...
        # One problem, one explanation. A batch error whose own targets already say their results
        # went missing is that same news a second time, in a unit the reader cannot navigate by.
        if batch.error is not None and not any(
            job.latest is not None and job.latest.error is not None for job in batch.jobs_progress
        ):
