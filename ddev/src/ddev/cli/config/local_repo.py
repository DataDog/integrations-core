...
def local_repo(app: "Application"):
    ...
    """
    local_repo_config = {
        'repos': {'local': str(app.config_file.local_path.resolve().parent)},
        'repo': 'local',
    }

    if app.config_file.local_path.exists():
        app.display_info('Local config file already exists. Updating...')
        local_config = app.config_file.local_model.raw_data
        config = deep_merge_with_list_handling(local_config, local_repo_config)
    else:
        config = local_repo_config

    app.config_file.local_model = RootConfig(config)
    app.config_file.update(with_local=True)

    app.display_success(f'Local repo configuration added in {app.config_file.local_path}')
    app.display('Local config content:')
    scrub_config(app.config_file.local_model.raw_data)
    app.output(
        Syntax(dumps_toml_data(app.config_file.local_model.raw_data).rstrip(), 'toml', background_color='default')
    )
