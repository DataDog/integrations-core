...
## 1 — Validate and run the end-to-end environment (the decisive check)

Use `ddev_env_show` to resolve every concrete environment. Start each one with `ddev_env_start`
and `dev=true`, then stop it with `ddev_env_stop` before running tests. Stop any environment you
started even when startup fails. A startup failure means the goal is not met; report the concrete
environment error rather than treating it as a test failure.
...
- `test_unit.py` mocks every inspected endpoint from its fixture (`metrics.txt` for one endpoint,
  or every `<endpoint_name>_metrics.txt` for multiple endpoints), runs each endpoint instance at
  least once and repeats it only when custom check behavior or a targeted cross-scrape assertion
  requires prior state, then makes one union metadata cross-check with `assert_metrics_using_metadata`
...
- `test_integration.py` is marked `@pytest.mark.integration`, uses the real environment,
  runs the check once unless prior-scrape state requires a second call, and makes the same metadata
  cross-check. Its live `exclude=[...]` contains the handoff's
...
- `test_e2e.py` is marked `@pytest.mark.e2e`, asserts metrics via
  `dd_agent_check(check_rate=True)` so the Agent performs the two runs needed for rate coverage,
  and uses the same justified live exclusion list as the integration test.
...
- There are no pointless or redundant tests, **and nothing extraneous is left in `tests/`**. List
  the directory: it should hold the four suite files, shared helpers that the suite uses, the
  fixtures, and the environment. The fixtures and environment are required materialized inputs
  and must be preserved. A stray test module is a fail only when it is irrelevant, incorrect, or
  duplicates the final suite; say which file and why it does not belong.
