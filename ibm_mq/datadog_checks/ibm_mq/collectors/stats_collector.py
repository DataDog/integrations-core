...
class StatsCollector(object):
    ...
    def collect(self, queue_manager, filtered_queue_names):
        ...
        try:
            ...
            while True:
                ...
                stats = self._get_stats(message, header, filtered_queue_names, self.config.qm_stats_tz)
    ...
    @staticmethod
    def _get_stats(message, header, filtered_queue_names, timezone=None):
        if header.Command == MQCMD_STATISTICS_CHANNEL:
            stats = ChannelStats(message, timezone)
        elif header.Command == MQCMD_STATISTICS_Q:
            stats = QueueStats(message, filtered_queue_names, timezone)
        else:
