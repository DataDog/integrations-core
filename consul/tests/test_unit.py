...
def test_consul_request(aggregator, instance, mocker, mock_http, mock_http_response_cls):
    consul_check = ConsulCheck(common.CHECK_NAME, {}, [consul_mocks.MOCK_CONFIG])
    mocker.patch("datadog_checks.base.utils.serialization.json.loads")
    mock_http.get.return_value = mock_http_response_cls()
...
def test_prometheus_endpoint_invalid_url_does_not_abort_check(aggregator, caplog):
    ...
    consul_check = ConsulCheck(common.CHECK_NAME, {}, [config])
    consul_mocks.mock_check(consul_check, consul_mocks._get_consul_mocks())
    consul_check.process = mock.Mock(side_effect=HTTPClientInvalidURLError('No scheme supplied'))
...
def test_prometheus_endpoint_malformed_header_does_not_abort_check(aggregator, caplog):
    ...
    subtype and collapses it into a bare HTTPClientRequestError.
    """
    config = dict(consul_mocks.MOCK_CONFIG, use_prometheus_endpoint=True)
    consul_check = ConsulCheck(common.CHECK_NAME, {}, [config])
    consul_mocks.mock_check(consul_check, consul_mocks._get_consul_mocks())
    consul_check.process = mock.Mock(
        side_effect=HTTPClientRequestError('Content-Length contained multiple unmatching values')
    )
    caplog.set_level(logging.WARNING)
...
@pytest.mark.parametrize(
    'exception',
    [
        pytest.param(HTTPClientReadTimeoutError('read timed out'), id='read_timeout'),
        pytest.param(HTTPClientSSLError('certificate verify failed'), id='ssl'),
    ],
)
def test_prometheus_endpoint_transport_failures_still_abort_check(exception):
    ...
    They are HTTPClientRequestError subclasses, so the arm that catches malformed responses has to let
...
def test_prometheus_endpoint_status_error_without_response_is_surfaced(aggregator):
    ...
    consul_check = ConsulCheck(common.CHECK_NAME, {}, [config])
    consul_mocks.mock_check(consul_check, consul_mocks._get_consul_mocks())
    consul_check.process = mock.Mock(side_effect=HTTPClientStatusError('403 Client Error'))
