...
from .common import HOST, MASTER_PORT, REPLICA_PORT, UNHEALTHY_REPLICA_PORT, redis_client
...
def test_redis_repl(aggregator, dd_run_check, check, master_instance):
    master_db = redis_client(port=MASTER_PORT, db=14, host=HOST)
    replica_db = redis_client(port=REPLICA_PORT, db=14, host=HOST)
    master_db.flushdb()
