...
from datadog_checks.postgres.explain_parameterized_queries import (
    ExplainParameterizedQueries,
)
...
class PostgresStatementSamples(DBMAsyncJob):
    ...
    def __init__(self, check, config, shutdown_callback):
        ...
        self._explain_function = config.explain_plan_config.get(
        ...
        self._explained_statements_ratelimiter = RateLimitingTTLCache(
            maxsize=int(
                config.explain_plan_config.get("explained_queries_cache_maxsize", 5000)
            ),
            ttl=60
            * 60
            / int(
                config.explain_plan_config.get(
        ...
        self._seen_samples_ratelimiter = RateLimitingTTLCache(
            ...
            maxsize=int(
                config.statement_samples_config.get("seen_samples_cache_maxsize", 10000)
            ),
            ttl=60
            * 60
            / int(
                config.statement_samples_config.get("samples_per_hour_per_query", 15)
            ),
        )
    ...
    def _get_active_connections(self):
        ...
        with self._conn_pool.get_connection(
            self._config.dbname, ttl_ms=self._conn_ttl_ms
        ...
        self._log.debug(
            "Loaded %s rows from %s", len(rows), self._config.pg_stat_activity_view
        )
    ...
    def _get_available_activity_columns(self, all_expected_columns):
        with self._conn_pool.get_connection(
            ...
            if missing_columns:
                self._log.debug(
                    "missing the following expected columns from pg_stat_activity: %s",
                    missing_columns,
                )
            self._log.debug(
    ...
    def _filter_and_normalize_statement_rows(self, rows):
        ...
        for row in rows:
            ...
            if self._activity_last_query_start is None or (
                row["query_start"]
                and row["query_start"] > self._activity_last_query_start
            ):
    ...
    def _get_extra_filters_and_params(self, filter_stale_idle_conn=False):
        ...
        if filter_stale_idle_conn and self._activity_last_query_start:
            ...
            extra_filters = (
                extra_filters + " AND NOT (query_start < %s AND state = 'idle')"
            )
    ...
    def _to_active_session(row, track_activity_query_size):
        if (row.get("backend_type", "client backend") != "client backend") or (
            ...
            active_row[
                "query_truncated"
            ] = PostgresStatementSamples._get_truncation_state(
    ...
    def _can_explain_statement(self, obfuscated_statement):
        ...
        if obfuscated_statement.startswith("autovacuum:"):
            return False
        if (
            obfuscated_statement.split(" ", 1)[0].lower()
            not in SUPPORTED_EXPLAIN_STATEMENTS
        ):
            return False
        return True
    ...
    def _get_db_explain_setup_state(self, dbname):
        ...
        try:
            ...
            self._log.warning(
                "cannot collect execution plans due to failed DB connection to dbname=%s: %s",
                dbname,
                repr(e),
            )
            ...
            self._log.warning(
                "cannot collect execution plans due to a database error in dbname=%s: %s",
                dbname,
                repr(e),
            )
        ...
        try:
            ...
            self._log.warning(
                "cannot collect execution plans due to invalid schema in dbname=%s: %s",
                dbname,
                repr(e),
            )
    ...
    def _get_db_explain_setup_state_cached(self, dbname):
        ...
        if strategy_cache:
            ...
            self._log.debug(
                "using cached explain_setup_state for DB '%s': %s",
                dbname,
                db_explain_error,
            )
    ...
    def _run_explain(self, dbname, statement, obfuscated_statement):
        start_time = time.time()
        with self._conn_pool.get_connection(
            dbname, ttl_ms=self._conn_ttl_ms
        ).cursor() as cursor:
            self._log.debug(
                "Running query on dbname=%s: %s(%s)",
                dbname,
                self._explain_function,
    ...
    def _run_and_track_explain(
        ...
        if (
            explain_err_code
            and explain_err_code != DBExplainError.explained_with_prepared_statement
        ):
    ...
    def _run_explain_safe(
        ...
        if (
            self._get_truncation_state(track_activity_query_size, statement)
        ...
        try:
            return (
                self._run_explain(dbname, statement, obfuscated_statement),
                None,
                None,
            )
            ...
            if is_affirmative(
                self._config.explain_plan_config.get(
    ...
    def _collect_plan_for_statement(self, row):
        ...
        if is_affirmative(self._config.explain_plan_config.get("enabled", True)):
            ...
            plan, normalized_plan, obfuscated_plan, plan_signature = (
                None,
                None,
                None,
                None,
        ...
        for row in rows:
            try:
                if (
                    ...
                    or row.get("backend_type", "client backend") != "client backend"
                ):
                    continue
                ...
                self._log.exception(
                    "Crashed trying to collect execution plan for statement in dbname=%s",
                    row["datname"],
                )
                self._check.count(
                    "dd.postgres.statement_samples.error",
    ...
    def _get_track_activity_query_size(self):
        return int(
            self._check.pg_settings.get(
                "track_activity_query_size", TRACK_ACTIVITY_QUERY_SIZE_UNKNOWN_VALUE
            )
        )
    ...
    def _get_truncation_state(track_activity_query_size, statement):
        ...
        return (
            StatementTruncationState.truncated
            if truncated
            else StatementTruncationState.not_truncated
        )
