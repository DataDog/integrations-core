...
#
# Test Drive routing is a narrow development knob, not a production request
# field: the Agent config value names a Test Drive, and the uploader emits the
# header ``test-drive-<validated-name>: 1``. The name never travels through
# resultDelivery or AP action input. When the config is absent or invalid, no
# Test Drive header is emitted, so the upload follows the permanent-service path.

REMOTE_QUERY_UPLOAD_TEST_DRIVE_CONFIG_KEY = 'remote_queries.execute.intake_test_drive'
# The header name is built from the validated Test Drive name as ``test-drive-<name>`` with the
# fixed value ``1``. The name is validated against REMOTE_QUERY_UPLOAD_TEST_DRIVE_NAME_PATTERN so
# it cannot inject arbitrary headers (no colons, spaces, CR/LF, or other control characters).
