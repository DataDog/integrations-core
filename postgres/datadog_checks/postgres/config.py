...
class PostgresConfig:
    ...
    def __init__(self, instance):
        ...
        self.disable_generic_tags = (
            is_affirmative(instance.get("disable_generic_tags", False))
            if instance
            else False
        )
        ...
        if not self.isascii(self.application_name):
            raise ConfigurationError(
                "Application name can include only ASCII characters: %s",
        ...
        self.collect_activity_metrics = is_affirmative(
            instance.get("collect_activity_metrics", False)
        )
        ...
        self.collect_wal_metrics = is_affirmative(
            instance.get("collect_wal_metrics", False)
        )
        self.collect_bloat_metrics = is_affirmative(
            instance.get("collect_bloat_metrics", False)
        )
        self.data_directory = instance.get("data_directory", None)
        self.ignore_databases = instance.get(
            "ignore_databases", DEFAULT_IGNORE_DATABASES
        )
        if is_affirmative(instance.get("collect_default_database", True)):
            self.ignore_databases = [
                d for d in self.ignore_databases if d != "postgres"
            ]
        self.custom_queries = instance.get("custom_queries", [])
        self.tag_replication_role = is_affirmative(
            instance.get("tag_replication_role", False)
        )
        self.custom_metrics = self._get_custom_metrics(
        ...
        self.dbm_enabled = is_affirmative(
            instance.get("dbm", instance.get("deep_database_monitoring", False))
        )
        ...
        self.pg_stat_statements_view = instance.get(
            "pg_stat_statements_view", "pg_stat_statements"
        )
        # statement samples & execution plans
        self.pg_stat_activity_view = instance.get(
            "pg_stat_activity_view", "pg_stat_activity"
        )
        self.statement_samples_config = (
            instance.get("query_samples", instance.get("statement_samples", {})) or {}
        )
        self.explain_plan_config = instance.get("explain_plans", {}) or {}
        ...
        azure = {
            k if k != "fully_qualified_domain_name" else "name": v
        ...
        self.obfuscator_options = {
            ...
            "dollar_quoted_func": is_affirmative(
                obfuscator_options_config.get("keep_dollar_quoted_func", True)
            ),
            "keep_sql_alias": is_affirmative(
                obfuscator_options_config.get("keep_sql_alias", True)
            ),
            "return_json_metadata": is_affirmative(
                obfuscator_options_config.get("collect_metadata", True)
            ),
            "table_names": is_affirmative(
                obfuscator_options_config.get("collect_tables", True)
            ),
            "collect_commands": is_affirmative(
                obfuscator_options_config.get("collect_commands", True)
            ),
            "collect_comments": is_affirmative(
                obfuscator_options_config.get("collect_comments", True)
            ),
        }
        self.log_unobfuscated_queries = is_affirmative(
            instance.get("log_unobfuscated_queries", False)
        )
        self.log_unobfuscated_plans = is_affirmative(
            instance.get("log_unobfuscated_plans", False)
        )
