...
# Current WAL position, used to express each slot LSN as a byte delay behind the server.
REPLICATION_SLOT_CURRENT_LSN = (
    'CASE WHEN pg_is_in_recovery() THEN pg_last_wal_receive_lsn() ELSE pg_current_wal_lsn() END'
)

# pg_replication_slots columns. two_phase_at is the LSN from which a two-phase slot must retain WAL;
# it was added in PG 18, so it is gated to those servers with a VersionRange.
REPLICATION_SLOTS_COLUMNS = [
    ...
    (
        'pg_wal_lsn_diff({}, restart_lsn)'.format(REPLICATION_SLOT_CURRENT_LSN),
        {'name': 'replication_slot.restart_delay_bytes', 'type': 'gauge'},
    ),
    (
        'pg_wal_lsn_diff({}, confirmed_flush_lsn)'.format(REPLICATION_SLOT_CURRENT_LSN),
        {'name': 'replication_slot.confirmed_flush_delay_bytes', 'type': 'gauge'},
    ),
    (
        'pg_wal_lsn_diff({}, two_phase_at)'.format(REPLICATION_SLOT_CURRENT_LSN),
...
def get_replication_slots_query(version: VersionInfo) -> dict:
    """Build the pg_replication_slots query for `version`; two_phase_at was added in PG 18."""
    return build_versioned_query(
        'pg_replication_slots', REPLICATION_SLOTS_COLUMNS, '\n  FROM pg_replication_slots\n', version
    )
