...
# pg_replication_slots columns. two_phase_at, the LSN from which a two-phase slot must retain WAL,
# was added in PG 18, so it is gated to that major with a VersionRange. The LSN columns are reported
# as the byte delay between the server's current WAL position and the slot's stored LSN, mirroring
# restart_delay_bytes / confirmed_flush_delay_bytes.
...
PG_REPLICATION_SLOTS_COLUMNS = [
    ...
    (
        ...
        {'name': 'replication_slot.two_phase_delay_bytes', 'type': 'gauge'},
        VersionRange(min_version=V18),
    ),
]


def get_pg_replication_slots_query(version: VersionInfo) -> dict:
