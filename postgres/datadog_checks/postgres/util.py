...
class DBExplainError(Enum):
    ...
    # the sampled statement holds more than one statement, or text we can't tokenize with certainty.
    # explaining it would mean sending attacker-controllable statement separators to the server.
...
# Matches the opening delimiter of a dollar-quoted string, e.g. `$$` or `$body$`. A `$` followed by
# digits (`$1`) is a parameter marker, not a dollar quote, so the tag must not start with a digit.
DOLLAR_QUOTE_TAG_PATTERN = re.compile(r'\$([A-Za-z_][A-Za-z0-9_]*)?\$')
...
def is_single_statement(sql: str) -> bool:
    ...
    i = 0
    n = len(sql)
    # once the top-level statement has been terminated by a `;`, only whitespace and comments may follow
    terminated = False
    ...
    while i < n:
        ...
        if c.isspace():
            ...
            if terminated:
                # an empty statement between two `;` still means more than one statement was sent
                return False
            terminated = True
            i += 1
        elif terminated:
            ...
            i = skip_single_quoted(sql, i, is_escape_string(sql, i))
            if i == -1:
                return False
        elif c == '"':
            ...
            match = DOLLAR_QUOTE_TAG_PATTERN.match(sql, i)
            if match:
                closing = sql.find(match.group(0), match.end())
                if closing == -1:
                    return False
...
def skip_block_comment(sql: str, start: int) -> int:
    """Return the index just past the block comment starting at start, or -1 if it is unterminated.
...
def is_escape_string(sql: str, quote_index: int) -> bool:
    """Return True when the quote at quote_index opens an escape string constant (`E'...'`).

    The `E` must be a standalone prefix, not the last character of an identifier or keyword:
    in `WHERE x LIKE'a\\'` the `E` belongs to `LIKE` and the string is a plain one.
    """
    if quote_index == 0 or sql[quote_index - 1] not in 'Ee':
        return False
    preceding = quote_index - 2
    return preceding < 0 or not (sql[preceding].isalnum() or sql[preceding] in '_$')


def skip_single_quoted(sql: str, start: int, escapes_with_backslash: bool) -> int:
    """Return the index just past the single-quoted string starting at start, or -1 if it can't be
    tokenized unambiguously.

    A doubled quote (`''`) is always an escaped quote. A backslash escapes the next character only
    in an escape string constant (`E'...'`); in a plain string it depends on the server's
    ``standard_conforming_strings``, so a plain string containing a backslash is rejected rather
    than guessed at.
    """
    i = start + 1
    n = len(sql)
...
def skip_double_quoted(sql: str, start: int) -> int:
    """Return the index just past the double-quoted identifier starting at start, or -1 if it is
    unterminated. A doubled quote (`""`) is an escaped quote; backslashes are not special.
    """
    i = start + 1
    n = len(sql)
    while i < n:
