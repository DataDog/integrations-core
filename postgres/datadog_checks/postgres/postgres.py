...
class PostgreSql(DatabaseCheck):
    ...
    def _get_local_wal_file_age(self):
        ...
        if not self.safe_os.isdir(wal_log_dir):
            self.log.warning(
        ...
        all_dir_contents = self.safe_os.listdir(wal_log_dir)
        all_files = [f for f in all_dir_contents if self.safe_os.isfile(os.path.join(wal_log_dir, f))]
