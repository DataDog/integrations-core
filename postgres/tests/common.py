...
requires_static_version = pytest.mark.skipif(
    USING_LATEST, reason="Version `latest` is ever-changing, skipping"
)


def assert_metric_at_least(
    aggregator, metric_name, lower_bound=None, higher_bound=None, count=None, tags=None
):
    found_values = 0
    ...
    for metric in aggregator.metrics(metric_name):
        if expected_tags and expected_tags == sorted(metric.tags):
            if lower_bound is not None:
                assert (
                    metric.value >= lower_bound
                ), "Expected {} with tags {} to have a value >= {}, got {}".format(
                    metric_name, expected_tags, lower_bound, metric.value
                )
            if higher_bound is not None:
                assert (
                    metric.value <= higher_bound
                ), "Expected {} with tags {} to have a value <= {}, got {}".format(
    ...
    if count:
        assert (
            found_values == count
        ), "Expected to have {} with tags {} values for metric {}, got {}".format(
...
def check_replication_delay(aggregator, metrics_cache, expected_tags, count=1):
    replication_metrics = metrics_cache.get_replication_metrics(
        VersionUtils.parse_version(POSTGRES_VERSION), False
    )
