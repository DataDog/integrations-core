...
def check_replication_slots(aggregator, expected_tags, count=1):
    if float(POSTGRES_VERSION) < 10.0:
        return
    # The fixtures do not create two-phase slots, so two_phase_at is NULL and its metric is never
    # submitted; iterate over the base (pre-18) column set to match what the slots emit.
    for metric_name in _iterate_metric_name(get_replication_slots_query(V10)):
