...
def test_custom_metrics(aggregator, pg_instance):
    ...
    tags = [
        ...
        "dd.internal.resource:database_instance:{}".format(
            postgres_check.resolved_hostname
        ),
    ]
...
def test_custom_queries(aggregator, pg_instance):
    ...
    for tag in ("a", "b", "c"):
        ...
        aggregator.assert_metric(
            "custom.num", value=value, tags=custom_tags + ["query:custom"]
        )
        aggregator.assert_metric(
            "another_custom_one.num",
            value=value,
            tags=custom_tags + ["query:another_custom_one"],
        )
