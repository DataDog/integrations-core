...
@requires_over_10
def test_common_replica_metrics(
    aggregator, integration_check, metrics_cache_replica, pg_replica_instance
):
    ...
    check_replication_delay(
        aggregator, metrics_cache_replica, expected_tags=expected_tags
    )
    check_wal_receiver_metrics(
        aggregator, expected_tags=expected_tags + ["status:streaming"]
    )
...
@requires_over_10
def test_wal_receiver_metrics(
    aggregator, integration_check, pg_instance, pg_replica_instance
):
    check = integration_check(pg_replica_instance)
    expected_tags = pg_replica_instance["tags"] + [
        "port:{}".format(pg_replica_instance["port"]),
    ...
    aggregator.assert_metric(
        "postgresql.wal_receiver.last_msg_send_age", count=1, tags=expected_tags
    )
    aggregator.assert_metric(
        "postgresql.wal_receiver.last_msg_receipt_age", count=1, tags=expected_tags
    )
    aggregator.assert_metric(
        "postgresql.wal_receiver.latest_end_age", count=1, tags=expected_tags
    )
...
@requires_over_10
def test_conflicts_lock(
    aggregator, integration_check, pg_instance, pg_replica_instance2
):
...
def test_conflicts_snapshot(
    ...
    check.check(pg_replica_instance2)
    aggregator.assert_metric(
        "postgresql.conflicts.snapshot", value=1, tags=expected_tags
    )


@pytest.mark.skip(reason="Failing on master")
@requires_over_10
def test_conflicts_bufferpin(
    aggregator, integration_check, pg_instance, pg_replica_instance2
):
    check = integration_check(pg_replica_instance2)
    expected_tags = pg_replica_instance2["tags"] + [
    ...
    check.check(pg_replica_instance2)
    aggregator.assert_metric(
        "postgresql.conflicts.bufferpin", value=1, tags=expected_tags
    )
