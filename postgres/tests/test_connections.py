...
def test_conn_pool_no_leaks_on_close(pg_instance):
    ...
    pool2 = MultiDatabaseConnectionPool(
        lambda dbname: psycopg2.connect(
            host=HOST, dbname=dbname, user=USER_ADMIN, password=PASSWORD_ADMIN
        )
    )
    ...
    for _ in range(20):
        ...
        def get_activity():
            ...
            """
            with pool2.get_connection("postgres", 1).cursor(
...
def test_conn_pool_no_leaks_on_prune(pg_instance):
    ...
    for i in range(0, 50):
        ...
        approximate_deadline = datetime.datetime.now() + datetime.timedelta(
            milliseconds=ttl_long
        )
        assert (
    ...
    for _ in range(100):
        ...
        for attempt in range(attempts_to_verify):
            ...
            assert (
                len(leaked_rows) == 0
            ), "Found leaked rows on the server not in the connection pool"
    ...
    for attempt in range(attempts_to_verify):
        ...
        assert (
            len(leaked_rows) == 0
        ), "Found leaked rows remaining after TTL was updated to short TTL"
...
@pytest.mark.integration
@pytest.mark.usefixtures("dd_environment")
def test_conn_pool_hit_pool_limit_and_evict(pg_instance):
    """
    Test creating a limited connection pool and adding connections
    until the pool is filled. Then, set one to Done and try to evict it.
    """
    limit = 5
    check = PostgreSql("postgres", {}, [pg_instance])
    ...
    for i in range(limit):
        pool.get_connection("dogs_{}".format(i), 60000)
    ...
    expected_evicted = "dogs_3"
    pool.done(expected_evicted)
    evicted = pool.evict_lru()
...
@pytest.mark.integration
@pytest.mark.usefixtures("dd_environment")
def test_conn_pool_multithreaded(pg_instance):
    """
    Test creating a limited connection pool that is shared among several threads.
    """
    ...
    for i in range(limit):
        thread = threading.Thread(
            target=pretend_to_run_query, args=(pool, "dogs_{}".format(i))
        )
