...
def dbm_instance(pg_instance):
    ...
    pg_instance['collect_column_statistics'] = {
        'enabled': True,
...
def nofunc_instance(pg_instance):
    """Instance pointing at dogs_nofunc, which has the datadog schema but no column_statistics() function."""
    pg_instance['dbm'] = True
    pg_instance['min_collection_interval'] = 0.1
...
def test_collect_column_statistics_happy_path(integration_check, dbm_instance, pg_instance, aggregator):
    ...
    run_one_check(check)
    events = aggregator.get_event_platform_events("dbm-column-statistics")
    assert len(events) > 0, "Expected at least one column stats event"
    event = events[0]
    assert event['dbm_type'] == 'column_statistics'
    assert event['dbms'] == 'postgres'
    assert event['host'] == 'stubbed.hostname'
    assert 'timestamp' in event
    assert 'collection_interval' in event
    assert isinstance(event['column_statistics'], list)
    assert len(event['column_statistics']) > 0

    # Verify table structure including metadata fields
    table_entry = event['column_statistics'][0]
    ...
    all_tables = set()
    for evt in events:
        for entry in evt['column_statistics']:
...
def test_collect_column_statistics_include_filter(integration_check, dbm_instance, pg_instance, aggregator):
    """include_tables restricts the result set to tables whose name matches the regex."""
    _analyze_tables(pg_instance)
    dbm_instance['collect_column_statistics']['include_tables'] = ['persons']
    check = integration_check(dbm_instance)
    run_one_check(check)
    events = aggregator.get_event_platform_events("dbm-column-statistics")
    assert len(events) > 0
    for event in events:
        for table_entry in event['column_statistics']:
            assert 'persons' in table_entry['table'], f"Expected only 'persons' tables but got '{table_entry['table']}'"


def test_collect_column_statistics_exclude_filter(integration_check, dbm_instance, pg_instance, aggregator):
    """exclude_tables removes matching tables from the result set while allowing others through."""
    _analyze_tables(pg_instance)
    dbm_instance['collect_column_statistics']['exclude_tables'] = ['persons', 'pgtable', 'cities']
    check = integration_check(dbm_instance)
    run_one_check(check)
    events = aggregator.get_event_platform_events("dbm-column-statistics")
...
def test_collect_column_statistics_max_tables(integration_check, dbm_instance, pg_instance, aggregator):
    """max_tables caps the number of collected tables at the configured limit."""
    _analyze_tables(pg_instance)
    dbm_instance['collect_column_statistics']['max_tables'] = 1
    check = integration_check(dbm_instance)
    run_one_check(check)
    events = aggregator.get_event_platform_events("dbm-column-statistics")
    total_tables = sum(len(e['column_statistics']) for e in events)
    assert total_tables == 1, f"Expected exactly 1 table with max_tables=1, got {total_tables}"


def test_collect_column_statistics_disabled(integration_check, dbm_instance, aggregator):
    """collect_column_statistics.enabled=False suppresses collection entirely."""
    dbm_instance['collect_column_statistics']['enabled'] = False
    check = integration_check(dbm_instance)
    run_one_check(check)
    events = aggregator.get_event_platform_events("dbm-column-statistics")
...
def test_collect_column_statistics_function_not_found(integration_check, nofunc_instance, aggregator):
    """Test that missing datadog.column_statistics() emits a health WARNING and no column stats."""
    ...
    function_not_found_events = [e for e in health_events if e['name'] == 'column_statistics_function_not_found']
    assert len(function_not_found_events) == 1
    assert function_not_found_events[0]['status'] == 'warning'


def test_collect_column_statistics_function_not_found_logs_once(integration_check, nofunc_instance, aggregator):
    """Test that repeated failures only emit a single health WARNING."""
    nofunc_instance['collect_column_statistics']['collection_interval'] = 0.1
    check = integration_check(nofunc_instance)
    run_one_check(check)
    run_one_check(check)
    run_one_check(check)
...
def test_collect_column_statistics_recovery(check_runner, dbm_instance, pg_instance, aggregator):
    ...
    collector.collect_column_statistics([])

    health_events = aggregator.get_event_platform_events("dbm-health")
    ok_events = [
        e for e in health_events if e['name'] == 'column_statistics_function_not_found' and e['status'] == 'ok'
    ]
...
def test_collect_column_statistics_collection_interval_in_event(
    ...
    _analyze_tables(pg_instance)
    dbm_instance['collect_column_statistics']['collection_interval'] = 7200
    check = integration_check(dbm_instance)
    run_one_check(check)
    events = aggregator.get_event_platform_events("dbm-column-statistics")
    assert len(events) > 0, "Expected at least one column stats event"
...
def test_collect_column_statistics_include_and_exclude(integration_check, dbm_instance, pg_instance, aggregator):
    """When both include and exclude match, exclude takes precedence."""
    _analyze_tables(pg_instance)
    # Include all tables starting with 'p', but exclude 'pgtable'
    dbm_instance['collect_column_statistics']['include_tables'] = ['^p.*']
    dbm_instance['collect_column_statistics']['exclude_tables'] = ['^pgtable$']
    check = integration_check(dbm_instance)
    run_one_check(check)
    events = aggregator.get_event_platform_events("dbm-column-statistics")
...
def test_collect_column_statistics_default_config(integration_check, pg_instance, aggregator):
...
def test_collect_column_statistics_no_health_event_on_success(integration_check, dbm_instance, pg_instance, aggregator):
    ...
    column_stats_health = [
        ...
        if e['name'] in ('column_statistics_function_not_found', 'column_statistics_insufficient_privilege')
    ]
    assert len(column_stats_health) == 0, f"Expected no column stats health events, got {column_stats_health}"


def test_collect_column_statistics_respects_interval(integration_check, dbm_instance, pg_instance, aggregator):
    """Test that the second run within the collection interval does not emit a new event."""
    _analyze_tables(pg_instance)
    dbm_instance['collect_column_statistics']['collection_interval'] = 36000
    check = integration_check(dbm_instance)

    run_one_check(check)
    first_count = len(aggregator.get_event_platform_events("dbm-column-statistics"))
    assert first_count > 0, "Expected at least one event on first run"

    run_one_check(check)
    second_count = len(aggregator.get_event_platform_events("dbm-column-statistics"))
    assert second_count == first_count, (
        f"Expected no new events on second run (interval not elapsed), got {second_count - first_count} new"
    )


@patch('datadog_checks.postgres.column_statistics.PAYLOAD_MAX_COLUMNS', 5)
def test_collect_column_statistics_payload_chunking(integration_check, dbm_instance, pg_instance, aggregator):
    ...
    all_tables = set()
    for event in events:
        for entry in event['column_statistics']:
            all_tables.add(entry['table'])
...
def test_collect_column_statistics_multi_database(check_runner, dbm_instance, pg_instance, aggregator):
...
def test_collect_column_statistics_multi_database_error_isolation(check_runner, dbm_instance, pg_instance, aggregator):
...
def test_collect_column_statistics_exclude_all(integration_check, dbm_instance, pg_instance, aggregator):
    """Test that excluding all tables produces no events."""
    _analyze_tables(pg_instance)
    dbm_instance['collect_column_statistics']['exclude_tables'] = ['.*']
    check = integration_check(dbm_instance)
    run_one_check(check)
    events = aggregator.get_event_platform_events("dbm-column-statistics")
...
def test_collect_column_statistics_include_nonexistent(integration_check, dbm_instance, pg_instance, aggregator):
    """Test that including only nonexistent tables produces no events."""
    _analyze_tables(pg_instance)
    dbm_instance['collect_column_statistics']['include_tables'] = ['this_table_does_not_exist']
    check = integration_check(dbm_instance)
    run_one_check(check)
    events = aggregator.get_event_platform_events("dbm-column-statistics")
...
def test_collect_column_statistics_max_tables_zero(integration_check, dbm_instance, pg_instance, aggregator):
    """Test that max_tables=0 produces no events and no error."""
    _analyze_tables(pg_instance)
    dbm_instance['collect_column_statistics']['max_tables'] = 0
    check = integration_check(dbm_instance)
    run_one_check(check)
    events = aggregator.get_event_platform_events("dbm-column-statistics")
...
def test_collect_column_statistics_insufficient_privilege(check_runner, dbm_instance, aggregator):
    """InsufficientPrivilege on datadog.column_statistics() emits a dedicated health WARNING."""
    check = check_runner(dbm_instance)
    collector = check.metadata_samples._column_statistics_collector
    exc = psycopg.errors.InsufficientPrivilege("permission denied for function datadog.column_statistics")
    ...
    health_events = aggregator.get_event_platform_events("dbm-health")
    priv_events = [e for e in health_events if e['name'] == 'column_statistics_insufficient_privilege']
...
def test_collect_column_statistics_insufficient_privilege_logs_once(check_runner, dbm_instance, aggregator):
...
def test_collect_column_statistics_insufficient_privilege_recovery(check_runner, dbm_instance, pg_instance, aggregator):
    ...
    health_events = aggregator.get_event_platform_events("dbm-health")
    ok_events = [
        e for e in health_events if e['name'] == 'column_statistics_insufficient_privilege' and e['status'] == 'ok'
    ]
...
def test_collect_column_statistics_disabled_when_dbm_false(integration_check, pg_instance, aggregator):
    """Column stats should not be collected when dbm=False, even if collect_column_statistics.enabled=True."""
    _analyze_tables(pg_instance)
    pg_instance['dbm'] = False
    pg_instance['collect_settings'] = {'enabled': False, 'run_sync': True}
    pg_instance['collect_schemas'] = {'enabled': False}
    pg_instance['collect_column_statistics'] = {'enabled': True, 'collection_interval': 36000, 'max_tables': 100}
    check = integration_check(pg_instance)
    run_one_check(check)
...
def test_collect_column_statistics_statement_timeout_handled(check_runner, dbm_instance, aggregator):
...
def test_collect_column_statistics_cancel_stops_multi_db_iteration(check_runner, dbm_instance, pg_instance):
...
def test_collect_column_statistics_event_payload_fields(integration_check, dbm_instance, pg_instance, aggregator):
    ...
    # tags_no_db must not include a `db:` tag (column_statistics aggregates across databases)
...
def test_collect_column_statistics_metrics_emitted_on_success(integration_check, dbm_instance, pg_instance, aggregator):
...
def test_collect_column_statistics_metrics_emitted_on_error(check_runner, dbm_instance, aggregator):
...
def test_collect_column_statistics_special_chars_in_pattern_handled_safely(
    integration_check, dbm_instance, pg_instance, aggregator
):
    ...
    dbm_instance['collect_column_statistics']['exclude_tables'] = ["bad'pattern"]
    check = integration_check(dbm_instance)
    run_one_check(check)
    events = aggregator.get_event_platform_events("dbm-column-statistics")
    tables_seen = {entry['table'] for evt in events for entry in evt['column_statistics']}
    assert {'persons', 'cities', 'pgtable'}.issubset(tables_seen)


def test_collect_column_statistics_invalid_regex_does_not_crash(check_runner, dbm_instance, pg_instance, aggregator):
    """Invalid regex is caught at the per-database boundary; no events emitted, no exception escapes."""
    _analyze_tables(pg_instance)
    dbm_instance['collect_column_statistics']['include_tables'] = ["[invalid"]
    check = check_runner(dbm_instance)
    collector = check.metadata_samples._column_statistics_collector
...
def test_collect_column_statistics_handles_missing_version(check_runner, dbm_instance, pg_instance, aggregator):
...
def test_collect_column_statistics_uses_autodiscovered_databases(check_runner, dbm_instance):
...
def test_collect_column_statistics_pre_cancelled_skips_collection(check_runner, dbm_instance, aggregator):
...
def test_collect_column_statistics_partial_recovery_waits_for_all_dbs(
...
def test_collect_column_statistics_non_database_error_is_absorbed(check_runner, dbm_instance, aggregator):
    ...
    with patch.object(check.db_pool, 'get_connection', _fake_pool_raising(exc)):
        collector.collect_column_statistics([])  # must not raise
...
def test_collect_column_statistics_include_schemas(integration_check, dbm_instance, pg_instance, aggregator):
    """include_schemas restricts the result set to tables in matching schemas."""
    _analyze_tables(pg_instance)
    _analyze_public2_cities(pg_instance)
    dbm_instance['collect_column_statistics']['include_schemas'] = ['^public2$']
    check = integration_check(dbm_instance)
    run_one_check(check)
    events = aggregator.get_event_platform_events("dbm-column-statistics")
    assert len(events) > 0
    schemas_seen = {entry['schema'] for evt in events for entry in evt['column_statistics']}
    assert schemas_seen == {'public2'}


def test_collect_column_statistics_exclude_schemas(integration_check, dbm_instance, pg_instance, aggregator):
    """exclude_schemas removes matching schemas from the result set."""
    _analyze_tables(pg_instance)
    _analyze_public2_cities(pg_instance)
    dbm_instance['collect_column_statistics']['exclude_schemas'] = ['^public$']
    check = integration_check(dbm_instance)
    run_one_check(check)
    events = aggregator.get_event_platform_events("dbm-column-statistics")
    assert len(events) > 0
...
def test_collect_column_statistics_include_and_exclude_schemas(
    integration_check, dbm_instance, pg_instance, aggregator
):
    """When both include_schemas and exclude_schemas match a schema, exclude wins."""
    _analyze_tables(pg_instance)
    _analyze_public2_cities(pg_instance)
    dbm_instance['collect_column_statistics']['include_schemas'] = ['^public']
    dbm_instance['collect_column_statistics']['exclude_schemas'] = ['^public2$']
    check = integration_check(dbm_instance)
    run_one_check(check)
    events = aggregator.get_event_platform_events("dbm-column-statistics")
    assert len(events) > 0
...
def test_collect_column_statistics_include_databases(check_runner, dbm_instance, aggregator):
    """include_databases restricts the database list to names matching the regex."""
    dbm_instance['collect_column_statistics']['include_databases'] = ['^dogs$']
    check = check_runner(dbm_instance)
    collector = check.metadata_samples._column_statistics_collector
    autodiscovery = MagicMock()
...
def test_collect_column_statistics_exclude_databases(check_runner, dbm_instance, aggregator):
    """exclude_databases removes matching databases from the list."""
    dbm_instance['collect_column_statistics']['exclude_databases'] = ['^dogs']
    check = check_runner(dbm_instance)
    collector = check.metadata_samples._column_statistics_collector
    autodiscovery = MagicMock()
    autodiscovery.get_items.return_value = ['datadog_test', 'dogs', 'dogs_3']
...
def test_collect_column_statistics_include_and_exclude_databases(check_runner, dbm_instance, aggregator):
    """When both filters apply, exclude takes precedence."""
    dbm_instance['collect_column_statistics']['include_databases'] = ['^dogs']
    dbm_instance['collect_column_statistics']['exclude_databases'] = ['^dogs_[345]$']
    check = check_runner(dbm_instance)
    collector = check.metadata_samples._column_statistics_collector
    autodiscovery = MagicMock()
