...
def test_data_observability_cron_startup_lookback_default(mock_check, minimal_instance):
    """cron_startup_lookback_seconds defaults to 300 when unset."""
    minimal_instance['data_observability'] = {'enabled': True, 'queries': []}
    mock_check.instance = minimal_instance
    mock_check.init_config = {}
    config, result = build_config(check=mock_check)
    assert result.valid
    assert config.data_observability.cron_startup_lookback_seconds == 300


def test_data_observability_cron_startup_lookback_explicit(mock_check, minimal_instance):
    """A user-supplied cron_startup_lookback_seconds is preserved through build_config."""
    minimal_instance['data_observability'] = {
        'enabled': True,
        'cron_startup_lookback_seconds': 42,
        'queries': [],
    }
    mock_check.instance = minimal_instance
    mock_check.init_config = {}
    config, result = build_config(check=mock_check)
    assert result.valid
    assert config.data_observability.cron_startup_lookback_seconds == 42


def test_data_observability_cron_startup_lookback_zero(mock_check, minimal_instance):
    """Zero is a valid value (disables startup recovery) and survives through build_config."""
    minimal_instance['data_observability'] = {
        'enabled': True,
        'cron_startup_lookback_seconds': 0,
        'queries': [],
    }
    mock_check.instance = minimal_instance
    mock_check.init_config = {}
    config, result = build_config(check=mock_check)
    assert result.valid
    assert config.data_observability.cron_startup_lookback_seconds == 0
