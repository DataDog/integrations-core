...
def test_relations_metrics(aggregator, integration_check, pg_instance):
    ...
    expected_tags = pg_instance["tags"] + [
        ...
        "dd.internal.resource:database_instance:{}".format(
            posgres_check.resolved_hostname
        ),
    ]
...
def test_bloat_metrics(
    aggregator, collect_bloat_metrics, expected_count, integration_check, pg_instance
):
    ...
    aggregator.assert_metric(
        "postgresql.table_bloat", count=expected_count, tags=base_tags
    )
    ...
    for index in indices:
        ...
        aggregator.assert_metric(
            "postgresql.index_bloat", count=expected_count, tags=expected_tags
        )
...
def test_locks_metrics(
    ...
    pg_instance[
        "query_timeout"
    ] = 1000  # One of the relation queries waits for the table to not be locked
    ...
    if tags is not None:
        ...
        aggregator.assert_metric(
            "postgresql.locks", count=lock_count, tags=expected_tags
        )
    else:
...
def test_relations_validation_fails_if_relkind_is_wrong_type():
    with pytest.raises(ConfigurationError):
        RelationsManager.validate_relations_config(
            [{"relation_name": "person", "relkind": "foo"}]
        )
