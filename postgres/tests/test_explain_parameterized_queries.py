...
def test_explain_parameterized_queries(
    integration_check, dbm_instance, query, expected_explain_err_code
):
...
def test_explain_parameterized_queries_generic_params(
    ...
    explain_param_queries = check.statement_samples._explain_parameterized_queries
    assert (
        explain_param_queries._create_prepared_statement(
            DB_NAME, query, query, query_signature
        )
        is True
    )
    assert (
        expected_generic_values
        == explain_param_queries._get_number_of_parameters_for_prepared_statement(
