...
def test_malformed_get_custom_queries(check):
    ...
    db.cursor().__enter__().fetchall.return_value = iter([query_return])
    check._collect_custom_queries([])
    check.log.error.assert_called_once_with(
    ...
    db.cursor().__enter__().fetchall.return_value = iter([[]])
    check._collect_custom_queries([])
    ...
    db.cursor().__enter__().fetchall.return_value = iter([[1337]])
    check._collect_custom_queries([])
    check.log.error.assert_called_once_with(
