...
def test_get_instance_metrics_state(integration_check, pg_instance):
    ...
    res = check.metrics_cache.get_instance_metrics(
        "foo"
    )  # metrics were cached so this shouldn't be called
    assert res["metrics"] == dict(c_metrics, **util.NEWER_92_METRICS)
...
def test_get_wal_dir(integration_check, pg_instance, pg_version, wal_path):
    ...
    with mock.patch(
        "datadog_checks.postgres.PostgreSql.version", new_callable=PropertyMock
    ) as mock_version:
...
def test_replication_tag(aggregator, integration_check, pg_instance):
    ...
    aggregator.assert_metric(
        REPLICATION_TAG_TEST_METRIC, tags=expected_tags + [ROLE_TAG]
    )
    aggregator.reset()
...
@pytest.mark.parametrize(
    "disable_generic_tags, expected_hostname",
    [(True, "resolved.hostname"), (False, "stubbed.hostname")],
)
def test_resolved_hostname(disable_generic_tags, expected_hostname, pg_instance):
    ...
    with mock.patch(
        ...
        return_value="resolved.hostname",
    ) as resolve_db_host_mock:
        check = PostgreSql("test_instance", {}, [instance])
        assert check.resolved_hostname == expected_hostname
        assert (
            resolve_db_host_mock.called == disable_generic_tags
        ), "Expected resolve_db_host.called to be " + str(disable_generic_tags)
