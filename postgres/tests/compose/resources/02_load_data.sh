...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" datadog_test <<-EOSQL
    ...
    -- The remote query E2E producer runs as the limited ``datadog`` user and reads this
    -- table through the allowlisted proof query.
    GRANT SELECT ON cities TO datadog;
