...
def test_can_connect_service_check(aggregator, check, pg_instance):
    expected_tags = pg_instance['tags'] + ['host:{}'.format(HOST), 'port:{}'.format(PORT), 'db:{}'.format(DB_NAME)]
    check.check(pg_instance)
...
def test_schema_metrics(aggregator, check, pg_instance):
    ...
    aggregator.assert_metric('postgresql.table.count', value=1, count=1, tags=pg_instance['tags'] + ['schema:public'])
