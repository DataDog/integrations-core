...
def test_statement_metrics_optimization(
    aggregator,
    integration_check,
    dbm_instance,
):
    print("[AMW] statement metrics optimization test")
    ...
    # Get the base number of calls generated by the agent
    run_one_check(check, dbm_instance, cancel=False)
    base_value = aggregator.metrics('dd.postgres.queries.query_rows_raw')[0].value
    print("[AMW] base value is " + str(base_value))
    assert base_value > 0
    aggregator.reset()

    # First check run, no rows generated yet
    _run_queries()
    run_one_check(check, dbm_instance, cancel=False)
    aggregator.assert_metric('dd.postgres.queries.query_rows_raw', value=base_value, count=1,
                             tags=expected_dbm_metrics_tags)
    aggregator.reset()

    # Second check run, we should have one row per sample
    _run_queries()
    run_one_check(check, dbm_instance, cancel=False)
    aggregator.assert_metric('dd.postgres.queries.query_rows_raw', value=base_value + len(SAMPLE_QUERIES), count=1,
                             tags=expected_dbm_metrics_tags)
    
    events = aggregator.get_event_platform_events("dbm-metrics")
    print("[AMW] events: " + str(events))
#    assert len(events) == 2
    event = events[0]  # first item is from the initial dummy check to load pg_settings
...
def test_statement_metrics_multiple_pgss_rows_single_query_signature(
    ...
    def _run_query(idx):
        ...
        print("[AMW] Executing query " + str(query) + " with args " + str(args))
...
def test_statement_metrics_unchanged(
    ...
    print("[AMW] base value: " + str(base_value))

    print("[AMW] run queries 1")
    ...
    print("[AMW] run queries 3")


    # Third check run, no metrics generated since no queries were called
    run_one_check(check, dbm_instance, cancel=False)
    aggregator.assert_metric('dd.postgres.queries.query_rows_raw', value=base_value, count=1,
                             tags=expected_dbm_metrics_tags)
    aggregator.reset()

    print("[AMW] run queries 4")

    # Fourth check run, we should have queries being reported
    _run_queries()
#    _run_queries()
    run_one_check(check, dbm_instance, cancel=False)

    # Now, a metric won't be sent because previous_statements is empty
#    aggregator.assert_metric('dd.postgres.queries.query_rows_raw', value=base_value + len(SAMPLE_QUERIES), count=1,
#                             tags=expected_dbm_metrics_tags)
    _run_queries()
    run_one_check(check, dbm_instance, cancel=False)
    
    # This probably results in an inflated metric being sent
