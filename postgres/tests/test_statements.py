...
def test_dbm_enabled_config(
    integration_check, dbm_instance, dbm_enabled_key, dbm_enabled
):
...
def test_statement_metrics_version(
    integration_check, dbm_instance, version, expected_payload_version
):
    if version:
        check = integration_check(dbm_instance)
        ...
        with mock.patch(
            "datadog_checks.postgres.postgres.PostgreSql.version",
            ...
            check._connect()
            assert (
                check.statement_metrics._payload_pg_version()
...
@pytest.mark.parametrize(
    "pg_stat_statements_view", ["pg_stat_statements", "datadog.pg_stat_statements()"]
)
...
def test_statement_metrics(
    ...
    def _run_queries():
        for user, password, dbname, query, arg in SAMPLE_QUERIES:
            if dbname not in connections:
                connections[dbname] = psycopg2.connect(
    ...
    def _should_catch_query(dbname):
        ...
        # agent is connecting
        if (
            POSTGRES_VERSION.split(".")[0] == "9"
            and pg_stat_statements_view == "pg_stat_statements"
        ):
    ...
    for username, _, dbname, query, _ in SAMPLE_QUERIES:
        ...
        matching_rows = [
            r for r in event["postgres_rows"] if r["query_signature"] == query_signature
        ]
        ...
        if track_io_timing_enabled:
            assert (
                available_columns & PG_STAT_STATEMENTS_TIMING_COLUMNS
            ) == PG_STAT_STATEMENTS_TIMING_COLUMNS
        else:
...
def test_statement_metrics_with_duplicates(
    aggregator, integration_check, dbm_instance, datadog_agent
):
    # don't need samples for this test
    dbm_instance["query_samples"] = {"enabled": False}
    # very low collection interval for test purposes
    ...
    normalized_query = (
        "select * from pg_stat_activity where application_name = ANY(array [ ? ])"
    )
    ...
    matching = [
        e for e in event["postgres_rows"] if e["query_signature"] == query_signature
    ]
    assert len(matching) == 1
    row = matching[0]
    assert row["calls"] == 2
...
def test_get_db_explain_setup_state(
    integration_check, dbm_instance, dbname, expected_db_explain_error
):
    check = integration_check(dbm_instance)
    check._connect()
...
def test_failed_explain_handling(
    ...
    if skip_on_versions is not None and float(POSTGRES_VERSION) in skip_on_versions:
        pytest.skip(
            "not relevant for postgres {version}".format(version=POSTGRES_VERSION)
        )
...
@pytest.mark.parametrize(
    "pg_stat_activity_view", ["pg_stat_activity", "datadog.pg_stat_activity()"]
)
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            [
                {
                    "code": "invalid_schema",
                    "message": "<class 'psycopg2.errors.InvalidSchemaName'>",
                }
            ],
            StatementTruncationState.not_truncated.value,
            [],
        ...
        (
            ...
            [
                {
                    "code": "failed_function",
                    "message": "<class 'psycopg2.errors.UndefinedFunction'>",
                }
            ],
            StatementTruncationState.not_truncated.value,
            [
...
def test_statement_samples_collect(
    ...
    try:
        ...
        expected_query = query % (
            "'" + arg + "'" if isinstance(arg, string_types) else arg
        )
        ...
        matching = [
            ...
            if expected_query.encode("utf-8").startswith(
                e["db"]["statement"].encode("utf-8")
            )
        ]

        if (
            POSTGRES_VERSION.split(".")[0] == "9"
            and pg_stat_activity_view == "pg_stat_activity"
        ):
        ...
        if expected_error_tag:
            assert (
                event["db"]["plan"]["definition"] is None
            ), "did not expect to collect an execution plan"
            aggregator.assert_metric(
                "dd.postgres.statement_samples.error",
            ...
            assert (
                event["db"]["plan"]["definition"] is not None
            ), "missing execution plan"
            assert "Plan" in json.loads(
                event["db"]["plan"]["definition"]
            ), "invalid json execution plan"
        ...
        for event in matching:
            ...
            if event["db"]["plan"]["definition"] is None:
                assert (
                    event["db"]["plan"]["collection_errors"]
                    == expected_collection_errors
                )
            else:
...
def test_statement_metadata(
    ...
    conn = psycopg2.connect(
        host=HOST, dbname="datadog_test", user="bob", password="bob"
    )
    cursor = conn.cursor()
    ...
    matching_samples = [
        s for s in samples if s["db"]["query_signature"] == query_signature
    ]
    ...
    fqt_samples = [
        s
        for s in samples
        if s.get("dbm_type") == "fqt"
        and s["db"]["query_signature"] == normalized_query_signature
    ]
    ...
    matching_metrics = [
        m
        for m in metric["postgres_rows"]
        if m["query_signature"] == normalized_query_signature
    ]
...
def test_activity_snapshot_collection(
    ...
    conn = psycopg2.connect(
        host=HOST, dbname=dbname, user=user, password=password, async_=1
    )
    blocking_conn = psycopg2.connect(
        host=HOST, dbname=dbname, user="blocking_bob", password=password
    )
    ...
    try:
        ...
        bobs_query = next(
            (q for q in event["postgres_activity"] if q.get("usename", None) == "bob"),
            None,
        )
        blocking_bobs_query = next(
            (
                ...
                if q.get("usename", None) == "blocking_bob"
            ),
            None,
        )
        ...
        non_client_backend = [
            q
            for q in event["postgres_activity"]
            if q.get("backend_type", "client backend") != "client backend"
        ]
        ...
        if POSTGRES_VERSION.split(".")[0] == "9":
            ...
            assert len(non_client_backend) > 0
            assert all(
                i["backend_type"] == i["statement"] and i["query_signature"]
                for i in non_client_backend
            )
        ...
        assert parser.isoparse(
            bobs_query["now"]
        ).tzinfo, "current timestamp not formatted correctly"
...
def test_truncate_activity_rows(
    integration_check, dbm_instance, active_rows, expected_users
):
    check = integration_check(dbm_instance)
    check._connect()
...
def test_statement_samples_dbstrict(
    aggregator, integration_check, dbm_instance, dbstrict
):
    dbm_instance["query_activity"]["enabled"] = False
    dbm_instance["query_metrics"]["enabled"] = False
    dbm_instance["dbstrict"] = dbstrict
...
def test_async_job_enabled(
    integration_check,
    dbm_instance,
    statement_samples_enabled,
    statement_metrics_enabled,
):
    dbm_instance["query_samples"] = {
        "enabled": statement_samples_enabled,
        "run_sync": False,
    }
    dbm_instance["query_metrics"] = {
        "enabled": statement_metrics_enabled,
        "run_sync": False,
    }
    check = integration_check(dbm_instance)
    check._connect()
...
def test_statement_samples_main_collection_rate_limit(
...
def test_activity_collection_rate_limit(aggregator, integration_check, dbm_instance):
    ...
    assert (
        max_activity_collections / 2.0
        <= len(activity_metrics)
        <= max_activity_collections
    )


@pytest.mark.skip(reason="debugging flaky test (2021-09-03)")
def test_statement_samples_unique_plans_rate_limits(
    aggregator, integration_check, dbm_instance, bob_conn
):
    ...
    dbm_samples = [
        e
        for e in aggregator.get_event_platform_events("dbm-samples")
        if e.get("dbm_type") != "fqt"
    ]
    statement_counts = Counter(_sample_key(e) for e in dbm_samples)
    assert (
        len(statement_counts) == cache_max_size
    ), "expected to collect at most {} unique statements".format(cache_max_size)

    for _, count in statement_counts.items():
        assert (
            count == 1
        ), "expected to collect exactly one sample per statement during this time"
    ...
    assert (
        len(matching) > 0
    ), "should have collected exactly at least one matching event"
...
def test_async_job_cancel_cancel(aggregator, integration_check, dbm_instance):
    ...
    check.statement_metrics._job_loop_future.result()
    assert (
        not check.statement_samples._job_loop_future.running()
    ), "samples thread should be stopped"
    assert (
        not check.statement_metrics._job_loop_future.running()
    ), "metrics thread should be stopped"
    # if the thread doesn't start until after the cancel signal is set then the db connection will never
    # be created in the first place
    assert (
        check._db_pool.get(dbm_instance["dbname"]) is None
    ), "db connection should be gone"
    for job in ["query-metrics", "query-samples"]:
        aggregator.assert_metric(
...
def test_statement_samples_invalid_activity_view(
    aggregator, integration_check, dbm_instance
):
...
def test_statement_samples_config_invalid_number(
    integration_check, pg_instance, attribute, number_key
):
...
def test_pg_stat_statements_max_warning(
    ...
    dbm_instance["statement_samples"]["enabled"] = False
    dbm_instance["query_metrics"][
        "pg_stat_statements_max_warning_threshold"
    ] = pg_stat_statements_max_threshold
    check = integration_check(dbm_instance)
    check._connect()
    check.check(dbm_instance)
...
@requires_over_10
def test_pg_stat_statements_dealloc(
    aggregator, integration_check, dbm_instance_replica2
):
    ...
    aggregator.assert_metric(
        "postgresql.pg_stat_statements.max", value=100, tags=expected_tags
    )
    if float(POSTGRES_VERSION) >= 14.0:
        aggregator.assert_metric(
            "postgresql.pg_stat_statements.dealloc", value=0, tags=expected_tags
        )
    ...
    if float(POSTGRES_VERSION) >= 14.0:
        aggregator.assert_metric(
            "postgresql.pg_stat_statements.dealloc", value=1, tags=expected_tags
        )
    aggregator.assert_metric("postgresql.pg_stat_statements.count", tags=expected_tags)
