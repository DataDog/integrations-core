...
def test_buffercache_metrics(aggregator, integration_check, pg_instance):
    ...
    # Check metrics reported for unused buffers
    # Unused buffers have no usage, can't be pinned and dirtied. No metrics should be emitted for those
    metrics_not_emitted_for_unused_buffers = [
        'postgresql.buffercache.usage_count',
        'postgresql.buffercache.pinning_backends',
        'postgresql.buffercache.dirty_buffers',
    ]
    unused_buffers_tags = base_tags + ['db:shared', 'relation:unused']
    for metric in _iterate_metric_name(BUFFERCACHE_METRICS):
        if metric in metrics_not_emitted_for_unused_buffers:
            aggregator.assert_metric(metric, count=0, tags=unused_buffers_tags)
        else:
            aggregator.assert_metric(metric, count=1, tags=unused_buffers_tags)
