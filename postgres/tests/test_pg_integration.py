...
def test_uptime(aggregator, integration_check, pg_instance):
    ...
    with conn.cursor() as cur:
        cur.execute(
            "SELECT FLOOR(EXTRACT(EPOCH FROM current_timestamp - pg_postmaster_start_time()))"
        )
...
def test_session_number(aggregator, integration_check, pg_instance):
    ...
    aggregator.assert_metric(
        "postgresql.sessions.count", value=session_number, count=1, tags=expected_tags
    )
...
def test_session_idle_and_killed(aggregator, integration_check, pg_instance):
    ...
    aggregator.assert_metric(
        "postgresql.sessions.killed", value=0, count=1, tags=expected_tags
    )
    aggregator.assert_metric(
        "postgresql.sessions.fatal", value=0, count=1, tags=expected_tags
    )
    aggregator.assert_metric(
        "postgresql.sessions.abandoned", value=0, count=1, tags=expected_tags
    )
    ...
    aggregator.assert_metric(
        "postgresql.sessions.killed", value=1, count=1, tags=expected_tags
    )
    aggregator.assert_metric(
        "postgresql.sessions.fatal", value=0, count=1, tags=expected_tags
    )
    aggregator.assert_metric(
        "postgresql.sessions.abandoned", value=1, count=1, tags=expected_tags
    )
...
def test_can_connect_service_check(aggregator, integration_check, pg_instance):
    ...
    aggregator.assert_service_check(
        "postgres.can_connect", count=1, status=PostgreSql.OK, tags=expected_tags
    )
    aggregator.reset()

    # Second: keep the connection open but an unexpected error happens during check run
    orig_db = check.db
    check.db = mock.MagicMock(
        spec=("closed", "status"), closed=False, status=psycopg2.extensions.STATUS_READY
    )
    with pytest.raises(AttributeError):
        check.check(pg_instance)
    aggregator.assert_service_check(
        "postgres.can_connect", count=1, status=PostgreSql.CRITICAL, tags=expected_tags
    )
    aggregator.reset()
...
def test_schema_metrics(aggregator, integration_check, pg_instance):
    ...
    aggregator.assert_metric(
        "postgresql.table.count", value=1, count=1, tags=expected_tags
    )
...
def test_locks_metrics_no_relations(aggregator, integration_check, pg_instance):
    ...
    with psycopg2.connect(
        host=HOST, dbname=DB_NAME, user="postgres", password="datad0g"
    ) as conn:
        with conn.cursor() as cur:
...
def test_activity_metrics_no_application_aggregation(
...
def test_backend_transaction_age(aggregator, integration_check, pg_instance):
    ...
    if float(POSTGRES_VERSION) >= 9.6:
        aggregator.assert_metric(
            "postgresql.activity.backend_xmin_age", value=0, count=1, tags=dd_agent_tags
        )
    else:
        aggregator.assert_metric(
            "postgresql.activity.backend_xmin_age", count=0, tags=dd_agent_tags
        )
    aggregator.assert_metric(
        "postgresql.activity.xact_start_age", count=1, tags=dd_agent_tags
    )
    ...
    if float(POSTGRES_VERSION) >= 9.6:
        aggregator.assert_metric(
            "postgresql.activity.backend_xid_age", value=1, count=1, tags=test_tags
        )
        aggregator.assert_metric(
            "postgresql.activity.backend_xmin_age", value=1, count=1, tags=test_tags
        )

        aggregator.assert_metric(
            "postgresql.activity.backend_xid_age", count=0, tags=dd_agent_tags
        )
        aggregator.assert_metric(
            "postgresql.activity.backend_xmin_age", value=1, count=1, tags=dd_agent_tags
        )
    else:
        aggregator.assert_metric(
            "postgresql.activity.backend_xid_age", count=0, tags=test_tags
        )
        aggregator.assert_metric(
            "postgresql.activity.backend_xmin_age", count=0, tags=test_tags
        )
    ...
    aggregator.assert_metric(
        "postgresql.activity.xact_start_age", count=1, tags=test_tags
    )
    ...
    if float(POSTGRES_VERSION) >= 9.6:
        ...
        aggregator.assert_metric(
            "postgresql.activity.backend_xid_age", value=2, count=1, tags=test_tags
        )
        aggregator.assert_metric(
            "postgresql.activity.backend_xmin_age", value=2, count=1, tags=test_tags
        )
        ...
        aggregator.assert_metric(
            "postgresql.activity.backend_xmin_age", value=2, count=1, tags=dd_agent_tags
        )
...
def test_correct_hostname(
    dbm_enabled, reported_hostname, expected_hostname, aggregator, pg_instance
):
    ...
    with mock.patch(
        ...
        if reported_hostname:
            assert (
                resolve_db_host.called is False
            ), "Expected resolve_db_host.called to be False"
        else:
            assert (
                resolve_db_host.called == dbm_enabled
            ), "Expected resolve_db_host.called to be " + str(dbm_enabled)
    ...
    for name in c_metrics:
        aggregator.assert_metric(
            name, count=1, tags=expected_tags_with_db, hostname=expected_hostname
        )
    check_activity_metrics(
        aggregator, tags=expected_activity_tags, hostname=expected_hostname
    )

    for name in CONNECTION_METRICS:
        aggregator.assert_metric(
            name, count=1, tags=expected_tags_no_db, hostname=expected_hostname
        )
