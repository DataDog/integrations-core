...
| `resource_filters`        | No       | Set parameters (`resource`, `property`, `type` `patterns`) to apply a filter to a VM, host, datastore, data center or cluster. **Requires Datadog Agent v6, v7.18+.**                                                                                                                                                                                         |
| `host_include_only_regex` | No       | Use a regex like this if you want the check to only fetch metrics for these ESXi hosts and the VMs running on it. **Note**: This is a legacy option. Use `resource_filters` for Datadog Agent v6, v7.18.                                                                                                                                                                                                                                                        |
| `vm_include_only_regex`   | No       | Use a regex to include only the VMs that are matching this pattern. **Note**: This is a legacy option. Use `resource_filters` for Datadog Agent v6, v7.18.                                                                                                                                                                                                                                                                                                             |
