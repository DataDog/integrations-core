...
@pytest.mark.parametrize(
    ...
    [
        (None, AgentCheck.OK, {}),
        (HTTPClientStatusError('health check failed'), AgentCheck.CRITICAL, {'message': 'health check failed'}),
        (HTTPClientConnectionError('connection refused'), AgentCheck.CRITICAL, {'message': 'connection refused'}),
    ],
...
def test_healthcheck_client_carries_instance_tls_config(instance_overrides, expected_verify, expected_ignore_warning):
    ...
    """
    check = KubeMetricsServerCheck(CHECK_NAME, {}, [{**instance, **instance_overrides}])
