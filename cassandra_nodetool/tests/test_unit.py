...
@patch('datadog_checks.cassandra_nodetool.check.get_subprocess_output', side_effect=mock_output)
def test_check(mock_output, aggregator):
    _check(mock_output, aggregator)


@patch('datadog_checks.cassandra_nodetool.check.get_subprocess_output', side_effect=mock_output_old_format)
