...
from datadog_checks.gitlab_runner.config_models import discovery_overrides
from datadog_checks.gitlab_runner.config_models.instance import InstanceConfig
from datadog_checks.gitlab_runner.config_models.shared import SharedConfig
...
def _generated_candidates(service: Service) -> Iterator[dict[str, Any]]:
    ...
    for port in candidate_ports(service, [9252]):
        ctx = {'port': port}
        instance_data = {
            'prometheus_endpoint': 'http://{service.host}:{port.number}/metrics'.format(service=service, **ctx),
        }
