...
from .common import LEGACY_REST_INSTANCE, OPENMETRICS_INSTANCE, REST_INSTANCE, REST_METRICS, USE_OPENMETRICS
...
def test_e2e_legacy(aggregator, dd_agent_check):
    dd_agent_check(LEGACY_REST_INSTANCE, rate=True)
    for metric in REST_METRICS:
        aggregator.assert_metric(metric)
    aggregator.assert_service_check('teamcity.build.status')
    aggregator.assert_service_check('teamcity.build.problems')
    aggregator.assert_service_check('teamcity.test.results')
