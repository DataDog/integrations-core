...
def test_omv2_check_2023_05_04(aggregator, openmetrics_instance, mock_http_response, dd_run_check, teamcity_om_check):
    mock_http_response(file_path=get_fixture_path('metrics2023.05.4.txt'))
    check = teamcity_om_check(openmetrics_instance)
    dd_run_check(check)
    for m in PROMETHEUS_METRICS_2023_05_04:
        aggregator.assert_metric(m)
        aggregator.assert_metric_has_tag(m, 'endpoint:http://localhost:8111/guestAuth/app/metrics')
        aggregator.assert_metric_has_tag(m, 'nodeId:MAIN_SERVER')
    aggregator.assert_all_metrics_covered()
    aggregator.assert_metrics_using_metadata(get_metadata_metrics())
    aggregator.assert_service_check(
        'teamcity.openmetrics.health', check.OK, tags=['endpoint:http://localhost:8111/guestAuth/app/metrics']
