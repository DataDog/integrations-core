        ...
        for part in psutil.disk_partitions(all=True):
            ...
            if self._service_check_rw:
                ...
                if len(rwro) == 1:
                    ...
                    self.service_check(
                        'disk.read_write', AgentCheck.UNKNOWN, tags=tags + ['device:{}'.format(device_name)]
                    )
    ...
    def collect_latency_metrics(self):
        for disk_name, disk in iteritems(psutil.disk_io_counters(True)):
            ...
            try:
                ...
                self.rate(
                    self.METRIC_DISK.format('read_time_pct'),
                    read_time_pct,
                    device_name=disk_name,
                    tags=self._custom_tags,
                )
                self.rate(
                    self.METRIC_DISK.format('write_time_pct'),
                    write_time_pct,
                    device_name=disk_name,
                    tags=self._custom_tags,
                )
