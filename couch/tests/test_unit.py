...
def test_check_routes_to_couchdb1_for_major_version_one():
    ...
    with (
        mock.patch("datadog_checks.couch.couch.CouchDB1") as mock_v1,
        mock.patch("datadog_checks.couch.couch.CouchDB2") as mock_v2,
    ):
        check.check({})
...
def test_couchdb2_build_system_metrics_recurses_for_distribution():
    ...
    mock_agent_check.gauge.assert_called_once_with("couchdb.erlang.distribution.inbound", 7, ["t:1", "node:node@host"])
...
def test_couchdb2_build_active_tasks_metrics_covers_all_task_types():
    ...
    mock_agent_check.gauge.assert_any_call("couchdb.active_tasks.view_compaction.changes_done", 7, view_tags_with_phase)

    view_tags_no_phase = ["t:1", "database:kennel", "design_document:dummy"]
    mock_agent_check.gauge.assert_any_call("couchdb.active_tasks.view_compaction.changes_done", 10, view_tags_no_phase)
    assert (
...
def test_couchdb2_get_dbs_to_scan_raises_and_logs_when_node_not_found():
    ...
    mock_agent_check.log.error.assert_called_once_with("Could not find node %r in %r", "missing_node", ["other_node"])
