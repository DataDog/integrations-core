...
from datadog_checks.cockroachdb.config_models import discovery_overrides
from datadog_checks.cockroachdb.config_models.instance import InstanceConfig
from datadog_checks.cockroachdb.config_models.shared import SharedConfig
...
def _generated_candidates(service: Service) -> Iterator[dict[str, Any]]:
    ...
    for port in candidate_ports(service, [8080]):
        ctx = {'port': port}
        instance_data = {
            'openmetrics_endpoint': 'http://{service.host}:{port.number}/_status/vars'.format(service=service, **ctx),
        }
