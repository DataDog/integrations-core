...
from datadog_checks.proxysql.queries import STATS_MYSQL_GLOBAL, STATS_COMMAND_COUNTERS, STATS_MYSQL_CONNECTION_POOL, \
    STATS_MYSQL_USERS, STATS_MEMORY_METRICS, STATS_MYSQL_QUERY_RULES
        ...
        for additional_group in additional_metrics:
            if additional_group not in ADDITIONAL_METRICS_MAPPING:
                raise ConfigurationError(
                    "There is no additional metric group called '%s' for the ProxySQL integration, it should be one "
                    "of (%s)",
                    additional_group,
