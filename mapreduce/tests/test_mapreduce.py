...
from datadog_checks.mapreduce.mapreduce import YARN_SERVICE_CHECK, MAPREDUCE_SERVICE_CHECK
...
def test_check(aggregator, mocked_request):
    ...
    aggregator.assert_service_check(
        YARN_SERVICE_CHECK, status=MapReduceCheck.OK, tags=service_check_tags, count=1
    )
    aggregator.assert_service_check(
        MAPREDUCE_SERVICE_CHECK, status=MapReduceCheck.OK, tags=service_check_tags, count=1
    )
