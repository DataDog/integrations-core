...
from datadog_checks.mapreduce.config_models import discovery_overrides
from datadog_checks.mapreduce.config_models.instance import InstanceConfig
from datadog_checks.mapreduce.config_models.shared import SharedConfig
...
def _generated_candidates(service: Service) -> Iterator[dict[str, Any]]:
    ...
    for port in candidate_ports(service, [8088]):
        ctx = {'port': port}
        instance_data = {
            'resourcemanager_uri': 'http://{service.host}:{port.number}'.format(service=service, **ctx),
            'cluster_name': 'mapreduce-{service.host}'.format(service=service, **ctx),
        }
