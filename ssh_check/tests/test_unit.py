...
def test_private_key_file_none_skips_key_loading():
    ...
    client, ssh = _setup_check_with_mock_client(
        instance, connect_result=CONNECTION_SUCCEEDED, authentication_result=True
    )
...
def test_private_key_type_selects_key_class(private_key_type, expected_ecdsa_calls, expected_rsa_calls):
    ...
    with (
        mock.patch("paramiko.ECDSAKey.from_private_key_file") as mock_ecdsa,
        mock.patch("paramiko.RSAKey.from_private_key_file") as mock_rsa,
    ):
        ssh.check({})
