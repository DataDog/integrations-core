...
def normalize_discover_config_include(log, config):
    normalized_config = {}
    log.debug("normalize_discover_config_include: %s", type(config))
    include_list = config.get('include') if isinstance(config, dict) else config.include if config else []
    log.debug("normalize_discover_config_include: %s", include_list)
