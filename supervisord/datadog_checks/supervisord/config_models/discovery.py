...
from datadog_checks.supervisord.config_models import discovery_overrides
from datadog_checks.supervisord.config_models.instance import InstanceConfig
from datadog_checks.supervisord.config_models.shared import SharedConfig
...
def _generated_candidates(service: Service) -> Iterator[dict[str, Any]]:
    ...
    for port in candidate_ports(service, [9001]):
        ctx = {'port': port}
        instance_data = {
            'name': '{service.id}'.format(service=service, **ctx),
