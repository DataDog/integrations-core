...
3. Finally, use the [Datadog Lambda Log Forwarder function][31] to collect logs from CloudWatch and send them to Datadog. To automatically enrich logs with key ECS tags (task_arn, service_arn, cluster_arn, ...), ensure the following configuration:
    ...
    3. The Lambda forwarder must include the environment variable `DD_SOURCE=ECS`.
