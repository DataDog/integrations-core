...
def test():
    ...
    assert instance_model_contents == normalize_yaml(
        ...
        class InstanceConfig(BaseModel):
            ...
            @validator('foo')
            def _run_foo_pkg_subpkg2_validate2(cls, v, field):
                if not v:
                    return v

                return validation.pkg.subpkg2.validate2(v, field=field)

            @validator('foo')
            def _run_foo_pkg_subpkg2_validate1(cls, v, field):
                if not v:
                    return v

                return validation.pkg.subpkg2.validate1(v, field=field)

            @validator('tags')
            def _run_tags_pkg_subpkg1_validate2(cls, v, field):
                if not v:
                    return v

                return validation.pkg.subpkg1.validate2(v, field=field)

            @validator('tags')
            def _run_tags_pkg_subpkg1_validate1(cls, v, field):
                if not v:
                    return v

                return validation.pkg.subpkg1.validate1(v, field=field)
