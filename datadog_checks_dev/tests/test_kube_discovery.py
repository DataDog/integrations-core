...
def _write_auto_conf(check_root):
...
def test_save_and_get_kube_discovery_state(monkeypatch):
    ...
    assert _get_kube_discovery_state() == {'kubeconfig_path': '/tmp/kubeconfig', 'namespace': DISCOVERY_NAMESPACE}
...
class TestSetupDiscoveryAgent:
    def test_applies_manifests_and_installs_package(self, tmp_path):
        ...
        with (
            ...
            mock.patch('datadog_checks.dev.kube_discovery._generate_auth_token', return_value='deadbeef'),
            mock.patch('datadog_checks.dev.kube_discovery._generate_ipc_cert_pem', return_value='cert-and-key'),
        ):
...
class TestBuildCheckArchive:
    def test_excludes_vcs_and_cache_dirs(self, tmp_path):
        ...
        _build_check_archive(str(check_root), str(archive_path))
...
class TestRunDiscoveryCheckKubernetes:
    ...
    def test_no_json_output_raises(self, monkeypatch):
        ...
        with mock.patch('datadog_checks.dev.kube_discovery.run_command', return_value=_result(stdout='no json here')):
            with mock.patch('datadog_checks.dev.kube_discovery.find_check_root', return_value='/root/test_check'):
...
class TestAssertAllDiscoveryCandidatesStableKubernetes:
    ...
    def test_probes_generated_candidates_and_detects_restart(self, monkeypatch):
        ...
        pod_states = iter([_pod_state(restart_count=0), _pod_state(restart_count=1)])

        def fake_run_command(command, **kwargs):
            if command[:3] == ['kubectl', 'get', 'pod']:
                return _result(stdout=json.dumps(next(pod_states)))
            if command[:2] == ['kubectl', 'logs']:
                return _result(stdout='ready\n')
            return _result()
    ...
    def test_no_candidates_raises(self, monkeypatch):
        ...
        def fake_run_command(command, **kwargs):
            if command[:3] == ['kubectl', 'get', 'pod']:
                return _result(stdout=json.dumps(_pod_state()))
            if command[:2] == ['kubectl', 'logs']:
                return _result(stdout='ready\n')
            return _result()
    ...
    def test_resolves_pod_via_selector(self, monkeypatch):
        ...
        def fake_run_command(command, **kwargs):
            ...
            if command[:3] == ['kubectl', 'get', 'pods']:
                return _result(stdout='workload-abc123')
            if command[:3] == ['kubectl', 'get', 'pod']:
                return _result(stdout=json.dumps(_pod_state()))
            if command[:2] == ['kubectl', 'logs']:
                return _result(stdout='ready\n')
...
class TestAssertPodStable:
    def test_detects_uid_change(self):
        with pytest.raises(AssertionError, match='Pod changed'):
            _assert_pod_stable(_pod_state(uid='a'), _pod_state(uid='b'), 1)

    def test_detects_not_running(self):
        with pytest.raises(AssertionError, match="Pod phase is 'Pending'"):
            _assert_pod_stable(_pod_state(), _pod_state(phase='Pending'), 1)

    def test_detects_restart(self):
        with pytest.raises(AssertionError, match='restart count changed'):
            _assert_pod_stable(_pod_state(restart_count=0), _pod_state(restart_count=1), 1)

    def test_detects_not_ready(self):
        with pytest.raises(AssertionError, match='is not ready'):
            _assert_pod_stable(_pod_state(), _pod_state(ready=False), 1)

    def test_detects_oom_killed(self):
        with pytest.raises(AssertionError, match='OOMKilled'):
            _assert_pod_stable(_pod_state(), _pod_state(terminated_reason='OOMKilled'), 1)

    def test_stable_pod_does_not_raise(self):
        _assert_pod_stable(_pod_state(), _pod_state(), 1)
