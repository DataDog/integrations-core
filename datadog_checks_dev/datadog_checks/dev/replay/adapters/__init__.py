...
ADAPTERS = ('requests', 'subprocess', 'tcp', 'process', 'psycopg', 'clickhouse-connect')
MODES = ('record', 'replay')
...
def _install_one(
    ...
    *,
    ...
    elif adapter == 'clickhouse-connect':
        if mode == 'record':
            return install_live_recording_clickhouse_connect(monkeypatch, fixture_path)
        if mode == 'replay':
            return install_replay_clickhouse_connect(monkeypatch, fixture_path)
