...
def check_duplicates(file,directory):
    """
...
def verify_duplicate_metrics_profile_recursive(file, used_metrics, duplicated, relative_path):
    """
    ...
    """
    extensions = verify_duplicate_metrics_profile_file(file, used_metrics, duplicated, relative_path)
    if extensions:
        for file_name in extensions:
            verify_duplicate_metrics_profile_recursive(file_name, used_metrics, duplicated, relative_path)


def verify_duplicate_metrics_profile_file(file, used_metrics, duplicated, relative_path):
    #type: (any, dict, dict, any) -> list
    """
    ...
    with open(join(relative_path,file)) as f:
    ...
    if file_contents.get('metrics'):
        for metric in file_contents.get('metrics'):
            ...
            if metric.get('symbols'):
                extract_oids_from_symbols(metric, used_metrics, duplicated, file)
