...
class DockerInterface(object):
    ...
    def agent_command(self):
        return 'docker exec {} {}'.format(self.container_name, get_agent_exe(self.agent_version))

    def run_check(self, capture=False, rate=False):
        command = '{} check {}{}'.format(
            self.agent_command, self.check, ' {}'.format(get_rate_flag(self.agent_version)) if rate else ''
        )
