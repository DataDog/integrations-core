...
class BareMetal(Component):
    component_type = Component.Type.BAREMETAL
    service_check_id = IRONIC_SERVICE_CHECK

    def __init__(self, check):
        super(BareMetal, self).__init__(self, check)

    @Component.register_global_metrics(Component.Type.BAREMETAL)
    @Component.http_error(service_check=True)
    def _report_response_time(self, tags):
        self.check.log.debug("reporting `%s` response time", Component.Type.BAREMETAL.value)
        response_time = self.check.api.get_response_time(Component.Type.BAREMETAL)
        self.check.log.debug("`%s` response time: %s", Component.Type.BAREMETAL.value, response_time)
