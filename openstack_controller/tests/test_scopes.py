...
def test_get_endpoint():
    ...
    assert (
        scope_fetcher._get_nova_endpoint(common.EXAMPLE_AUTH_RESPONSE)
    ...
    assert scope_fetcher._get_valid_endpoint({'token': {"catalog": [{}]}}, None, None) is None
    assert (
        scope_fetcher._get_valid_endpoint({'token': {"catalog": [{u'type': u'compute', u'name': u'nova'}]}}, None, None)
        is None
    )
    assert (
        scope_fetcher._get_valid_endpoint(
...
PROJECTS_RESPONSE = [
    {"domain_id": "1111", "id": "3333", "name": "name 1"},
    {"domain_id": "22222", "id": "4444", "name": "name 2"},
]
...
def test_from_config_simple():
    ...
    with mock.patch(
        'datadog_checks.openstack_controller.scopes.KeystoneApi.post_auth_token', return_value=mock_response
    ):
        with mock.patch(
            'datadog_checks.openstack_controller.scopes.KeystoneApi.get_auth_projects', return_value=PROJECTS_RESPONSE
        ):
...
def test_from_config_with_missing_name():
    ...
    with mock.patch(
        ...
        with mock.patch(
            'datadog_checks.openstack_controller.scopes.KeystoneApi.get_auth_projects',
