...
# @pytest.fixture
# def mock_sdk_session_auth_get_access(request, mock_http_call):
#     param = request.param if hasattr(request, 'param') and request.param is not None else {}

#     def session_auth_get_access(session):
#         print(session.auth.project_id)
#         unscoped_auth_token = mock_http_call('POST', '/identity/v3/auth/tokens', 'unscoped')['token']
#         session_auth_access = mock.MagicMock(
#             service_catalog=mock.MagicMock(catalog=unscoped_auth_token.get('catalog', {})),
#             project_id=unscoped_auth_token.get('project', {}).get('id'),
#             role_names=[role.get('name') for role in unscoped_auth_token.get('roles', [])],
#         )
#         # print(session_auth_access)
#         return session_auth_access

#     return mock.MagicMock(side_effect=param.get('session_auth_access', session_auth_get_access))


# @pytest.fixture
# def mock_sdk_get_hypervisor_uptime(request, mock_http_call):
#     param = request.param if hasattr(request, 'param') and request.param is not None else {}

#     def get_hypervisor_uptime(id):
#         return mock_http_call('GET', f'/compute/v2.1/os-hypervisors/{id}/uptime')

#     return mock.MagicMock(side_effect=param.get('compute_hypervisor_uptime', get_hypervisor_uptime))


# @pytest.fixture
# def mock_create_session()
# @pytest.fixture
# def mock_keystoneauth1_session(monkeypatch, mock_responses):
#     def create_session(auth, session):
#         project_id = auth.project_id
#         session = mock.MagicMock(name='session')
#         token = mock_responses('POST', '/identity/v3/auth/tokens',
# auth.project_id if auth.project_id else 'unscoped')[
#             'token'
#         ]
#         print(f'{auth.project_id}:{token}')
#         # session.auth.get_access = mock.MagicMock(
#         #     service_catalog=mock.MagicMock(catalog=token.get('catalog', {})),
#         #     project_id=token.get('project', {}).get('id'),
#         #     role_names=[role.get('name') for role in token.get('roles', [])],
#         # )
#         return session

#     mock_session = mock.MagicMock(project_id=project_id, side_effect = create_session)
#     return mock_session
...
@pytest.fixture
def connection_session_auth(request, mock_responses):
    param = request.param if hasattr(request, 'param') and request.param is not None else {}
...
@pytest.fixture
def mock_sdk_connection(request, mock_http_call):
    request.param if hasattr(request, 'param') and request.param is not None else {}

    # connection = mock.MagicMock()
    # connection.authorize = mock.MagicMock(side_effect=param.get('authorize', ["test_token"] * 5))
    # projects = mock_responses('GET', '/identity/v3/auth/projects')['projects']
    # session_auth_access = [
    #     mock.MagicMock(
    #         service_catalog=mock.MagicMock(
    #             catalog=mock_responses('POST', '/identity/v3/auth/tokens', 'unscoped')['token'].get('catalog', {})
    #         ),
    #         project_id=mock_responses('POST', '/identity/v3/auth/tokens', 'unscoped')['token']
    #         .get('project', {})
    #         .get('id'),
    #         role_names=[
    #             role.get('name')
    #             for role in mock_responses('POST', '/identity/v3/auth/tokens', 'unscoped')['token'].get('roles', [])
    #         ],
    #     )
    # ]
    # for project in projects:
    #     session_auth_access.extend(
    #         [
    #             mock.MagicMock(
    #                 service_catalog=mock.MagicMock(
    #                     catalog=mock_responses('POST', '/identity/v3/auth/tokens', f'{project["id"]}')['token'].get(
    #                         'catalog', {}
    #                     )
    #                 ),
    #                 project_id=mock_responses('POST', '/identity/v3/auth/tokens', f'{project["id"]}')['token']
    #                 .get('project', {})
    #                 .get('id'),
    #                 role_names=[
    #                     role.get('name')
    #                     for role in mock_responses('POST', '/identity/v3/auth/tokens',
    # f'{project["id"]}')['token'].get(
    #                         'roles', []
    #                     )
    #                 ],
    #             ),
    ...
    #         ]
    #     )
    # connection.session.auth.get_access = mock_sdk_session_auth_get_access
    # connection.identity.domains = mock.MagicMock(
    #     side_effect=[
    #         param.get(
    #             'identity_domains',
    #             [
    ...
    #     ],
    # )
    # connection.identity.projects = mock.MagicMock(
    #     side_effect=[
    #         param.get(
    #             'identity_projects',
    #             [
    ...
    #     ],
    # )
    # connection.identity.users = mock.MagicMock(
    #     side_effect=[
    #         param.get(
    #             'identity_users',
    #             [
    ...
    #                     )
    #                 )
    #                 for project in mock_responses('GET', '/identity/v3/users')['users']
    #             ],
    #         )
    #     ],
    # )
    # connection.identity.groups = mock.MagicMock(
    #     side_effect=[
    #         param.get(
    #             'identity_groups',
    #             [
    ...
    #                     )
    #                 )
    #                 for project in mock_responses('GET', '/identity/v3/groups')['groups']
    #             ],
    #         )
    #     ],
    # )
    # identity_group_users_side_effecs = []
    # if param.get('identity_group_users'):
    #     for group in mock_responses('GET', '/identity/v3/groups')['groups']:
    #         if group['id'] in param.get('identity_group_users', {}):
    #             identity_group_users_side_effecs.append(param['identity_group_users'][group])
    #         else:
    #             users = []
    #             for user in mock_responses('GET', f'/identity/v3/groups/{group["id"]}/users')['users']:
    #                 users.append(mock.MagicMock(to_dict=mock.MagicMock(return_value=user)))
    #             identity_group_users_side_effecs.append(users)
    # else:
    #     identity_group_users_side_effecs = [
    #         [
    #             mock.MagicMock(
    ...
    #                 )
    #             )
    #             for domain in mock_responses('GET', f'/identity/v3/groups/{group["id"]}/users')['users']
    #         ]
    #         for group in mock_responses('GET', '/identity/v3/groups')['groups']
    #     ]
    # connection.identity.group_users = mock.MagicMock(
    #     side_effect=identity_group_users_side_effecs,
    # )
    # connection.identity.services = mock.MagicMock(
    #     side_effect=[
    #         param.get(
    #             'identity_services',
    #             [
    ...
    #                     )
    #                 )
    #                 for project in mock_responses('GET', '/identity/v3/services')['services']
    #             ],
    #         )
    #     ],
    # )
    # connection.identity.registered_limits = mock.MagicMock(
    #     side_effect=[
    #         param.get(
    #             'identity_registered_limits',
    #             [
    #                 mock.MagicMock(
    #                     to_dict=mock.MagicMock(
    #                         return_value=project,
    #                     )
    #                 )
    #                 for project in mock_responses('GET', '/identity/v3/registered_limits')['registered_limits']
    #             ],
    #         )
    #     ],
    # )
    # connection.identity.limits = mock.MagicMock(
    #     side_effect=[
    #         param.get(
    #             'identity_limits',
    #             [
    ...
    #                     )
    #                 )
    #                 for project in mock_responses('GET', '/identity/v3/limits')['limits']
    #             ],
    #         )
    #     ],
    # )
    # connection.compute.get_limits = mock.MagicMock(
    #     side_effect=[
    #         param.get(
    #             'compute_limits',
    #             mock.MagicMock(
    ...
    #     ],
    # )
    # connection.compute.services = mock.MagicMock(
    #     side_effect=[
    #         param.get(
    #             'compute_services',
    #             [
    ...
    #                     )
    #                 )
    #                 for project in mock_responses('GET', '/compute/v2.1/os-services')['services']
    #             ],
    #         )
    #     ],
    # )
    # connection.compute.flavors = mock.MagicMock(
    #     side_effect=[
    #         param.get(
    #             'compute_flavors',
    #             [
    ...
    #                     )
    #                 )
    #                 for project in mock_responses('GET', '/compute/v2.1/flavors/detail')['flavors']
    #             ],
    #         )
    #     ],
    # )
    # connection.compute.hypervisors = mock.MagicMock(
    #     side_effect=[
    #         param.get(
    #             'compute_hypervisors',
    #             [
    ...
    #                     )
    #                 )
    #                 for project in mock_responses('GET', '/compute/v2.1/os-hypervisors/detail')['hypervisors']
    #             ],
    #         )
    #     ],
    # )
    # connection.compute.get_hypervisor_uptime = mock_sdk_get_hypervisor_uptime
    # connection.network.agents = mock.MagicMock(
    #     side_effect=[
    #         param.get(
    #             'network_agents',
    #             [
    ...
    #                     )
    #                 )
    #                 for project in mock_responses('GET', '/networking/v2.0/agents')['agents']
    #             ],
    #         )
    #     ],
    # )
    # connection.compute.get_quota_set = mock.MagicMock(
    #     side_effect=param.get(
    #         'compute_quota_sets',
    #         [
    #             mock.MagicMock(
    #                 to_dict=mock.MagicMock(
    #                     return_value=mock_responses('GET',
    # f'/compute/v2.1/os-quota-sets/{project["id"]}')['quota_set'],
    #                 )
    #             )
    #             for project in projects
    #         ],
    #     )
    # )
    # connection.compute.servers = mock.MagicMock(
    #     side_effect=param.get(
    #         'compute_servers',
    #         [
    #             [
    #                 mock.MagicMock(to_dict=mock.MagicMock(return_value=server))
    #                 for server in mock_responses('GET', f'/compute/v2.1/servers/detail?project_id={project["id"]}')[
    #                     'servers'
    #                 ]
    #             ]
    #             for project in projects
    ...
    # server_diagnostics = []
    # for project in projects:
    #     for server in mock_responses('GET', f'/compute/v2.1/servers/detail?project_id={project["id"]}')['servers']:
    #         try:
    #             server_diagnostics.append(
    #                 mock.MagicMock(
    #                     to_dict=mock.MagicMock(
    #                         return_value=mock_responses('GET', f'/compute/v2.1/servers/{server["id"]}/diagnostics')
    #                     )
    #                 )
    #             )
    #         except KeyError:
    #             server_diagnostics.append(Exception())

    # connection.compute.get_server_diagnostics = mock.MagicMock(
    #     side_effect=param.get(
    #         'compute_servers',
    #         server_diagnostics,
    #     )
    # )
    # connection.network.get_quota = mock.MagicMock(
    #     side_effect=param.get(
    #         'network_quotas',
    #         [
    #             mock.MagicMock(
    #                 to_dict=mock.MagicMock(
    #                     return_value=mock_responses('GET', f'/networking/v2.0/quotas/{project["id"]}')['quota'],
    #                 )
    #             )
    #             for project in projects
    #         ],
    ...
    # def session_auth_get_access(session):
    #     auth_token = mock_http_call('POST', '/identity/v3/auth/tokens', project_id if project_id else 'unscoped')[
    #         'token'
    #     ]
    #     session_auth_access = mock.MagicMock(
    #         service_catalog=mock.MagicMock(catalog=auth_token.get('catalog', {})),
    #         project_id=auth_token.get('project', {}).get('id'),
    #         role_names=[role.get('name') for role in auth_token.get('roles', [])],
    #     )
    #     return session_auth_access
    ...
    # def create_connection(cloud, session, compute_api_version):
    #     connection = mock.MagicMock()
    #     connection.session.auth.get_access = mock.MagicMock(
    #         name='jejeje', side_effect=param.get('session_auth_access', session_auth_get_access)
    #     )
    #     return connection

    # with mock.patch('openstack.connection.Connection', mock_create_connection) as mock_connection:
    #     yield mock_connection
...
def mock_http_get(request, monkeypatch, mock_http_call):
    ...
    def get(url, *args, **kwargs):
        # nova_api_version = kwargs.get('headers', {}).get('X-OpenStack-Nova-API-Version')
        # ironic_api_version = kwargs.get('headers', {}).get('X-OpenStack-Ironic-API-Version')
        # nova = f'nova-{nova_api_version}' if nova_api_version else 'nova'
        # ironic = f'ironic-{ironic_api_version}' if ironic_api_version else 'ironic'
        method = 'GET'
