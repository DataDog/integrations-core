...
def mock_http_call(mock_responses):
    def call(method, url, file='response', headers=None, params=None):
        ...
        if response is not None:
            return response
        raise HTTPClientStatusError("404 Not Found", response=MockHTTPResponse(status_code=404, url=url))
...
def connection_baremetal(request, mock_responses):
    ...
    def ports(limit=None):
        ...
        return [
            ...
            for node in mock_responses('GET', '/baremetal/v1/ports/detail')['ports']
        ]
