...
def test_api_error_log_no_password(check, instance, caplog, mock_http):
    mock_http.post.side_effect = HTTPClientStatusError('not found')
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            HTTPClientStatusError('not found', response=mock.Mock(status_code=404)), id='agnostic_status_error'
        ),
...
@pytest.mark.parametrize(
    'error',
    [
        pytest.param(HTTPClientStatusError('boom', response=mock.Mock(status_code=500)), id='agnostic_status_500'),
        pytest.param(HTTPClientStatusError('boom'), id='agnostic_missing_response'),
...
@pytest.mark.parametrize(
    ...
    [
        (HTTPClientStatusError('client error', response=mock.Mock(status_code=400)), False),
        (HTTPClientStatusError('server error', response=mock.Mock(status_code=500)), True),
        (HTTPClientStatusError('no response'), True),
        (HTTPClientConnectionError('connection refused'), True),
