...
def test_readiness_check(
    ...
    mock_http: Any,
    mock_http_response_cls: Callable[..., Any],
    status_code: int,
    expected_value: int,
):
    mock_http.get.return_value = mock_http_response_cls(status_code=status_code)
...
def test_readiness_check_unreachable(aggregator: AggregatorStub, initialized_check: N8nCheck, mock_http: Any):
    mock_http.get.side_effect = HTTPClientConnectionError('boom')
    initialized_check._check_n8n_readiness()
