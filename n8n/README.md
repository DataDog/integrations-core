...
- Workflow metrics: started, success, failed counters; in n8n 2.x, an execution-duration histogram.
- Node metrics: per-node started and finished counters emitted by worker processes in queue mode.
- Queue metrics: queue depth, enqueued/dequeued/completed/failed counters, and scaling-mode worker gauges.
