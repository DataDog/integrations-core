...
from datadog_checks.base import AgentCheck, ensure_unicode, is_affirmative, ConfigurationError
...
class IbmWasCheck(AgentCheck):
    ...
    def __init__(self, name, init_config, instances):
        ...
        self.custom_queries = set(self.instance.get('custom_queries', []))
        self.custom_queries_units_gauge = set(self.instance.get('custom_queries_units_gauge', []))
        self.custom_tags = set(self.instance.get('tags', []))
        collect_stats = self.setup_configured_stats(self.instance)
        nested_tags, metric_categories = self.append_custom_queries(collect_stats)
        self.custom_stats = list(nested_tags)
        self.service_check_tags = self.custom_tags.union(set('url:{}'.format(self.url)))
    ...
    def append_custom_queries(self, collect_stats):
