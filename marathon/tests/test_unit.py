...
def test_get_json_skips_acs_token_refresh_without_acs_url(check):
    ...
    with (
        mock.patch.object(RequestsWrapper, "get", return_value=resp),
        mock.patch.object(RequestsWrapper, "post") as post_mock,
    ):
...
def test_get_json_refreshes_acs_token_when_missing(check):
    ...
    with (
        mock.patch.object(RequestsWrapper, "post", return_value=post_resp) as post_mock,
        mock.patch.object(RequestsWrapper, "get", return_value=get_resp),
    ):
...
def test_get_json_retries_after_401_with_acs_url(check):
    ...
    with (
        mock.patch.object(RequestsWrapper, "post", return_value=post_resp),
        mock.patch.object(RequestsWrapper, "get", side_effect=[resp_401, resp_ok]) as get_mock,
    ):
...
def test_process_queues_emits_all_queue_metrics(check, aggregator):
    ...
    aggregator.assert_metric(
        "marathon.queue.offers.reject.last", value=1, tags=q_tags + ["reason:r1", "status:declined"]
    )
    aggregator.assert_metric(
        "marathon.queue.offers.reject.last", value=4, tags=q_tags + ["reason:r1", "status:processed"]
    )
    aggregator.assert_metric(
