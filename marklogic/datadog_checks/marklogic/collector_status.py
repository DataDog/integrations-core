...
def collect_summary_status_resource_metrics(resource_type, data, tags):
    res_meta = RESOURCE_TYPES[resource_type]
    metrics = data['{}-status-list'.format(resource_type)]['status-list-summary']
    return _collect_status_metrics(res_meta['plural'], metrics, tags)


def collect_summary_status_base_metrics(data, tags):
    ...
    for key, resource_data in iteritems(relations):
        ...
        for metric in _collect_status_metrics(resource_type, metrics, tags):
            yield metric


def _collect_status_metrics(metric_prefix, metrics, tags):
    for key, data in iteritems(metrics):
        if key in ['rate-properties', 'load-properties']:
            ...
            for metric in _collect_status_metrics(metric_prefix, data[prop_type + '-detail'], tags):
                yield metric
        elif key == 'load-properties':
            yield build_metric_to_submit("{}.total-load".format(metric_prefix), data['total-load'], tags)
            for metric in _collect_status_metrics(metric_prefix, data['load-detail'], tags):
                yield metric
        elif is_metric(data):
