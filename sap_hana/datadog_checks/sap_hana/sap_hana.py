...
class SapHanaCheck(AgentCheck):
        ...
        try:
            ...
            if self._connection_lost:
                self.service_check(
                    ...
                    self.WARNING,
                    message="Lost connection to HANA server",
                    tags=self._tags,
                )
                try:
                    self._conn.close()
    ...
    def query_master_database(self):
        ...
        for master in self.iter_rows(queries.MasterDatabase()):
            tags = [
                "db:{}".format(master["db_name"]),
                "usage:{}".format(master["usage"]),
            ]
    ...
    def query_database_status(self):
        ...
        for status in self.iter_rows(queries.SystemDatabases()):
            ...
            self.service_check(
                self.SERVICE_CHECK_STATUS,
                db_status,
                message=message,
                tags=tags,
                hostname=self.get_hana_hostname(),
            )
    ...
    def query_backup_status(self):
        ...
        for backup in self.iter_rows(
            queries.GlobalSystemBackupProgress(schema=self._schema)
        ):
            tags = [
            ...
            seconds_since_last_backup = (
                backup["current_time"] - backup["end_time"]
            ).total_seconds()
            self.gauge(
                "backup.latest", seconds_since_last_backup, tags=tags, hostname=host
            )
    ...
    def query_licenses(self):
        ...
        for hana_license in self.iter_rows(
            queries.GlobalSystemLicenses(schema=self._schema)
        ):
            tags = [
                "sid:{}".format(hana_license["sid"]),
                "product_name:{}".format(hana_license["product_name"]),
            ]
            tags.extend(self._tags)
            ...
            if hana_license["expiration_date"]:
                expiration = (
                    hana_license["expiration_date"] - hana_license["start_date"]
                ).total_seconds()
            else:
                expiration = -1
    ...
    def query_connection_overview(self):
        ...
        for conn in self.iter_rows(
            queries.GlobalSystemConnectionsStatus(schema=self._schema)
        ):
            db_counts[(conn["db_name"], conn["host"], conn["port"])][
                conn["status"].lower()
            ] += conn["total"]
    ...
    def query_disk_usage(self):
        ...
        for disk in self.iter_rows(queries.GlobalSystemDiskUsage(schema=self._schema)):
            tags = [
                "db:{}".format(disk["db_name"]),
                "resource_type:{}".format(disk["resource"]),
            ]
            tags.extend(self._tags)
    ...
    def query_service_memory(self):
        ...
        for memory in self.iter_rows(
            queries.GlobalSystemServiceMemory(schema=self._schema)
        ):
            tags = [
            ...
            # Overall
            self.gauge(
                "memory.service.overall.physical.total",
                memory["physical"],
                tags=tags,
                hostname=host,
            )
            self.gauge(
                "memory.service.overall.virtual.total",
                memory["virtual"],
                tags=tags,
                hostname=host,
            )
            ...
            utilized = compute_percent(used, total)
            self.gauge(
                "memory.service.overall.utilized", utilized, tags=tags, hostname=host
            )

            # Heap
            heap_total = memory["heap_total"]
            self.gauge(
                "memory.service.heap.total", heap_total, tags=tags, hostname=host
            )
            ...
            self.gauge(
                "memory.service.heap.utilized", heap_utilized, tags=tags, hostname=host
            )

            # Shared
            shared_total = memory["shared_total"]
            self.gauge(
                "memory.service.shared.total", shared_total, tags=tags, hostname=host
            )

            shared_used = memory["shared_used"]
            self.gauge(
                "memory.service.shared.used", shared_used, tags=tags, hostname=host
            )

            shared_free = shared_total - shared_used
            self.gauge(
                "memory.service.shared.free", shared_free, tags=tags, hostname=host
            )

            shared_utilized = compute_percent(shared_used, shared_total)
            self.gauge(
                "memory.service.shared.utilized",
                shared_utilized,
                tags=tags,
                hostname=host,
            )
            ...
            self.gauge(
                "memory.service.compactor.total",
                compactors_total,
                tags=tags,
                hostname=host,
            )
            ...
            self.gauge(
                "memory.service.compactor.utilized",
                compactors_utilized,
                tags=tags,
                hostname=host,
            )
    ...
    def query_service_component_memory(self):
        ...
        for memory in self.iter_rows(
            queries.GlobalSystemServiceComponentMemory(schema=self._schema)
        ):
            tags = [
                "db:{}".format(memory["db_name"] or "none"),
                "hana_port:{}".format(memory["port"]),
    ...
    def query_row_store_memory(self):
        ...
        for memory in self.iter_rows(
            queries.GlobalSystemRowStoreMemory(schema=self._schema)
        ):
            tags = [
    ...
    def query_service_statistics(self):
        ...
        for service in self.iter_rows(
            queries.GlobalSystemServiceStatistics(schema=self._schema)
        ):
            tags = [
            ...
            self.gauge(
                "network.service.request.response_time",
                response_time,
                tags=tags,
                hostname=host,
            )
            ...
            self.gauge(
                "network.service.request.per_second",
                requests_per_second,
                tags=tags,
                hostname=host,
            )
            ...
            self.gauge(
                "network.service.request.active",
                requests_active,
                tags=tags,
                hostname=host,
            )
            ...
            self.gauge(
                "network.service.request.pending",
                requests_pending,
                tags=tags,
                hostname=host,
            )
            ...
            self.monotonic_count(
                "network.service.request.total_finished",
            ...
            self.monotonic_count(
                "network.service.request.external.total_finished",
                requests_finished_external,
                tags=tags,
                hostname=host,
            )

            requests_finished_internal = (
                requests_finished_total - requests_finished_external
            )
            self.monotonic_count(
                "network.service.request.internal.total_finished",
                requests_finished_internal,
                tags=tags,
                hostname=host,
            )
            ...
            self.gauge(
                "thread.service.active", threads_active, tags=tags, hostname=host
            )

            threads_inactive = threads_total - threads_active
            self.gauge(
                "thread.service.inactive", threads_inactive, tags=tags, hostname=host
            )
    ...
    def query_volume_io(self):
        ...
        for volume in self.iter_rows(queries.GlobalSystemVolumeIO(schema=self._schema)):
            ...
            self.monotonic_count(
                "volume.io.read.count", reads, tags=tags, hostname=host
            )

            read_size = volume["read_size"]
            self.gauge("volume.io.read.size.total", read_size, tags=tags, hostname=host)
            self.monotonic_count(
                "volume.io.read.size.count", read_size, tags=tags, hostname=host
            )

            read_percent = total_time_to_temporal_percent(
                volume["read_time"], scale=MICROSECOND
            )
            ...
            self.monotonic_count(
                "volume.io.write.count", writes, tags=tags, hostname=host
            )

            write_size = volume["write_size"]
            self.gauge(
                "volume.io.write.size.total", write_size, tags=tags, hostname=host
            )
            self.monotonic_count(
                "volume.io.write.size.count", write_size, tags=tags, hostname=host
            )

            write_percent = total_time_to_temporal_percent(
                volume["write_time"], scale=MICROSECOND
            )
            self.rate(
                "volume.io.write.utilized", write_percent, tags=tags, hostname=host
            )
        ...
        for custom_query in self._custom_queries:
            ...
            for row in chain((first_row,), rows):
                ...
                if len(columns) != len(row):
                    self.log.error(
                        "Custom query result expected %s column(s), got %s",
                        len(columns),
                        len(row),
                    )
                    continue
                ...
                for column, value in zip(columns, row):
                    ...
                    if not column_type:
                        self.log.error(
                            "Column field `type` is required for column `%s`", name
                        )
                        break
                    ...
                    if column_type == "tag":
                        ...
                        if not hasattr(self, column_type):
                            self.log.error(
                                "Invalid submission method `%s` for metric column `%s`",
                                column_type,
                                name,
                            )
                            break
    ...
    def iter_rows(self, query, implicit_values=True):
        ...
        with closing(self._conn.cursor()) as cursor:
            self.execute_query(
                cursor, query.query, "{}.{}".format(query.schema, query.view)
            )
