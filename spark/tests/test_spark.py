...
def test_yarn(aggregator):
    with mock.patch("requests.get", yarn_requests_get_mock):
        ...
        for metric, value in SPARK_JOB_RUNNING_METRIC_VALUES.iteritems():
            aggregator.assert_metric(metric, tags=SPARK_JOB_RUNNING_METRIC_TAGS + ["optional:tag1"], value=value)

        # Check the succeeded job metrics
        for metric, value in SPARK_JOB_SUCCEEDED_METRIC_VALUES.iteritems():
            aggregator.assert_metric(metric, value=value, tags=SPARK_JOB_SUCCEEDED_METRIC_TAGS + ["optional:tag1"])

        # Check the running stage metrics
        for metric, value in SPARK_STAGE_RUNNING_METRIC_VALUES.iteritems():
            aggregator.assert_metric(metric, value=value, tags=SPARK_STAGE_RUNNING_METRIC_TAGS + ["optional:tag1"])

        # Check the complete stage metrics
        for metric, value in SPARK_STAGE_COMPLETE_METRIC_VALUES.iteritems():
            aggregator.assert_metric(metric, value=value, tags=SPARK_STAGE_COMPLETE_METRIC_TAGS + ["optional:tag1"])

        # Check the driver metrics
        for metric, value in SPARK_DRIVER_METRIC_VALUES.iteritems():
            aggregator.assert_metric(metric, value=value, tags=SPARK_METRIC_TAGS + ["optional:tag1"])
...
def test_mesos(aggregator):
    with mock.patch("requests.get", mesos_requests_get_mock):
        ...
        # Check the running job metrics
        for metric, value in SPARK_JOB_RUNNING_METRIC_VALUES.iteritems():
            aggregator.assert_metric(metric, value=value, tags=SPARK_JOB_RUNNING_METRIC_TAGS + ["instance:mytag"])

        # Check the succeeded job metrics
        for metric, value in SPARK_JOB_SUCCEEDED_METRIC_VALUES.iteritems():
            aggregator.assert_metric(metric, value=value, tags=SPARK_JOB_SUCCEEDED_METRIC_TAGS + ["instance:mytag"])

        # Check the running stage metrics
        for metric, value in SPARK_STAGE_RUNNING_METRIC_VALUES.iteritems():
            aggregator.assert_metric(metric, value=value, tags=SPARK_STAGE_RUNNING_METRIC_TAGS + ["instance:mytag"])

        # Check the complete stage metrics
        for metric, value in SPARK_STAGE_COMPLETE_METRIC_VALUES.iteritems():
            aggregator.assert_metric(metric, value=value, tags=SPARK_STAGE_COMPLETE_METRIC_TAGS + ["instance:mytag"])

        # Check the driver metrics
        for metric, value in SPARK_DRIVER_METRIC_VALUES.iteritems():
            aggregator.assert_metric(metric, value=value, tags=SPARK_METRIC_TAGS + ["instance:mytag"])
