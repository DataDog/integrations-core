...
def _generated_candidates(service: Service) -> Iterator[dict[str, Any]]:
    ...
    for port in candidate_ports_by_name(service, ['spark-ui']):
        ctx = {'port': port}
        instance_data = {
            'spark_url': 'http://{service.host}:{port.number}'.format(service=service, **ctx),
            'spark_cluster_mode': 'spark_driver_mode',
            'cluster_name': 'spark-{service.host}'.format(service=service, **ctx),
        }
