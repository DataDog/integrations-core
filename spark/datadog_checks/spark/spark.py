...
RequestsConfig = namedtuple("RequestsConfig", ["auth", "ssl_cert", "ssl_key", "ssl_verify"])
...
class SparkCheck(AgentCheck):
    def check(self, instance):
        ...
        # Get the job metrics
        self._spark_job_metrics(instance, spark_apps, requests_config, tags)

        # Get the stage metrics
        self._spark_stage_metrics(instance, spark_apps, requests_config, tags)

        # Get the executor metrics
        self._spark_executor_metrics(instance, spark_apps, requests_config, tags)

        # Get the rdd metrics
        self._spark_rdd_metrics(instance, spark_apps, requests_config, tags)
    ...
    def _spark_job_metrics(self, instance, running_apps, requests_config, addl_tags):
        """
    ...
    def _spark_stage_metrics(self, instance, running_apps, requests_config, addl_tags):
        """
        ...
        for app_id, (app_name, tracking_url) in running_apps.iteritems():
            ...
            base_url = self._get_request_url(instance, tracking_url)
            response = self._rest_request_to_json(
                base_url, SPARK_APPS_PATH, SPARK_SERVICE_CHECK, requests_config, addl_tags, app_id, "stages"
            )
    ...
    def _spark_executor_metrics(self, instance, running_apps, requests_config, addl_tags):
        """
    ...
    def _spark_rdd_metrics(self, instance, running_apps, requests_config, addl_tags):
        """
    ...
    def _rest_request(self, address, object_path, service_name, requests_config, tags, *args, **kwargs):
        ...
        # Load (User, Pass) authentication (can be None)
