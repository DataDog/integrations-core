...
This validates that all CI entries for integrations are valid. This includes checking if the integration has the correct [Datadog Code Coverage config](https://github.com/DataDog/integrations-core/blob/master/code-coverage.datadog.yml), and has a valid [CI entry](testing.md#target-enumeration) if it is testable.
