...
- `logs_config` - A `list` of configs that will be used by the Logs Agent when `agent_type` is `docker`. You will never need to use this directly, but rather via [higher level abstractions](test.md#logs).
