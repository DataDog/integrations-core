...
- `kubernetes` - Configuration used when `agent_type` is `kubernetes`. It requires `kubeconfig` and optionally accepts `namespace`, `auto_conf`, `image_pull_policy`, and `wait_timeout`. The backend owns the namespace, which must not already exist.
