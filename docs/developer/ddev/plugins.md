...
- `docker_network` - An optional existing Docker network for a Linux Docker Agent to join instead of the host network. Agents on an isolated Docker network use the standard command API port because it is scoped to the container network namespace.
