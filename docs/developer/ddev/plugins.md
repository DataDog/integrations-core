...
- `docker_network` - An optional existing Docker network for a Linux Docker Agent to join instead of the host network. For isolated networks, ddev leaves the command API port unchanged, allowing the Agent to use its configured value or default. Explicit `host` mode behaves like the default and uses a dynamically selected command API port. The shared `container:<name-or-id>` network mode is not supported.
