...
- `docker_network` - An optional existing Docker network for a Linux Docker Agent to join instead of the host network. The Agent can then no longer reach services on the host's `localhost`, so instances must use addresses reachable on that network, such as container hostnames.
