...
1. Add a logo. The integration logo will automatically appear in the header, next to the dashboard title, if the icon exists [here](https://druids.us1.prod.dog/logos/logo-gallery?variant=avatar) and the `integration_id` matches the icon name. That means it will only appear when the dashboard you're working on is made into the official integration dashboard.
