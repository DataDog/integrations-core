...
| `assets`       | Array | Mandatory          | Relative location of where certain asset files live and their name.                                                                                                                                             |
| `assets`-> `dashboards`       | Array | Mandatory          | Dictionary where the key is the name of the dashboard (must be globally unique across integrations) and the value is the relative file path where the dashbaord definition lives.                                                                                                      |
| `assets`-> `monitors`       | Array | Mandatory          | Dictionary where the key is thename of the monitor (must be globally unique across integrations) and the value is the relative file path where the dashbaord definition lives.                                                                                                                                              |
| `assets`-> `service_checks`       | Array | Mandatory          | Relative location of where the `service_checks.json` file lives.                                                                       |
...
| `integration`   | Mandatory          | Name of the integration that emits the metric. Must be the normalzied version of the `display_name` from the `manifest.json` file. Any character besides letters, underscores, dashes and numbers are converted to underscores. E.g. `Openstack Controller` -> `openstack_controller`and `ASP.NET` -> `asp_net` and `CRI-o` -> `cri-o`.                                                                                                                                |
...
| `integration`   | The name of the integration that emits this service check. Must be the `display_name` from the manifest.json.                                                                                                      |
...
| `name`          | Displayed name of the Service Check. The displayed name must be unique across all integrations and self-explanatory.                             |
...
The `logos` folder, under the `assets` folder, must contain **three** images with filenames and sizes that match the following specifications _exactly_. Underneath each specification is a list of places where the images may appear in the web app.
