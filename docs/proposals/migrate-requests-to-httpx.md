...
- **Testing**: Many tests mock `requests.Session.get` or similar. The migration uses two implementation-independent fixtures: (1) **`mock_http_response(CheckClass, ...)`** patches the check’s `get_http_handler` with a `RequestWrapperMock` and response queue. (2) **`mock_http_client`** returns `(client, enqueue)` for tests without a check (inject `client` where the HTTP wrapper is created). Both use `HTTPResponseMock` / `RequestWrapperMock` from `datadog_checks.dev.http` so tests work with either requests or httpx. See `.cursor/rfc-requests-to-httpx-migration.md` and `docs/developer/base/http.md`.
