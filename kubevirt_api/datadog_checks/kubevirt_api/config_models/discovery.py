...
from datadog_checks.kubevirt_api.config_models import discovery_overrides
from datadog_checks.kubevirt_api.config_models.instance import InstanceConfig
from datadog_checks.kubevirt_api.config_models.shared import SharedConfig
...
def _generated_candidates(service: Service) -> Iterator[dict[str, Any]]:
    ...
    for port in candidate_ports(service, [8443]):
        ctx = {'port': port}
        instance_data = {
            'kubevirt_api_metrics_endpoint': 'https://{service.host}:{port.number}/metrics'.format(
                service=service, **ctx
            ),
            'kubevirt_api_healthz_endpoint': 'https://{service.host}:{port.number}/healthz'.format(
                service=service, **ctx
            ),
            'tls_verify': 'false',
        }
