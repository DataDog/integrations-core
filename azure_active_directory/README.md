...
This integration forwards logs to Datadog using the Log Forwarder. Configure Entra ID to forward activity logs to the Log Forwarder storage account.

### Configuration

1. Set up the Log Forwarder by following the [Send Azure Logs to Datadog][2] guide.
...
4. In the Diagnostics settings pane, do one of the following:
   ...
   - To add new settings, select **Add diagnostics setting**. You can have up to five settings.
...
You can view these events in [Event Explorer][4].

**Notes**: 

- To collect Azure app registration expiration events, [enable access to the Microsoft Graph API][5].
...
[4]: /event/explorer
[5]: https://docs.datadoghq.com/integrations/guide/azure-graph-api-permissions/
[6]: https://docs.datadoghq.com/integrations/azure/
