...
class KafkaCheck(AgentCheck):
    ...
    def __init__(self, name, init_config, instances):
        ...
        self.consumer_offsets = None
        self.highwater_offsets = None
    ...
    def check(self, _):
        ...
        try:
            if len(self.consumer_offsets) < self._context_limit:
                # Fetch highwater offsets
                # Expected format: {(topic, partition): offset}
                self.highwater_offsets = self.client.get_highwater_offsets()
            else:
        ...
        total_contexts = len(self.consumer_offsets) + len(self.highwater_offsets)
        ...
        self.report_consumer_offsets_and_lag(self._context_limit - len(self.highwater_offsets))
    ...
    def report_highwater_offsets(self, contexts_limit):
        ...
        for (topic, partition), highwater_offset in self.highwater_offsets.items():
    ...
    def report_consumer_offsets_and_lag(self, contexts_limit):
        ...
        for (consumer_group, topic, partition), consumer_offset in self.consumer_offsets.items():
            if reported_contexts >= contexts_limit:
                self.log.debug(
            ...
            if partitions is not None and partition in partitions:
                ...
                if (topic, partition) not in self.highwater_offsets:
                    self.log.warning(
                    ...
                    continue
                producer_offset = self.highwater_offsets[(topic, partition)]
