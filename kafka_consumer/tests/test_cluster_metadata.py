...
def test_schema_registry_two_tier_ttl(check):
    """Test that schema version checks reuse CONFIGS_REFRESH_INTERVAL and event emission uses longer TTL."""
    instance = {
        'kafka_connect_str': 'localhost:9092',
        'enable_cluster_monitoring': True,
        'schema_registry_url': 'http://localhost:8081',
    ...
    # Schema event emission TTL should be 2 hours (vs 10 min for broker/topic)
    assert collector.SCHEMA_EVENT_CACHE_TTL == 7200
    assert collector.SCHEMA_EVENT_CACHE_TTL > collector.EVENT_CACHE_TTL
...
def test_topic_config_metrics_cached_between_runs(check, dd_run_check, aggregator):
    """Test that topic config metrics are re-emitted from cache on runs where the topic is not freshly fetched."""
    instance = {
        'kafka_connect_str': 'localhost:9092',
        'enable_cluster_monitoring': True,
        'monitor_unlisted_consumer_groups': True,
    ...
    # First run: topics are freshly fetched and metrics emitted
    dd_run_check(kafka_consumer_check)
    ...
    # Verify metrics cache was populated
    assert 'kafka_topic_config_metrics_cache' in cache_storage
    metrics_cache = json.loads(cache_storage['kafka_topic_config_metrics_cache'])
    assert 'test-topic' in metrics_cache

    aggregator.reset()

    # Second run: topic config fetch cache is still valid (not expired),
    # so no describe_configs call, but metrics should still be emitted from cache.
    dd_run_check(kafka_consumer_check)
