...
def test_call_api_401_comparison_boundaries_do_not_trigger_credentials_event(
    mock_http_response, aggregator, status_code
):
...
def test_call_api_403_comparison_boundaries_do_not_trigger_permission_event(
    mock_http_response, aggregator, status_code
):
    mock_http_response(status_code=status_code, content="")
    check = SonatypeNexusCheck("sonatype_nexus", {}, [instance])
