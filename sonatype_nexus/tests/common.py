...
def get_nexus_password():
    container_id = run_command("docker ps --filter 'ancestor=sonatype/nexus3' --format '{{.ID}}'", capture=True)
    ...
    password = run_command(f"docker exec {container_id} sh -c 'cat {password_file}'", capture=True)
