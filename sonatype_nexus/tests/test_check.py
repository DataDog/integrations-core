...
def test_validate_minimum_collection_interval_called(mock_validate_minimum_collection_interval):
    check = setup_check()
...
def test_json_decode_error(mock_client_class):
    ...
    mock_client = MagicMock()
    mock_client_class.return_value = mock_client
    mock_client.call_sonatype_nexus_api.return_value.json.side_effect = requests.exceptions.JSONDecodeError('Test error', '', 0)
    check.sonatype_nexus_client = mock_client
    result = check.generate_and_yield_status_metrics()
    assert result == {'message': "can't decode response to json", 'error': 'Test error: line 1 column 1 (char 0)'}
...
class TestIngestMetric(unittest.TestCase):
    ...
    def test_valid_input(self):
        ...
        self.check.gauge.assert_called_once_with(metric_name, 10, base_tag + [f"{metric_info['tag_key']}:{format_type}"], hostname=None)
