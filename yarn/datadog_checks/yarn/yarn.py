...
class YarnCheck(AgentCheck):
    ...
    def check(self, instance):
        ...
        rm_address = instance.get("resourcemanager_uri", self.DEFAULT_RM_URI)
        ...
        if _is_affirmative(instance.get("collect_app_metrics", self.DEFAULT_COLLECT_APP_METRICS)):
    ...
    def _yarn_cluster_metrics(self, rm_address, auth, addl_tags):
        ...
        metrics_json = self._rest_request_to_json(
            rm_address, auth, self.YARN_CLUSTER_METRICS_PATH, addl_tags
        )

        if metrics_json:
            yarn_metrics = metrics_json[self.YARN_CLUSTER_METRICS_ELEMENT]
            if yarn_metrics is not None:
                self._set_yarn_metrics_from_json(addl_tags, yarn_metrics, self.YARN_CLUSTER_METRICS)
    ...
    def _yarn_app_metrics(self, rm_address, auth, app_tags, addl_tags):
        ...
        """
        metrics_json = self._rest_request_to_json(
            rm_address, auth, self.YARN_APPS_PATH, addl_tags, states=self.YARN_APPLICATION_STATES
        )
            ...
            for app_json in metrics_json["apps"]["app"]:
                ...
                for dd_tag, yarn_key in app_tags.iteritems():
                    try:
                        ...
                        if val:
                            tags.append("{}:{}".format(dd_tag, val))
                    except KeyError:
                        self.log.error("Invalid value %s for application_tag", yarn_key)

                tags.extend(addl_tags)

                self._set_yarn_metrics_from_json(tags, app_json, self.YARN_APP_METRICS)

    def _yarn_node_metrics(self, rm_address, auth, addl_tags):
        """
        Get metrics related to YARN nodes
        """
        metrics_json = self._rest_request_to_json(rm_address, auth, self.YARN_NODES_PATH, addl_tags)
        ...
        if metrics_json and metrics_json["nodes"] is not None and metrics_json["nodes"]["node"] is not None:
            ...
            for node_json in metrics_json["nodes"]["node"]:
                ...
                self._set_yarn_metrics_from_json(tags, node_json, self.YARN_NODE_METRICS)

    def _yarn_scheduler_metrics(self, rm_address, auth, addl_tags, queue_blacklist):
        """
        Get metrics from YARN scheduler
        """
        metrics_json = self._rest_request_to_json(rm_address, auth, self.YARN_SCHEDULER_PATH, addl_tags)
    ...
    def _yarn_capacity_scheduler_metrics(self, metrics_json, addl_tags, queue_blacklist):
        ...
        self._set_yarn_metrics_from_json(tags, metrics_json, self.YARN_ROOT_QUEUE_METRICS)
        ...
        if metrics_json["queues"] is not None and metrics_json["queues"]["queue"] is not None:
            ...
            for queue_json in metrics_json["queues"]["queue"]:
                ...
                queues_count += 1
                if queues_count > self.MAX_DETAILED_QUEUES:
                    self.warning(
                ...
                self._set_yarn_metrics_from_json(tags, queue_json, self.YARN_QUEUE_METRICS)
