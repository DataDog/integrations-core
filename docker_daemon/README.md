...
To collect Docker metrics about all your containers, run **one** Datadog Agent on every host. There are two ways to run the Agent: directly on each host, or within a [docker-dd-agent container][2]. We recommend the latter.  

Whichever you choose, your hosts need to have cgroup memory management enabled for the Docker check to succeed. See the [docker-dd-agent repository][3] for how to enable it.
...
| DD_URL | Sets the Datadog intake server URL where the Agent will send data. This is useful when [using an agent as a proxy][9]. |
...
| NON_LOCAL_TRAFFIC | Enabling this feature will allow statsd reporting from any external IP. For example, `-e NON_LOCAL_TRAFFIC=yes`. This can be used to report metrics from other containers or systems. See [network configuration][10] for more details.
| PROXY_HOST, PROXY_PORT, PROXY_USER, PROXY_PASSWORD | Sets proxy configuration details. For more information, see the [Agent proxy documentation][11] |
| SD_BACKEND, SD_CONFIG_BACKEND, SD_BACKEND_HOST, SD_BACKEND_PORT, SD_TEMPLATE_DIR, SD_CONSUL_TOKEN | Enables and configures Autodiscovery. For more information, please see the [Autodiscovery guide][12]. |
...
Our standard Docker image is based on Debian Linux, but as of version 5.7 of the Datadog Agent, we also offer an [Alpine Linux][13] based image. The Alpine Linux image is considerably smaller in size than the traditional Debian-based image. It also inherits Alpine's security-oriented design.
...
For more information about building custom Docker containers with the Datadog Agent, the Alpine Linux based image, versioning, and more, please reference [our `docker-dd-agent` project on Github][2].
...
The [`import`][16] command converts the old `docker_daemon.yaml` to the new `docker.yaml`. The command also moves needed settings from `docker_daemon.yaml` to `datadog.yaml`.
...
Learn more about how to monitor Docker performance metrics thanks to [our series of posts][21]. We detail the challenges when monitoring Docker, its key performance metrics, how to collect them, and lastly how the largest TV and radio outlet in the U.S. monitors Docker using Datadog.

We've also written several other in-depth blog posts to help you get the most out of Datadog and Docker:

* [How to Monitor Docker Resource Metrics][22]
* [How to Collect Docker Metrics][23]
* [8 Surprising Facts about Real Docker Adoption][24]
* [Monitor Docker on AWS ECS][25]
* [Dockerize Datadog][26]
* [Monitor Docker with Datadog][27]
...
[16]: https://docs.datadoghq.com/agent/#cli
[17]: https://github.com/DataDog/integrations-core/blob/master/docker_daemon/metadata.csv
[18]: https://docs.datadoghq.com/help
[19]: https://docs.datadoghq.com/integrations/faq/compose-and-the-datadog-agent
[20]: https://docs.datadoghq.com/integrations/faq/dogstatsd-and-docker
[21]: https://www.datadoghq.com/blog/the-docker-monitoring-problem
[22]: https://www.datadoghq.com/blog/how-to-monitor-docker-resource-metrics
[23]: https://www.datadoghq.com/blog/how-to-collect-docker-metrics
[24]: https://www.datadoghq.com/docker-adoption
[25]: https://www.datadoghq.com/blog/monitor-docker-on-aws-ecs
[26]: https://www.datadoghq.com/blog/docker-performance-datadog
[27]: https://www.datadoghq.com/blog/monitor-docker-datadog
