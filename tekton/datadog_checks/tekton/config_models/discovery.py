...
from datadog_checks.tekton.config_models.discovery_strategies import from_tekton_kube_container_name
...
def _generated_candidates(service: Service) -> Iterator[dict[str, Any]]:
    ...
    # discovery[0]: local:from_tekton_kube_container_name
    for ctx in from_tekton_kube_container_name(service):
        instance_data = {
            'pipelines_controller_endpoint': '{endpoints.pipelines_controller_endpoint}'.format(service=service, **ctx),
            'triggers_controller_endpoint': '{endpoints.triggers_controller_endpoint}'.format(service=service, **ctx),
        }
