...
This check monitors the kernel OOM kill process through the Datadog Agent. It also depends on the Process Agent and System Probe Agent.
...
3. Ensure that the Process Agent is enabled.

4. [Restart the Agent][2].

### Configuration with Helm

With the [Datadog Helm chart][3], ensure that `datadog.processAgent`, `datadog.systemProbe` and `datadog.systemProbe.enableOOMKill` parameters are enabled in the `values.yaml` file.
