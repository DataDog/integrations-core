...
class RabbitMQ(AgentCheck):
    ...
    def _get_data(self, url, auth=None, ssl_verify=True, proxies=None):
        ...
        try:
            r = requests.get(
                url, auth=auth, proxies=proxies, timeout=self.default_integration_http_timeout, verify=ssl_verify
            )
            r.raise_for_status()
            return r.json()
    ...
    def get_stats(
        ...
        if self._limit_vhosts(instance) and object_type == QUEUE_TYPE:
            for vhost in limit_vhosts:
                ...
                try:
                    data += self._get_data(
                        urlparse.urljoin(base_url, url), auth=auth, ssl_verify=ssl_verify, proxies=instance_proxy
                    )
                except Exception as e:
                    self.log.debug("Couldn't grab queue data from vhost, {}: {}".format(vhost, e))
        else:
            data = self._get_data(
                urlparse.urljoin(base_url, object_type), auth=auth, ssl_verify=ssl_verify, proxies=instance_proxy
            )
    ...
    def get_overview_stats(self, instance, base_url, custom_tags, auth=None, ssl_verify=True):
        instance_proxy = self.get_instance_proxy(instance, base_url)
        data = self._get_data(
            urlparse.urljoin(base_url, "overview"), auth=auth, ssl_verify=ssl_verify, proxies=instance_proxy
        )
