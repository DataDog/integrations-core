...
def test_system_errors(dd_agent_check, instance, clickhouse_client):
    aggregator = dd_agent_check(instance, rate=True)
    aggregator.assert_metric('clickhouse.ClickHouseErrors_UNKNOWN_IDENTIFIER.total', at_least=0)

    with pytest.raises(Exception):
        clickhouse_client.execute('SELECT unknown')

    aggregator = dd_agent_check(instance, rate=True)
    aggregator.assert_metric('clickhouse.ClickHouseErrors_UNKNOWN_IDENTIFIER.total', at_least=1)
