...
def test_e2e_discovery(dd_agent_check_discovery):
    if get_tls():
        pytest.skip('discovery does not configure TLS for Zookeeper client ports')
...
def test_e2e_discovery_all_candidates(dd_agent_check):
    if get_tls():
        pytest.skip('discovery candidate probing does not configure TLS for Zookeeper client ports')

    assert_all_discovery_candidates_stable(dd_agent_check, ZookeeperCheck)
