...
from datadog_checks.zk.config_models import discovery_overrides
from datadog_checks.zk.config_models.instance import InstanceConfig
from datadog_checks.zk.config_models.shared import SharedConfig
...
def _generated_candidates(service: Service) -> Iterator[dict[str, Any]]:
    ...
    for port in candidate_ports(service, [2181], fallback=False):
        ctx = {'port': port}
        instance_data = {
            'host': '{service.host!s}'.format(service=service, **ctx),
