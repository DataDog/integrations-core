...
from datadog_checks.hdfs_datanode.config_models import discovery_overrides
from datadog_checks.hdfs_datanode.config_models.instance import InstanceConfig
from datadog_checks.hdfs_datanode.config_models.shared import SharedConfig
...
def _generated_candidates(service: Service) -> Iterator[dict[str, Any]]:
    ...
    for port in candidate_ports(service, [9864]):
        ctx = {'port': port}
        instance_data = {
            'hdfs_datanode_jmx_uri': 'http://{service.host}:{port.number}'.format(service=service, **ctx),
        }
