...
def test_retry_on_rate_limit_success_no_retry(dd_run_check, aggregator, mock_instance, mock_http_get, mock_http):
    ...
    mock_http.get.return_value = MockHTTPResponse(json_data={"data": {"test": "data"}})
...
def test_retry_on_rate_limit_429_then_success(
    dd_run_check, aggregator, mock_instance, mock_http_get, mock_http, mocker
):
...
def test_retry_on_rate_limit_max_retries_exceeded(
    ...
    mock_http.get.reset_mock()
    mock_http.get.side_effect = None
    mock_http.get.return_value = MockHTTPResponse(status_code=429)
    mock_sleep = mocker.patch('time.sleep')
...
def test_retry_on_non_429_error_no_retry(dd_run_check, aggregator, mock_instance, mock_http_get, mock_http, mocker):
...
def test_retry_with_custom_config(dd_run_check, aggregator, mock_instance, mock_http_get, mock_http, mocker):
...
def test_retry_exponential_backoff(dd_run_check, aggregator, mock_instance, mock_http_get, mock_http, mocker):
...
def test_health_check_with_retry(dd_run_check, aggregator, mock_instance, mock_http_get, mock_http, mocker):
    ...
    mock_http.get.side_effect = [
        MockHTTPResponse(status_code=429),
        MockHTTPResponse(status_code=200),
    ]
    mock_sleep = mocker.patch('time.sleep')
