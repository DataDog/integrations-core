...
def test_get_alert_returns_none_on_404(dd_run_check, mock_instance, mock_http_get, mocker):
    ...
    mocker.patch.object(
        check, '_get_request_data', side_effect=HTTPClientStatusError("mocked HTTP error", response=response)
    )
...
def test_transient_alert_get_failure_preserves_tracking(
    ...
    mocker.patch.object(
        check.activity_monitor, '_get_alert', side_effect=HTTPClientStatusError("mocked HTTP error", response=response)
    )
