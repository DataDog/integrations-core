...
from datadog_checks.boundary.config_models import discovery_overrides
from datadog_checks.boundary.config_models.instance import InstanceConfig
from datadog_checks.boundary.config_models.shared import SharedConfig
...
def _generated_candidates(service: Service) -> Iterator[dict[str, Any]]:
    ...
    for port in candidate_ports(service, [9203]):
        ctx = {'port': port}
        instance_data = {
            'health_endpoint': 'http://{service.host}:{port.number}/health'.format(service=service, **ctx),
