...
This check collects Prometheus metrics from an SGLang server, including request throughput, token
throughput, latency (time to first token, inter-token latency, and end-to-end latency), queue depth,
KV cache utilization and hit rate, speculative decoding efficiency, and process resource usage.
...
This integration is part of GPU Monitoring and only runs when GPU Monitoring is enabled.
...
Run the [Agent status command][5] and look for `sglang` under the Checks section.
...
See [service_checks.json][7] for the service checks provided by this integration.
...
[5]: https://docs.datadoghq.com/agent/configuration/agent-commands/#agent-status-and-information
[6]: https://github.com/DataDog/integrations-core/blob/master/sglang/metadata.csv
[7]: https://github.com/DataDog/integrations-core/blob/master/sglang/assets/service_checks.json
