...
def test_can_connect_down(dd_run_check, aggregator, instance, mock_http):
    mock_http.post.side_effect = HTTPClientConnectionError('connection refused')
...
def test_version_failure(dd_run_check, aggregator, instance, mock_auth, mock_http, caplog):
    mock_http.get.side_effect = HTTPClientStatusError('500 Server Error', response=MockHTTPResponse(status_code=500))
