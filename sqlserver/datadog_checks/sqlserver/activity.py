...
SELECT
    ...
    LOWER(
        LTRIM(RTRIM(
            CASE 
                WHEN CHARINDEX('CREATE PROCEDURE ', UPPER(LTRIM(lqt.text))) > 0 
                    THEN SUBSTRING(LTRIM(lqt.text), CHARINDEX('CREATE PROCEDURE ', UPPER(LTRIM(lqt.text))) + LEN('CREATE PROCEDURE ') + 1, 
                                   PATINDEX('%[ ,'+CHAR(10)+CHAR(13)+']%', 
                                            SUBSTRING(LTRIM(lqt.text), CHARINDEX('CREATE PROCEDURE ', UPPER(LTRIM(lqt.text))) + LEN('CREATE PROCEDURE ') + 1, 1000)) - 1)
                WHEN CHARINDEX('CREATE PROC ', UPPER(LTRIM(lqt.text))) > 0 AND CHARINDEX('CREATE PROCEDURE ', UPPER(LTRIM(lqt.text))) = 0 
                    THEN SUBSTRING(LTRIM(lqt.text), CHARINDEX('CREATE PROC ', UPPER(LTRIM(lqt.text))) + LEN('CREATE PROC ') + 1, 
                                   PATINDEX('%[ ,'+CHAR(10)+CHAR(13)+']%', 
                                            SUBSTRING(LTRIM(lqt.text), CHARINDEX('CREATE PROC ', UPPER(LTRIM(lqt.text))) + LEN('CREATE PROC ') + 1, 1000)) - 1)
                ELSE NULL
            END
        ))
    ) AS procedure_name,
