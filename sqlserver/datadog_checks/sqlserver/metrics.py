...
# If the WSFC node has no quorum, no row is returned.
...
class SqlFailoverCluster(BaseSqlServerMetric):
    CUSTOM_QUERIES_AVAILABLE = False
    TABLE = 'sys.dm_hadr_cluster'
    DEFAULT_METRIC_TYPE = 'gauge'
    QUERY_BASE = """select * from {table}""".format(table=TABLE)
    ...
    def fetch_metric(self, rows, columns):
        value_column_index = columns.index(self.column)
        quorum_type_desc_index = columns.index("quorum_type_desc")
        quorum_state_desc_index = columns.index("quorum_state_desc")

        for row in rows:
            column_val = row[value_column_index]
            quorum_type_desc = row[quorum_type_desc_index]
            quorum_state_desc = row[quorum_state_desc_index]
            metric_tags = [
                'quorum_type_desc:{}'.format(str(quorum_type_desc)),
                'quorum_state_desc:{}'.format(str(quorum_state_desc)),
            ]
            metric_tags.extend(self.tags)
            metric_name = '{}'.format(self.datadog_name)
            self.report_function(metric_name, column_val, tags=metric_tags)
...
#
# If the WSFC node that hosts a local instance of SQL Server that is enabled for Always On availability groups
# has WSFC quorum, returns a row for each of the members that constitute the quorum and the state of each of them.
# https://docs.microsoft.com/en-us/sql/relational-databases/system-dynamic-management-views/sys-dm-hadr-cluster-members-transact-sql?view=sql-server-ver15
class SqlFailoverClusterMember(BaseSqlServerMetric):
    CUSTOM_QUERIES_AVAILABLE = False
    TABLE = 'sys.dm_hadr_cluster_members'
    DEFAULT_METRIC_TYPE = 'gauge'
    QUERY_BASE = """select * from {table}""".format(table=TABLE)
    ...
    def fetch_metric(self, rows, columns):
        value_column_index = columns.index(self.column)
        member_name_index = columns.index("member_name")
        member_type_desc_index = columns.index("member_type_desc")
        member_state_desc_index = columns.index("member_state_desc")

        for row in rows:
            column_val = row[value_column_index]
            member_name = row[member_name_index]
            member_type_desc = row[member_type_desc_index]
            member_state_desc = row[member_state_desc_index]
            metric_tags = [
                'member_name:{}'.format(str(member_name)),
                'member_type_desc:{}'.format(str(member_type_desc)),
                'member_state_desc:{}'.format(str(member_state_desc)),
            ]
            metric_tags.extend(self.tags)
            metric_name = '{}'.format(self.datadog_name)
            self.report_function(metric_name, column_val, tags=metric_tags)
...
class SqlFailoverClusterInstance(BaseSqlServerMetric):
    CUSTOM_QUERIES_AVAILABLE = False
    ...
    def fetch_metric(self, rows, columns):
        ...
        status_desc_index = columns.index("status_description")

        for row in rows:
            column_val = row[value_column_index]
            node_name = row[node_name_index]
            status_desc = row[status_desc_index]
            metric_tags = [
                'node_name:{}'.format(str(node_name)),
                'status_desc:{}'.format(str(status_desc)),
            ]
            metric_tags.extend(self.tags)
            metric_name = '{}'.format(self.datadog_name)
            self.report_function(metric_name, column_val, tags=metric_tags)
