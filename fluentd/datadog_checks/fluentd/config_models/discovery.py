...
from datadog_checks.fluentd.config_models import discovery_overrides
from datadog_checks.fluentd.config_models.instance import InstanceConfig
from datadog_checks.fluentd.config_models.shared import SharedConfig
...
def _generated_candidates(service: Service) -> Iterator[dict[str, Any]]:
    ...
    for port in candidate_ports(service, [24220]):
        ctx = {'port': port}
        instance_data = {
            'monitor_agent_url': 'http://{service.host}:{port.number}/api/plugins.json'.format(service=service, **ctx),
        }
