...
def test_e2e_user_profiles(dd_agent_check):
    ...
    aggregator.assert_metric(
        'snmp.upsAdvBatteryNumOfBattPacks', metric_type=aggregator.GAUGE, tags=metric_tags, count=2
    )
    aggregator.assert_metric(
        'snmp.upsAdvBatteryFullCapacity_userMetric', metric_type=aggregator.GAUGE, tags=metric_tags, count=2
    )
...
def test_e2e_memory_cpu_f5_big_ip(dd_agent_check):
    ...
    for metric in cpu_metrics:
        for cpu_index in cpu_indexes:
            cpu_tags = metric_tags + ['cpu:{}'.format(cpu_index)]
            aggregator.assert_metric(
                'snmp.{}'.format(metric),
                metric_type=aggregator.GAUGE,
                tags=cpu_tags,
...
def test_e2e_meraki_cloud_controller(dd_agent_check):
    ...
    if_tags = ['interface:wifi0', 'index:4', 'mac_address:02:02:00:66:f5:00'] + metric_tags
    ...
    custom_speed_tags = ['interface:eth0', 'interface_index:11', 'speed_source:device'] + metric_tags
    ...
    aggregator.assert_metric('snmp.sysUpTimeInstance', count=2, tags=metric_tags)
    aggregator.assert_metric(
...
def test_e2e_cisco_nexus(dd_agent_check):
    ...
    for index, state in [
        ...
        aggregator.assert_metric(
            ...
            metric_type=aggregator.GAUGE,
            tags=['temp_state:{}'.format(state), 'temp_index:{}'.format(index)] + metric_tags,
        )

    power_supply_tags = ['power_source:unknown', 'power_status_descr:Jaded driving their their their'] + metric_tags
    ...
    for tag_row in tag_rows:
        aggregator.assert_metric('snmp.cswSwitchState', metric_type=aggregator.GAUGE, tags=tag_row + metric_tags)

    frus = [2, 7, 8, 21, 26, 27, 30, 31]
    for fru in frus:
        tags = ['fru:{}'.format(fru)] + metric_tags
        aggregator.assert_metric(
            'snmp.cefcFanTrayOperStatus', metric_type=aggregator.GAUGE, tags=['fru:{}'.format(fru)] + metric_tags
        )
    ...
    for cpu in cpu_ids:
        aggregator.assert_metric(
            'snmp.cpu.usage', metric_type=aggregator.GAUGE, tags=metric_tags + ['cpu:{}'.format(cpu)]
        )

    nexus_mem_metrics = ["memory.free", "memory.used", "memory.usage"]
    for metric in nexus_mem_metrics:
        aggregator.assert_metric('snmp.{}'.format(metric), metric_type=aggregator.GAUGE, tags=metric_tags + ['mem:1'])
...
def test_e2e_cisco_legacy_wlc(dd_agent_check):
    ...
    if_tags = ["interface:If1", "interface_index:1"] + metric_tags
    ...
    wlan_tags = ["ssid:DD-1", "wlan_index:17"] + metric_tags
