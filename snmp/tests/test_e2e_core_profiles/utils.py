...
"""
    ...
    for cpu_row in cpu_rows:
        ...
        aggregator.assert_metric('snmp.hrProcessorLoad', metric_type=aggregator.GAUGE, tags=common_tags + [f'processorid:{cpu_row}'])
    ...
    for mem_row in mem_rows:
        aggregator.assert_metric('snmp.memory.total', metric_type=aggregator.GAUGE, tags=common_tags + [f'mem:{mem_row}'])
        aggregator.assert_metric('snmp.memory.usage', metric_type=aggregator.GAUGE, tags=common_tags + [f'mem:{mem_row}'])
        aggregator.assert_metric('snmp.memory.used', metric_type=aggregator.GAUGE, tags=common_tags + [f'mem:{mem_row}'])
...
"""
    additional_tags = ['ent_phy_sensor_oper_status:nonoperational', 'sensor_precision:7', 'sensor_scale:micro', 'sensor_type:percent_rh', 'sensor_units:driving driving forward acted their but']
