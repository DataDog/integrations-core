...
def test_e2e_profile__generic_ucd_base(dd_agent_check):
    profile = '_generic-ucd-base'
    config = create_e2e_core_test_config(profile)
    aggregator = common.dd_agent_check_wrapper(dd_agent_check, config, rate=True)

    ip_address = get_device_ip_from_config(config)
    common_tags = [
        'snmp_profile:generic-ucd-base',
    ...
    device = {
        ...
        'profile': 'generic-ucd-base',
        'status': 1,
        'sys_object_id': '1.2.3.40',
        'device_type': 'other',
        'integration': 'snmp',
    }
