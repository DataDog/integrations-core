...
def test_detect_sli_endpoint_404(aggregator, instance, mock_openmetrics_http, mock_http_response_cls):
    mock_openmetrics_http.get.side_effect = [
        mock_http_response_cls(status_code=404),
        mock_http_response_cls(status_code=200),
    ]
    c = KubeAPIServerMetricsCheck(CHECK_NAME, {}, [instance])
    c.check(instance)
    SLIMetric = namedtuple("SLIMetric", ["name", "count"])
...
def test_detect_sli_endpoint_403(aggregator, instance, mock_openmetrics_http, mock_http_response_cls):
    mock_openmetrics_http.get.side_effect = [
        mock_http_response_cls(status_code=403),
        mock_http_response_cls(status_code=200),
    ]
    c = KubeAPIServerMetricsCheck(CHECK_NAME, {}, [instance])
    c.check(instance)
