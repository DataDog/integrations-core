...
1. To monitor the `istiod` deployment in Istio `v1.5+`, use the following configuration:
   ...
   Istio mesh metrics are now only available from `istio-proxy` contaienrs. These metrics are supported out-of-the-box via autodiscovery, see [`istio.d/auto_conf.yaml`][17].
