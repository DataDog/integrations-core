...
from datadog_checks.impala.config_models import discovery_overrides
from datadog_checks.impala.config_models.instance import InstanceConfig
from datadog_checks.impala.config_models.shared import SharedConfig
...
def _generated_candidates(service: Service) -> Iterator[dict[str, Any]]:
    ...
    for port in candidate_ports(service, [25000]):
        ctx = {'port': port}
        instance_data = {
            'service_type': 'daemon',
            'openmetrics_endpoint': 'http://{service.host}:{port.number}/metrics_prometheus'.format(
                service=service, **ctx
            ),
        }
