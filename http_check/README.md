...
| `tls_verify`                     | This setting requires SSL certificate validation and is enabled by default. If you want to skip SSL certificate validation, set this to `false`. This option is only used when gathering the response time/aliveness from the specified endpoint. **Note**: This setting doesn't apply to the `check_certificate_expiration` option. |
