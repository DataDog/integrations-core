...
| `check_certificate_expiration`   | When `check_certificate_expiration` is enabled, the service check checkww the expiration date of the SSL certificate. Note that this causes the SSL certificate to be validated, regardless of the value of the `disable_ssl_validation` setting. |
