...
class MoreUnixCheck(AgentCheck):
    ...
    def get_inode_info(self):
        with self.safe_os.open(self.proc_path_map['inode_info'], 'r') as inode_info:
    ...
    def get_stat_info(self):
        with self.safe_os.open(self.proc_path_map['stat_info'], 'r') as stat_info:
    ...
    def get_entropy_info(self):
        with self.safe_os.open(self.proc_path_map['entropy_info'], 'r') as entropy_info:
    ...
    def get_process_states(self):
        ...
        ps = self.safe_os.get_subprocess_output(['ps', '--no-header', '-eo', 'stat'], self.log)
        for state in ps[0]:
    ...
    def get_interrupts_info(self):
        with self.safe_os.open(self.proc_path_map['interrupts_info'], 'r') as interrupts_info:
