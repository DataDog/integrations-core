...
def test_scroll_events_errors(_, __, ___, ____, aggregator, instance, events_v3_p1):
    ...
    with mock.patch.object(check, "_http") as http_mock:
        http_mock.get.side_effect = HTTPClientError("error")
        check.scroll_events("", {}, {})
        aggregator.assert_service_check(
            name="cloud_foundry_api.api.can_connect",
    ...
    with mock.patch.object(check, "_http") as http_mock:
        http_mock.get.return_value = mock.MagicMock(
            raise_for_status=mock.MagicMock(side_effect=HTTPClientStatusError("error"))
        )
        check.scroll_events("", {}, {})
        aggregator.assert_service_check(
            name="cloud_foundry_api.api.can_connect",
    ...
    with mock.patch.object(check, "_http") as http_mock:
        events_res_p1 = mock.MagicMock()
        events_res_p1.json.return_value = events_v3_p1
        http_mock.get.side_effect = (events_res_p1, HTTPClientError("error"))
...
def test_discover_api_errors(_, __, instance):
    ...
    with mock.patch.object(check, "_http") as http_mock, pytest.raises(HTTPClientError):
        http_mock.get.side_effect = HTTPClientError("error")
        check.discover_api()

    with mock.patch.object(check, "_http") as http_mock, pytest.raises(HTTPClientStatusError):
        http_mock.get.return_value = mock.MagicMock(
            raise_for_status=mock.MagicMock(side_effect=HTTPClientStatusError("error"))
        )
        check.discover_api()
