...
class TestMain:
    ...
    def test_dry_run_does_not_output_batches(self, monkeypatch, tmp_path):
        self._base_env(monkeypatch, tmp_path, dry_run=True)
        with patch("release_dispatch.set_outputs") as mock_outputs, \
    ...
    def test_dry_run_writes_summary(self, monkeypatch, tmp_path):
        self._base_env(monkeypatch, tmp_path, dry_run=True)
        with patch("release_dispatch.set_outputs"), \
    ...
    def test_non_dry_run_outputs_batches(self, monkeypatch, tmp_path):
        ...
        with patch("release_dispatch.set_outputs") as mock_outputs, \
             patch("release_dispatch.write_summary"):
            release_dispatch.main()
        expected_batches = json.dumps([{"packages": ["postgres", "mysql"], "source_repo": "integrations-core", "source_repo_ref": "abc123"}])
        assert mock_outputs.mock_calls == [call(batches=expected_batches)]
