...
def test_check_metrics_1_14(aggregator, mock_metrics, mock_leader, mock_healthcheck_wrapper):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (HTTPClientStatusError('health check failed'), AgentCheck.CRITICAL, 'health check failed'),
        (HTTPClientConnectionError('connection refused'), AgentCheck.CRITICAL, 'connection refused'),
    ],
...
def test_healthcheck_client_carries_instance_tls_config(instance_overrides, expected_verify, expected_ignore_warning):
    ...
    """
    # __init__ probes the SLI endpoint unconditionally, and this test is about the healthcheck client.
    with mock.patch.object(KubeSchedulerCheck, 'detect_sli_endpoint', return_value=False):
        check = KubeSchedulerCheck(CHECK_NAME, {}, [{**instance, **instance_overrides}])
