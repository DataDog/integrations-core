...
class GrpcCheck(AgentCheck):
    ...
    def _get_server_metrics(self, channelz_stub, server):
        ...
        while not end:
            logger.debug(
                "Queryring sockets, server_id: %d, start_socket_id: %d",
                server.ref.server_id,
                last_socket_id,
            )
            sockets_stub = channelz_stub.GetServerSockets(
        ...
        self.monotonic_count(
            "server.calls_started", server_data.calls_started, tags=tags
        )
        self.monotonic_count(
            "server.calls_succeeded", server_data.calls_succeeded, tags=tags
        )
    ...
    def _channel_data_metrics(self, channel_type, channel_data, additional_tags):
        ...
        self.monotonic_count(
            f"{channel_type}.calls_succeeded",
            channel_data.calls_succeeded,
            tags=state_tags,
        )
        self.monotonic_count(
            f"{channel_type}.calls_failed", channel_data.calls_failed, tags=state_tags
        )

    def _get_subchannel_metrics(self, channelz_stub, subchannel_id, additional_tags):
        subchannel_response = channelz_stub.GetSubchannel(
            channelz_pb2.GetSubchannelRequest(subchannel_id=subchannel_id)
        )
    ...
    def _get_channels_metrics(self, channelz_stub):
        top_channels = channelz_stub.GetTopChannels(
        ...
        for channel in channels:
            ...
            for subchannel_ref in channel.subchannel_ref:
                self._get_subchannel_metrics(
                    channelz_stub, subchannel_ref.subchannel_id, additional_tags
                )
