...
def _expected_check_requests(
    *,
    ...
    include_workhorse: bool = False,
    include_sidekiq: bool = False,
    ...
    for _ in range(runs):
        ...
        if include_workhorse:
            requests.append(RecordedRequest('GET', GITLAB_WORKHORSE_PROMETHEUS_ENDPOINT, {'stream': True}))
        if include_sidekiq:
            requests.append(RecordedRequest('GET', GITLAB_SIDEKIQ_PROMETHEUS_ENDPOINT, {'stream': True}))
        if include_health:
            requests.extend(
                [
...
def test_check_workhorse_and_sidekiq_given_endpoints_configured_collects_their_metrics(
    ...
    fake_http = mock_data(use_openmetrics=True, runs=2, include_workhorse=True, include_sidekiq=True)
    config = get_config(True)
    instance = config['instances'][0]
    instance["openmetrics_endpoint"] = instance["prometheus_url"]
    ...
    fake_http.assert_requests(
        _expected_check_requests(use_openmetrics=True, runs=2, include_workhorse=True, include_sidekiq=True)
    )
    fake_http.assert_all_responses_consumed()
