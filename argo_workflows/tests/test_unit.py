...
V3_6_METRICS = {
    ('cronworkflows.concurrencypolicy_triggered', agg.MONOTONIC_COUNT),
    ('cronworkflows.triggered_total', agg.MONOTONIC_COUNT),
    ('deprecated.feature', agg.GAUGE),
    ('current_workflows', agg.GAUGE),
    ('is_leader', agg.GAUGE),
    #('k8s_request.duration', agg.MONOTONIC_COUNT),
    ('log_messages', agg.MONOTONIC_COUNT),
    ('pod.pending', agg.GAUGE),
    ('pods', agg.GAUGE),
    ('pods.count', agg.MONOTONIC_COUNT),
    ('queue_depth', agg.GAUGE),
    ('queue.longest_running', agg.GAUGE),
    ('queue.retries', agg.MONOTONIC_COUNT),
