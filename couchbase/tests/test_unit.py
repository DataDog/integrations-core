...
def test_check_gates_index_stats_on_major_version(
    instance_index_stats, mocker, aggregator, version, index_stats_called
):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"type": "rebalance", "status": "running", "subtype": "gracefulFailover"},
            {"rebalance": ("gracefulFailover", None)},
        ),
...
def test_collect_sync_gateway_metrics_global_and_per_db(instance_sg, aggregator, mocker):
    ...
    aggregator.assert_metric("couchbase.sync_gateway.cache.abandoned_seqs", value=3, tags=["db:db1"] + couchbase._tags)
